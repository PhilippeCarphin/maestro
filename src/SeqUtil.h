...
#define TRACE(x) do { if(DEBUG) {SeqUtil_TRACE x ;} }while(0)
#define LOCATE TRACE((TL_FULL_TRACE,"%s():[%s:%d]",__func__,__FILE__,__LINE__));
#define FUNCBEGIN \
   TRACE((TL_FULL_TRACE, "%s() BEGIN (%s,%d)\n", __func__,__FILE__,__LINE__));

#define FUNCEND \
   TRACE((TL_FULL_TRACE, "%s() END (%s,%d)\n", __func__,__FILE__,__LINE__));



TRACE((TL_FULL_TRACE,"%s Something something", str ));
