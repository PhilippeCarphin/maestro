...
namespace eval ::dkfFontSel {
      ...
      if { ${errorMsg} != "" } {
         tk_messageBox -message ${errorMsg} \
           ...
           if { [llength ${List_Exps}] > 0} {
             ...
             foreach _expPath $ll {
               set ModuleNode "/[ExpLayout_getEntryModule ${_expPath}]"
               set topWidget [ModuleFlowView_getTopLevel ${_expPath} ${ModuleNode}]
               if { [winfo exists ${topWidget}] } {
                 ModuleFlowControl_refreshSelected ${_expPath} ${ModuleNode} ${topWidget}
               }
            # refresh module tree
	...
	$classicEntry $w.sample.text -background [$w.sample cget -background] -relief flat -bd 0 -width 0 -highlightthickness 0 -takefocus 0 
	$w.sample.text insert 0 "ABCabcXYZxyz123"
	$w.sample create window 0 0 -anchor center -window $w.sample.text -height 40 \
		-tag foo
	...
	foreach {but code dir target text} {
	    ok  0  Down can OK
	    can 1  Up   ok  Cancel
	} {
	    set b $w.butnframe.$but
	    button $b -text $text -command [namespace code [list set Done $code]]
    ...
    proc 'load_resources {filebase} {
        global env
        set dirbase $env(MAESTRO_SRC)/xm/etc/config/
...
proc DkfFont_init {} {
  ...
  set font [dkf_chooseFont -apply "wm title ."]
  return
}
