/* logreader.c - log and statistics reader/creator used by the Maestro sequencer
 * software package.
 */
...
void insert_node(char S, char *node, char *loop, char *stime, char *btime,
                 char *etime, char *atime, char *itime, char *wtime,
                 char *dtime, char *exectime, char *submitdelay,
                 char *waitmsg) {
  ...
  if (MyListListNodes.Nodelength == -1) {
    ...
    if ((ptr_lhead = (struct _ListNodes *)malloc(sizeof(struct _ListNodes))) !=
        NULL) {
    ...
    for (ptr_LLtrotte = &MyListListNodes; ptr_LLtrotte != NULL;
         ptr_LLtrotte = ptr_LLtrotte->next) {
      if (ptr_LLtrotte->Nodelength == len) {
        found_len = 1;
        /* found same length, find  exact node if any  */
        for (ptr_Ltrotte = ptr_LLtrotte->Ptr_LNode; ptr_Ltrotte != NULL;
        ...
        if (found_node == 0 && (ptr_Lpreced->next = (struct _ListNodes *)malloc(
                                    sizeof(struct _ListNodes))) != NULL) {
    ...
    if (found_len == 0 && (ptr_LLpreced->next = (struct _ListListNodes *)malloc(
                               sizeof(struct _ListListNodes))) != NULL) {
      ptr_LLpreced->next->Nodelength = len;
      ptr_LLpreced->next->next = NULL;

      if ((ptr_Ltrotte = (struct _ListNodes *)malloc(
               sizeof(struct _ListNodes))) != NULL) {
...
void read_file(char *base) {
  ...
  if (regcomp(&regex,
              "^TIMESTAMP=([0-9]){8}.([0-9]){2}:([0-9]){2}:([0-9]){2}:SEQNODE=("
              "[^:]+):MSGTYPE=([^:]+):SEQLOOP=(.*)$",
  ...
  for (ptr = base; ptr < &base[pt.st_size]; ptr++) {
    ...
    if (regexec(&regex, log, 0, NULL, 0)) {
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "logreader read_file() ignoring malformed line: %s\n", log);
      ptr = qq;
      continue;
    }
...
void print_LListe(struct _ListListNodes MyListListNodes, FILE *outputFile) {
  ...
  int n, found_loopNode, shortestLength = 256, submit_time_found = 0,
                         begin_time_found = 0, end_time_found = 0;
  static char sbuffer[10], ebuffer[10], rbuffer[10];
  char *stats_output = NULL, *tmp_output, *tmp_statstring = NULL;
  char output_buffer[256];

  /* Traverse the list to find the top node (shortest nodelength) to get its
   * submit time to do the relative end time calculation */
  for (ptr_LLtrotte = &MyListListNodes; ptr_LLtrotte != NULL;
       ptr_LLtrotte = ptr_LLtrotte->next) {
  ...
  if (ptr_shortest_LLNode->Ptr_LNode != NULL) {
    n = sscanf(ptr_shortest_LLNode->Ptr_LNode->PNode.stime,
               "%4d%2d%2d.%2d:%2d:%2d", &ti.tm_year, &ti.tm_mon, &ti.tm_mday,
  ...
  for (ptr_LLtrotte = &MyListListNodes; ptr_LLtrotte != NULL;
    ...
    for (ptr_Ltrotte = ptr_LLtrotte->Ptr_LNode; ptr_Ltrotte != NULL;
      ...
      /* timing */
      n = sscanf(ptr_Ltrotte->PNode.stime, "%4d%2d%2d.%2d:%2d:%2d", &ti.tm_year,
      ...
      n = sscanf(ptr_Ltrotte->PNode.btime, "%4d%2d%2d.%2d:%2d:%2d", &ti.tm_year,
                 &ti.tm_mon, &ti.tm_mday, &ti.tm_hour, &ti.tm_min, &ti.tm_sec);
      if (n == 6) {
        begin_time_found = 1;
        Bepoch = mktime(&ti);
      ...
      n = sscanf(ptr_Ltrotte->PNode.etime, "%4d%2d%2d.%2d:%2d:%2d", &ti.tm_year,
                 &ti.tm_mon, &ti.tm_mday, &ti.tm_hour, &ti.tm_min, &ti.tm_sec);
      if (n == 6) {
        end_time_found = 1;
        Eepoch = mktime(&ti);
      ...
      if (strcmp(MyNodeLoopList.Node, "first") == 0) {
        ...
        if ((ptr_LEXHead =
                 (struct _LoopExt *)malloc(sizeof(struct _LoopExt))) != NULL) {
        ...
        for (ptr_NLHtrotte = &MyNodeLoopList; ptr_NLHtrotte != NULL;
             ptr_NLHtrotte = ptr_NLHtrotte->next) {
          if (strcmp(ptr_NLHtrotte->Node, ptr_Ltrotte->PNode.TNode) == 0) {
            if (strcmp(ptr_Ltrotte->PNode.loop, "") != 0) {
              for (ptr_LXHtrotte = ptr_NLHtrotte->ptr_LoopExt;
                   ptr_LXHtrotte != NULL; ptr_LXHtrotte = ptr_LXHtrotte->next) {
            ...
            for (ptr_LXHtrotte = ptr_NLHtrotte->ptr_LoopExt;
                 ptr_LXHtrotte != NULL; ptr_LXHtrotte = ptr_LXHtrotte->next) {
              ptr_LXHpreced = ptr_LXHtrotte;
            }
            if ((ptr_LXHpreced->next = (struct _LoopExt *)malloc(
                     sizeof(struct _LoopExt))) != NULL) {
              if (strcmp(ptr_Ltrotte->PNode.loop, "") != 0) {
        ...
        if (found_loopNode == 0 &&
            (ptr_NLHpreced->next = (struct _NodeLoopList *)malloc(
                 sizeof(struct _NodeLoopList))) != NULL) {
          strcpy(ptr_NLHpreced->next->Node, ptr_Ltrotte->PNode.TNode);
          ptr_NLHpreced->next->next = NULL;
          if ((ptr_LEXHead = (struct _LoopExt *)malloc(
                   sizeof(struct _LoopExt))) != NULL) {
  ...
  for (ptr_NLHtrotte = &MyNodeLoopList; ptr_NLHtrotte != NULL;
    ...
    for (ptr_LXHtrotte = ptr_NLHtrotte->ptr_LoopExt; ptr_LXHtrotte != NULL;
         ptr_LXHtrotte = ptr_LXHtrotte->next) {
      if (ptr_LXHtrotte->ignoreNode == 0) {
        ...
        if (ptr_LXHtrotte->submitdelay &&
            strlen(ptr_LXHtrotte->submitdelay) > 0) {
        ...
        if (ptr_LXHtrotte->deltafromstart &&
            strlen(ptr_LXHtrotte->deltafromstart) > 0) {
          tmp_statstring = sconcat(tmp_statstring, " deltafromstart ");
          tmp_statstring =
              sconcat(tmp_statstring, ptr_LXHtrotte->deltafromstart);
        }
        ...
        if (read_type == LR_SHOW_ALL || read_type == LR_SHOW_STATUS) {
          ...
          case 'a':
            fprintf(stdout, "%s {abort %s} ", ptr_LXHtrotte->Lext,
                    ptr_LXHtrotte->latime);
            break;
          case 'b':
            fprintf(stdout, "%s {begin %s} ", ptr_LXHtrotte->Lext,
                    ptr_LXHtrotte->lbtime);
            break;
          case 'e':
            fprintf(stdout, "%s {end %s} ", ptr_LXHtrotte->Lext,
                    ptr_LXHtrotte->letime);
            break;
          case 'i':
            fprintf(stdout, "%s {init %s} ", ptr_LXHtrotte->Lext,
                    ptr_LXHtrotte->litime);
            break;
          case 's':
            fprintf(stdout, "%s {submit %s} ", ptr_LXHtrotte->Lext,
                    ptr_LXHtrotte->lstime);
            break;
          case 'w':
            fprintf(stdout, "%s {wait %s {%s}} ", ptr_LXHtrotte->Lext,
                    ptr_LXHtrotte->lwtime, ptr_LXHtrotte->waitmsg);
            break;
          case 'd':
            fprintf(stdout, "%s {discret %s} ", ptr_LXHtrotte->Lext,
                    ptr_LXHtrotte->ldtime);
            break;
          case 'c':
            fprintf(stdout, "%s {catchup %s} ", ptr_LXHtrotte->Lext,
                    ptr_LXHtrotte->ldtime);
            break;
          }
        }
      ...
      if (outputFile != NULL) {
        fprintf(outputFile,
                "SEQNODE=/"
                "%s:MEMBER=%s:SUBMIT=%s:BEGIN=%s:END=%s:EXECTIME=%s:"
                "SUBMITDELAY=%s:DELTAFROMSTART=%s\n",
                ptr_NLHtrotte->Node, ptr_LXHtrotte->Lext, ptr_LXHtrotte->lstime,
                ptr_LXHtrotte->lbtime, ptr_LXHtrotte->letime,
                ptr_LXHtrotte->exectime, ptr_LXHtrotte->submitdelay,
                ptr_LXHtrotte->deltafromstart);
      }
    }
...
void getAverage(char *exp, char *datestamp) {
  ...
  snprintf(char_datestamp, 15, "%s",
           SeqDatesUtil_getPrintableDate(prev, -1, 0, 0, 0));

  SeqUtil_TRACE(TL_FULL_TRACE,
                "logreader parsing averages on exp: %s for datestamp: %s\n",
                exp, datestamp);
  ...
  if (f != NULL) {
    if (getStats(f) != 0) {
      SeqUtil_TRACE(TL_ERROR,
                    "logreader error while parsing the following avg file:\n");
...
int printAverage() {
  ...
  while (node_tmpptr != NULL) {
    ...
    if (time_tmpptr->deltafromstart &&
        strlen(time_tmpptr->deltafromstart) > 0) {
    ...
    free(output_line);
    if ((node_tmpptr->next == NULL) ||
        strcmp(node_tmpptr->next->node, node_tmpptr->node) != 0) {
      fprintf(stdout, "}");
    }
...
/*outputs a node-specific line formatted to fit in the tsv structure for
 * logreader.tcl*/
char *getNodeAverageLine(char *node, char *member) {
  ...
  while ((strcmp(node_tmpptr->node + 1, node) != 0 ||
          strcmp(node_tmpptr->member, member) != 0) &&
         node_tmpptr->next != NULL) {
    node_tmpptr = node_tmpptr->next;
  }
  if (strcmp(node_tmpptr->node + 1, node) == 0 &&
      strcmp(node_tmpptr->member, member) == 0) {
...
void computeAverage(char *exp, char *datestamp, int stats_days,
  ...
  SeqUtil_TRACE(TL_FULL_TRACE,
                "logreader computing averages on exp: %s for datestamp: %s "
                "since last %d days\n",
  ...
  for (i = 0; i < stats_days; i++) {
    ...
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "logreader collecting stats for datestamp %s in file %s\n",
    ...
    if (f != NULL) {
      if (getStats(f) != 0) {
        SeqUtil_TRACE(
            TL_ERROR,
            "logreader error while parsing the following stats file:\n");
        SeqUtil_TRACE(TL_ERROR, "%s\n", full_path);
      }
      ++count;
      fclose(f);
  ...
  if (count == 0) {
    fprintf(stderr,
            "Unable to calculate average; missing required statistics files "
            "under %s/stats\n",
            exp);
    exit(1);
  }
...
int getStats(FILE *_stats) {
  ...
  while ((fgets(tmpline, sizeof(tmpline), _stats) != NULL) &&
         (strstr(tmpline, "SEQNODE") != NULL)) {
    if (parseStatsLine(tmpline) != 0) {
      SeqUtil_TRACE(
          TL_ERROR,
          "logreader parsing error at the following stats line:\n%s \n",
          tmpline);
    }
  }
  return 0;
...
int parseStatsLine(char line[1024]) {
  ...
  char tmp_node[] = "SEQNODE=", tmp_member[] = "MEMBER=",
       tmp_stime[] = "SUBMIT=", tmp_btime[] = "BEGIN=", tmp_etime[] = "END=",
       tmp_exectime[] = "EXECTIME=", tmp_submitdelay[] = "SUBMITDELAY=",
       tmp_deltafromstart[] = "DELTAFROMSTART=";

  int node_str_len = strlen(tmp_node), member_str_len = strlen(tmp_member),
      stime_str_len = strlen(tmp_stime), btime_str_len = strlen(tmp_btime),
      submitdelay_str_len = strlen(tmp_submitdelay),
      deltafromstart_str_len = strlen(tmp_deltafromstart),
      etime_str_len = strlen(tmp_etime),
  ...
  int nodelen, memberlen, stimelen, btimelen, etimelen, exectimelen,
  ...
  if (!(nodeptr || memberptr || stimeptr || btimeptr || etimeptr ||
        exectimeptr || submitdelay || deltafromstartptr)) {
    SeqUtil_TRACE(TL_ERROR,
                  "parseStatsLine error at the following stats line:\n%s \n",
                  line);
    return -1;
  }
  deltafromstartlen = strlen(deltafromstartptr) - 1;
  submitdelaylen = strlen(submitdelayptr) - deltafromstartlen -
                   deltafromstart_str_len - 1 - 1;
  exectimelen = strlen(exectimeptr) - submitdelaylen - deltafromstartlen -
                deltafromstart_str_len - submitdelay_str_len - 1 - 1 - 1;
  etimelen = strlen(etimeptr) - exectimelen - submitdelaylen -
             deltafromstartlen - deltafromstart_str_len - submitdelay_str_len -
             exectime_str_len - 1 - 1 - 1 - 1;
  btimelen = strlen(btimeptr) - etimelen - exectimelen - submitdelaylen -
             deltafromstartlen - deltafromstart_str_len - submitdelay_str_len -
             exectime_str_len - etime_str_len - 1 - 1 - 1 - 1 - 1;
  stimelen = strlen(stimeptr) - btimelen - etimelen - exectimelen -
             submitdelaylen - deltafromstartlen - deltafromstart_str_len -
             submitdelay_str_len - exectime_str_len - etime_str_len -
             btime_str_len - 1 - 1 - 1 - 1 - 1 - 1;
  memberlen = strlen(memberptr) - stimelen - btimelen - etimelen - exectimelen -
              submitdelaylen - deltafromstartlen - deltafromstart_str_len -
              submitdelay_str_len - exectime_str_len - etime_str_len -
              btime_str_len - stime_str_len - 1 - 1 - 1 - 1 - 1 - 1 - 1;
  nodelen = strlen(nodeptr) - memberlen - stimelen - btimelen - etimelen -
            exectimelen - submitdelaylen - deltafromstartlen -
            deltafromstart_str_len - submitdelay_str_len - exectime_str_len -
            etime_str_len - btime_str_len - stime_str_len - member_str_len - 1 -
            1 - 1 - 1 - 1 - 1 - 1 - 1;
  ...
  if (node == NULL || member == NULL || stime == NULL || btime == NULL ||
      etime == NULL || exectime == NULL || submitdelay == NULL ||
      deltafromstart == NULL) {
  ...
  ret = addStatsNode(node, member, stime, btime, etime, exectime, submitdelay,
                     deltafromstart);
  return ret;
}

/*add stat node to linked list plus set root node if the list is empty*/
int addStatsNode(char *node, char *member, char *stime, char *btime,
  ...
  if (rootStatsNode == NULL) {
    ...
    while ((strcmp(stats_node_ptr->node, node) != 0 ||
            strcmp(stats_node_ptr->member, member) != 0) &&
           stats_node_ptr->next != NULL) {
      stats_node_ptr = stats_node_ptr->next;
    }

    if (strcmp(stats_node_ptr->node, node) == 0 &&
        strcmp(stats_node_ptr->member, member) == 0) {
...
/*final step to the computation of averages; it calculates from the linked list
 * and outputs results to avg file*/
int processStats(char *exp, char *datestamp, FILE *output_file) {
  StatsNode *node_tmpptr;
  PastTimes *time_tmpptr;
  char *begin = NULL, *end = NULL, *submitdelay = NULL, *exectime = NULL,
       *deltafromstart = NULL, *submit = NULL;
  int int_begin[30], int_end[30], int_submitdelay[30], int_exectime[30],
      int_deltafromstart[30], int_submit[30];
  ...
  while (node_tmpptr != NULL) {
    ...
    if (avg_counter > 30) {
      raiseError("ERROR: Maximum average count is 30. Please reduce the amount "
                 "of days you wish to calculate the mean. \n");
    }
    ...
    while (time_tmpptr != NULL) {
      ...
      int_deltafromstart[iter_counter] =
          charToSeconds(time_tmpptr->deltafromstart);
      iter_counter++;
      time_tmpptr = time_tmpptr->next;
    }
    ...
    SeqUtil_printOrWrite(output_file,
                         ...
                         "EXECTIME=%s:SUBMITDELAY=%s:DELTAFROMSTART=%s\n",
                         node_tmpptr->node, node_tmpptr->member, submit, begin,
                         end, exectime, submitdelay, deltafromstart);
    node_tmpptr = node_tmpptr->next;
  }
...
char *secondsToChar(int seconds) {
  ...
  buffer[8] = '\0';
  snprintf(buffer, 9, "%02i:%02i:%02i", result_hour, result_minute,
           result_second);
...
void reset_branch(char *node, char *ext) {
  ...
  for (ptr_LLtrotte = &MyListListNodes; ptr_LLtrotte != NULL;
    ...
    while (ptr_Ltrotte != NULL) {
      ...
      if (strcmp(node, tmp_prev_list->PNode.TNode) == 0 &&
          strncmp(ext, tmp_prev_list->PNode.loop, strlen(ext)) == 0) {

        tmp_prev_list->PNode.ignoreNode = 1;
        SeqUtil_TRACE(TL_FULL_TRACE,
                      "logreader reset branch done on node: %s ext: %s \n",
                      node, ext);
      }
    }
  }
 ...
 *  clobberFile -- is there a filecheck before writing to the output. 0 = don't
 * clobber
 *
 */
void logreader(char *inputFilePath, char *outputFilePath, char *exp,
  ...
  if (exp == NULL || datestamp == NULL) {
    raiseError(
        "logreader: exp and datestamp must be defined to use logreader\n");
  }

  if (strlen(datestamp) > 14)
    raiseError(
        "logreader: datestamp format error. Should be YYYYMMDDHHmmss, is %s\n",
        datestamp);
  ...
  if (read_type == LR_SHOW_STATS) {
    if (outputFilePath == NULL) {
      ...
      if ((clobberFile == 0) && (access(optional_output_path, R_OK) == 0)) {
        SeqUtil_TRACE(TL_FULL_TRACE, "logreader: no clobber output flag set "
                                     "and file exists, writing to /dev/null\n");
  ...
  if ((read_type == LR_SHOW_ALL) || (read_type == LR_SHOW_STATUS) ||
    ...
    if ((base = mmap(NULL, pt.st_size, PROT_READ, MAP_SHARED, fp, (off_t)0)) ==
        (char *)MAP_FAILED) {
      if (errno == EINVAL) {
        fprintf(stdout, "\n");
        close(fp);
