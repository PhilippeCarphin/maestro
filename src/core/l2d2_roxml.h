 ...
 *
 *	// roxml_get_content allocate a buffer and store the content in it if no
 *buffer was given printf("root content = '%s'\n", roxml_get_content(root, NULL,
 *0, &len));
 *
 *	// release the last allocated buffer even if no pointer is maintained by
 *the user roxml_release(RELEASE_LAST);
 *
 ...
 * \warning the file is not fully copied and thus, it should stay untouched
 * until roxml_close is called on the corresponding XML tree. \param filename
 * the XML document to load \return the root node or NULL \see roxml_close \see
 * roxml_load_fd \see roxml_load_buf
 */
 ...
 * It release all memory allocated during roxml_load_doc or roxml_load_file or
 * roxml_add_node. All nodes from the tree are not available anymore. \param n
 * is any node of the tree to be cleaned \return void \see roxml_load_doc \see
 * roxml_load_buf \see roxml_add_node
 */
 ...
 * The root is defined as a virtual node that contains all first rank nodes if
document is
...
\endverbatim
 * will be processed sucessfully and the root node will have 2 children: data1
and data2
 *
 * if document was:
 * \verbatim
...
\endverbatim
 * In this case, the node "doc" will be the root, and will contain 2 children:
data1 and data2
 *
 * For a document to be valid, following conditions must be met:
 * <ul>
 * <li>the first node is a processing instruction with the string "xml" as
target application.</li>
 * <li>there is only one ELM node containing all document (but there may be
several process-instructions or comments)</li>
 * </ul>
 * \param n is one node of the tree
 ...
 * 	}
 * 	if(strcmp(roxml_get_content(node1_ns, NULL, 0, NULL),
"http://www.test.org") == 0) {
 ...
 * The namespace must be previously defined in the xml tree in an ancestor of
 * node n.
 *
 * \param n is one node of the tree
 ...
 * - it will update all element and attribute that are descendant from current
 * node
 * - namespace will be applied to all new node added as descendant as current
 * node
 */
 ...
 * This function returns a given chld of a node etheir by name, or the nth
child.
 *
 * \param n is one node of the tree
 ...
 * This function return the name of the node or fill the current buffer with it
 * if not NULL. if name is NULL, the function will allocate a buffer that user
 * should free using roxml_release when no longer needed. depending on node type
 * it will return: <ul> <li> ROXML_ELM_NODE: returns the node name</li> <li>
 * ROXML_ATTR_NODE: returns the attribute name</li> <li> ROXML_PI_NODE: returns
 * the process-instruction targeted application</li> <li> ROXML_TXT_NODE:
 * returns NULL (or empty string if you provided a buffer in buffer param)</li>
 * <li> ROXML_CMT_NODE: returns NULL (or empty string if you provided a buffer
 * in buffer param)</li> <li> ROXML_NS_NODE: returns the namespace alias
 * associated with the ns node </li>
 * </ul>
 * Be carreful as if your buffer is too short for the returned string, it will
 * be truncated \param n is one node of the tree \param buffer a buffer pointer
 * or NULL if has to be auto allocated \param size the size of buffer pointed by
 * buffer if not NULL \return the name of the node (return our buffer pointer if
 * it wasn't NULL) \see roxml_release
 */
char *ROXML_API roxml_get_name(node_t *n, char *buffer, int size);

/** \brief content getter function
 *
 * \fn char * ROXML_API roxml_get_content(node_t *n, char * buffer, int bufsize,
 * int * size);
 *
 * This function returns the content of a node.;
 * if the returned pointer is NULL then the node either has no content or this
 * function is irrelevant for this kind of node. depending on node type it will
 * return: <ul> <li> ROXML_ELM_NODE: returns the concatenation of all direct
 * text node children</li> <li> ROXML_ATTR_NODE: returns the attribute
 * value</li> <li> ROXML_PI_NODE: returns the process-instruction
 * instruction</li> <li> ROXML_TXT_NODE: returns the text content of the
 * node</li> <li> ROXML_CMT_NODE: returns the text content of the comment</li>
 ...
 * \param n is one node of the tree
 * \param buffer is the buffer where result will be written or NULL for internal
 * allocation \param bufsize the size if using custom buffer \param size the
 * actual size of copied result. returned size should be less that buffer size
 * since roxml_get_content will add the \0. if buffer was not NULL and size ==
 * buf_size, then given buffer was too small and node content was truncated
 ...
 *
 * This function returns the number of nodes matching type flag contained in a
given node
 ...
 * 	node_t * root = roxml_load_doc("/tmp/doc.xml");
 *
 *	int all_nodes_1 = roxml_get_nodes_nb(root, ROXML_ELM_NODE |
ROXML_CMT_NODE | ROXML_PI_NODE | ROXML_TXT_NODE | ROXML_ATTR_NODE); *	int
all_nodes_2 = roxml_get_nodes_nb(root, ROXML_ALL_NODES);
 *
 ...
 *	int almost_all_nodes = roxml_get_nodes_nb(root, ROXML_ALL_NODES &
~ROXML_ELM_NODE); *	// here almost_all_nodes = 2 since we have one comment
node and one processing-instruction node *	if(almost_all_nodes == 2) {
 ...
 *
 * \fn char* ROXML_API roxml_get_nodes(node_t *n, int type, char * name, int
 * nth); This function get the nth node matching type contained in a node, or
 * the first node named name. All other roxml_get_* are wrapper to this
 * function. When asking for several node type (let say ROXML_ALL_NODE), all
 * ROXML_ATTR_NODE will be placed first, then, all other nodes will come mixed,
 * depending on xml document order.
 *
 * \param n is one node of the tree
 * \param type is the bitmask of node types we want to consider
 * \param name is the name of the child to get. This parameter is only relevant
 * for node with types: \ref ROXML_ELM_NODE, \ref ROXML_ATTR_NODE, \ref
 * ROXML_PI_NODE \param nth the id of attribute to read \return the node
 * corresponding to name or id (if both are set, name is used) \see
 * roxml_get_attr \see roxml_get_chld \see roxml_get_txt \see roxml_get_cmt \see
 * roxml_get_pi
 */
 ...
 * \return the attribute corresponding to name or id (if both are set, name is
used)
 *
 * example:
 * given the following xml file
 ...
 * This function return a node set (table of nodes) corresponding to a given
 * xpath. resulting node set should be roxml_release when not used anymore (but
 * not individual nodes) \param n is one node of the tree \param path the xpath
 * to use \param nb_ans the number of results \return the node table or NULL
 *
 ...
 * This function tells if a node is an \ref ROXML_ATTR_NODE, \ref
 * ROXML_TXT_NODE, \ref ROXML_PI_NODE, \ref ROXML_CMT_NODE or \ref
 * ROXML_ELM_NODE. Warning: ROXML_CDATA_NODE are special. They return a type as
 * ROXML_TXT_NODE. \param n is the node to test \return the node type
 */
 ...
 * if using roxml_release on a variable non roxml_mallocated, nothing will
 * happen (ie variable won't be freed) \param data the pointer to delete or NULL
 * or \ref RELEASE_ALL or \ref RELEASE_LAST \return void
 */
void ROXML_API roxml_release(void *data);

/** \brief add a node to the tree
 *
 * \fn roxml_add_node(node_t * parent, int position, int type, char *name, char
*value);
 * this function add a new node to the tree. This will not update de buffer or
file,
 * only the RAM loaded tree. One should call \ref roxml_commit_changes to save
modifications.
 * If the parent node is an \ref ROXML_ELM_NODE, then, new node will be added as
a child. Else
 * the node will be added as a sibling of the parent node. In the later case,
position parameter describes
 * the position in the sibling list, instead of position in the children list.
 * \param parent the parent node
 * \param position the position as a child of parent 1 is the first child, 0 for
auto position at the end of children list...
 * \param type the type of node between \ref ROXML_ATTR_NODE, \ref
ROXML_ELM_NODE, \ref ROXML_TXT_NODE, \ref ROXML_CDATA_NODE, \ref ROXML_PI_NODE,
\ref ROXML_CMT_NODE, \ref ROXML_NSDEF_NODE, \ref ROXML_NS_NODE.
 * \param name the name of the node (mandatory for \ref ROXML_ATTR_NODE and \ref
ROXML_ELM_NODE and \ref ROXML_PI_NODE and \ref ROXML_NSDEF_NODE and \ref
ROXML_NS_NODE only)
 * \param value the text content (mandatory for \ref ROXML_TXT_NODE, \ref
ROXML_CDATA_NODE, \ref ROXML_CMT_NODE, \ref ROXML_ATTR_NODE and \ref
ROXML_NSDEF_NODE optional for \ref ROXML_ELM_NODE, \ref ROXML_PI_NODE). The text
content for an attribute is the attribute value
 ...
 * - \ref ROXML_ELM_NODE take at least a node name. the parameter value is
optional and represents the text content.
 * - \ref ROXML_TXT_NODE ignore the node name. the parameter value represents
the text content.
 * - \ref ROXML_CDATA_NODE ignore the node name. the parameter value represents
the text content that will be encapsulated in CDATA tags.
 * - \ref ROXML_CMT_NODE ignore the node name. the parameter value represents
the comment.
 * - \ref ROXML_PI_NODE take the node name as process-instruction target. the
parameter value represents the content of processing-instruction.
 * - \ref ROXML_ATTR_NODE take an attribute name. and the attribute value as
given by parameter value.
 * - \ref ROXML_NSDEF_NODE take an attribute name (empty string for default
namespace). and the namespace value as given by parameter value.
 * - \ref ROXML_NS_NODE take an attribute name (empty string for default
namespace).
 *
 ...
 * 	node_t * tmp = roxml_add_node(root, 0, ROXML_CMT_NODE, NULL, "sample XML
file");
 ...
 * \endcode
 *
 * If you need a valid XML doc, just start by adding a corresponding
process-instruction.
 ...
 * int main(void)
 * {
 *	node_t * root = roxml_add_node(NULL, 0, ROXML_PI_NODE, "xml",
"version=\"1.0\" encoding=\"UTF-8\"");
 *	node_t * node = roxml_add_node(root, 0, ROXML_CMT_NODE, NULL, "sample
file"); *	node = roxml_add_node(root, 0, ROXML_ELM_NODE, "doc", "basic
content"); *	roxml_add_node(node, 0, ROXML_ELM_NODE, "item", NULL);
 ...
 * from the file or buffer until the roxml_commit_changes is called, but it
 * won't be visible anymore in the XML tree. \param n the node to delete \return
 * \see roxml_add_node
 * \see roxml_commit_changes
 *
 * \warning when removing a nsdef node, all node using this namespace will be
 * updated and inherit their parent namespace
 */
 ...
 * this function sync changes from the RAM tree to the given buffer or file in
human or one-line format
 * The tree will be processed starting with the root node 'n' and following with
all its children or if n is the root, all its siblings and children.
 * The tree will be dumped to a file if 'dest' is not null and contains a valid
path.
 * The tree will be dumped to a buffer if 'buffer' is not null. the buffer is
allocated by the library
 * and a pointer to it will be stored into 'buffer'. The allocated buffer can be
freed usinf free()
 * \warning in the case of a tree loaded using roxml_load_doc, the
roxml_commit_changes cannot be done to that same file
 * since it may override datas it need. This usually result in a new file filled
with garbages. The solution is to write it to a temporary file and rename it
after roxml_close the current tree.
 *
 * \param n the root node of the tree to write
 * \param dest the path to a file to write tree to
 * \param buffer the adress of a buffer where the tree will be written. This
buffer have to be freed after use
 * \param human 0 for one-line tree, or 1 for human format (using tabs,
newlines...)
