 ...
 *
 * \fn node_t* ROXML_API roxml_load(node_t *current_node, FILE *file, char
 * *buffer); This function load a document and all the corresponding nodes file
 * and buffer params are exclusive. You usualy want to load either a file OR a
 * buffer \param current_node the XML root \param file file descriptor of
 * document \param buffer address of buffer that contains xml \return the root
 * node or NULL see roxml_close
 */
 ...
 * this function read inside a xml doc (file or buffer) and fill the given
 * buffer \param pos the pos in the xml document \param size the size of the
 * data to read \param buffer the destination buffer \param node the node that
 * belong to the tree we want to read to \return the number of bytes read
 */
 ...
 */
xpath_node_t *ROXML_INT roxml_set_axes(xpath_node_t *node, char *axes,
                                       int *offset);
 ...
 * this function release a previously required id and remove all id token from
 * the pool \param root the root of the tree that was used for id request \param
 * pool the result from xpath search using this id \param pool_len the number of
 * node in pool \param req_id the id to release \return
 */
void ROXML_INT roxml_release_id(node_t *root, node_t **pool, int pool_len,
                                int req_id);
 ...
 *
 * \fn roxml_validate_axes(node_t *root, node_t *candidat, node_t ***ans, int
 * *nb, int *max, xpath_node_t *xn, int req_id); this function validate if an
 * axe is matching the current node \param root the root node \param candidat
 * the node to test \param ans the pointer to answers pool \param nb the number
 * of answers in pool \param max the current size of the pool \param xn the
 * xpath node to test \param req_id the pool id \return 1 if axe is validated,
 * else 0
 */
int ROXML_INT roxml_validate_axes(node_t *root, node_t *candidat, node_t ***ans,
                                  int *nb, int *max, xpath_node_t *xn,
                                  int req_id);

/** \brief real xpath validation function
 *
 * \fn roxml_check_node(xpath_node_t *xp, node_t *root, node_t *context, node_t
 ****ans, int *nb, int *max, int ignore, int req_id); this function perform the
 *xpath test on a tree \param xp the xpath nodes to test \param root the root
 *node \param context the current context node \param ans the pointer to answers
 *pool \param nb the number of answers in pool \param max the current size of
 *the pool \param ignore a flag for some axes that are going thru all document
 * \param req_id the pool id
 * \return
 */
void ROXML_INT roxml_check_node(xpath_node_t *xp, node_t *root, node_t *context,
                                node_t ***ans, int *nb, int *max, int ignore,
                                int req_id);

/** \brief space printing function
 *
 * \fn roxml_print_space(FILE *f, char ** buf, int * offset, int * len, int
 * lvl); this function add some space to output when committing change in human
 * format \param f the file pointer if any \param buf the pointer to string if
 * any \param offset the current offset in stream \param len the total len of
 * buffer if any \param lvl the level in the tree \return
 */
void ROXML_INT roxml_print_space(FILE *f, char **buf, int *offset, int *len,
                                 int lvl);

/** \brief string writter function
 *
 * \fn roxml_write_string(char ** buf, FILE * f, char * str, int *offset, int *
 * len); this function write a string to output when committing change \param f
 * the file pointer if any \param buf the pointer to string if any \param str
 * the string to write \param offset the current offset in stream \param len the
 * total len of buffer if any \return
 */
void ROXML_INT roxml_write_string(char **buf, FILE *f, char *str, int *offset,
                                  int *len);

/** \brief tree write function
 *
 * \fn roxml_write_node(node_t * n, FILE *f, char ** buf, int human, int lvl,
 * int *offset, int *len); this function write each node of the tree to output
 ...
 * \return
 */
void ROXML_INT roxml_write_node(node_t *n, FILE *f, char **buf, int human,
                                int lvl, int *offset, int *len);
 ...
 *
 * \fn roxml_compute_and(node_t * root, node_t **node_set, int *count, int
 * cur_req_id, int prev_req_id); this function computes the AND of two node
 * pools. The resulting pool will have the same ID as cur_req_id. \param root
 * the root of the tree \param node_set the node set containing the 2 pools
 ...
 */
void ROXML_INT roxml_compute_and(node_t *root, node_t **node_set, int *count,
                                 int cur_req_id, int prev_req_id);

/** \brief node set or function
 *
 * \fn roxml_compute_or(node_t * root, node_t **node_set, int *count, int
 * req_id, int glob_id); this function computes the OR of two node pools. The
 * resulting pool will have the same ID as glob_id. \param root the root of the
 * tree \param node_set the node set containing the 2 pools \param count number
 * of node in the node set \param req_id the id of the first group \param
 * glob_id the id of the second group \return
 */
void ROXML_INT roxml_compute_or(node_t *root, node_t **node_set, int *count,
                                int req_id, int glob_id);
 ...
 *
 * \fn roxml_exec_xpath(node_t *root, node_t *n, xpath_node_t *xpath, int index,
 * int * count); this function exec a decoded xpath strcuture \param root the
 * root of the tree \param n the context node \param xpath the xpath structure
 * \param index the number of xpath condition in string
 * \param count the pointer to a variable that is filled with the resulting node
 * number \return  the resulting node set that have to be freed with
 * roxml_release
 */
node_t **ROXML_INT roxml_exec_xpath(node_t *root, node_t *n,
                                    xpath_node_t *xpath, int index, int *count);
 ...
 * this function create a new node upon finding new opening sign. It closes
 * previous node if necessary \param context the parsing context \param position
 * the position in the file \return
 */
