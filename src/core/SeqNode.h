...
typedef enum _SeqLoopType { Numerical, Date, SwitchType } SeqLoopType;
...
typedef struct _SeqNodeData {
  ...
  char *npey;
  char *omp;
  /*extra soumet arguments provided via command-line, resource file or
   * interface*/
  char *soumetArgs;
  char *workq;
  char *workerPath;
...
void SeqNode_printDependencies(SeqNodeDataPtr _nodeDataPtr, FILE *filename,
                               int isPrettyPrint);
...
extern void SeqNode_generateConfig(const SeqNodeDataPtr _nodeDataPtr,
                                   const char *flow, const char *filename);
