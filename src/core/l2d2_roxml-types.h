 ...
 * the chunk that matched the key, and the context as a void. It should return
 * the number of handled bytes or 0 if doesn't want to handle this key
 */
...
typedef struct _roxml_load_ctx {
  ...
  int empty_text_node;   /*!< if text node is empty (only contains tabs, spaces,
