...
roxml_parser_item_t *roxml_append_parser_item(roxml_parser_item_t *head,
                                              char *key,
...
void roxml_parser_free(roxml_parser_item_t *head) { free(head); }
...
roxml_parser_item_t *roxml_parser_prepare(roxml_parser_item_t *head) {
  ...
  table = (roxml_parser_item_t *)malloc(sizeof(roxml_parser_item_t) *
                                        (head->def_count));
...
int roxml_parse_line(roxml_parser_item_t *head, char *line, int len,
                     void *ctx) {
...
int _func_xpath_condition_or(char *chunk, void *data) {
  ...
  if (strncmp(chunk, ROXML_COND_OR, len) == 0) {
    if (roxml_is_separator(*(chunk - 1)) &&
        roxml_is_separator(*(chunk + len))) {
      if (!ctx->bracket && !ctx->quoted && !ctx->dquoted) {
        ...
        tmp_node =
            (xpath_node_t *)calloc(ctx->nbpath + 1, sizeof(xpath_node_t));
...
static int _func_xpath_operators(char *chunk, void *data, int operator,
                                 int operator_bis) {
  roxml_xpath_ctx_t *ctx = (roxml_xpath_ctx_t *)data;
  int cur = 0;
  if (!ctx->bracket && !ctx->quoted && !ctx->dquoted) {
...
int _func_xpath_operator_equal(char *chunk, void *data) {
  ...
  fprintf(stderr, "calling func %s chunk %c\n", __func__, chunk[0]);
#endif
  return _func_xpath_operators(chunk, data, ROXML_OPERATOR_EQU,
                               ROXML_OPERATOR_EQU);
}
...
int _func_xpath_operator_sup(char *chunk, void *data) {
  ...
  fprintf(stderr, "calling func %s chunk %c\n", __func__, chunk[0]);
#endif
  return _func_xpath_operators(chunk, data, ROXML_OPERATOR_SUP,
                               ROXML_OPERATOR_ESUP);
}
...
int _func_xpath_operator_inf(char *chunk, void *data) {
  ...
  fprintf(stderr, "calling func %s chunk %c\n", __func__, chunk[0]);
#endif
  return _func_xpath_operators(chunk, data, ROXML_OPERATOR_INF,
                               ROXML_OPERATOR_EINF);
}
...
int _func_xpath_operator_diff(char *chunk, void *data) {
  ...
  fprintf(stderr, "calling func %s chunk %c\n", __func__, chunk[0]);
#endif
  return _func_xpath_operators(chunk, data, ROXML_OPERATOR_DIFF,
                               ROXML_OPERATOR_DIFF);
}
...
int _func_xpath_operator_add(char *chunk, void *data) {
  ...
  if (ctx->bracket && !ctx->quoted && !ctx->dquoted) {
    if (ctx->new_cond->func != ROXML_FUNC_XPATH) {
      if ((ctx->new_cond->func == ROXML_FUNC_LAST) ||
          (ctx->new_cond->func == ROXML_FUNC_FIRST)) {
...
int _func_load_close_node(char *chunk, void *data) {
  ...
  case STATE_NODE_NAME:
    ...
    context->current_node =
        roxml_append_node(context->current_node, context->candidat_node);
    break;
    ...
    if ((context->mode != MODE_COMMENT_DQUOTE) ||
      ...
      if (context->inside_node_state == STATE_INSIDE_VAL) {
        ...
        if (context->content_quoted) {
          ...
          to_be_closed = roxml_create_node(context->pos, context->src,
                                           ROXML_ATTR_NODE | context->type);
        }
...
int _func_load_colon(char *chunk, void *data) {
  ...
  if (context->state == STATE_NODE_NAME) {
    ...
    context->candidat_node->ns =
        roxml_lookup_nsdef(context->namespaces, context->curr_name);
    if (!context->candidat_node->ns) {
      ...
      context->candidat_node->ns = roxml_create_node(
          0, nsname, ROXML_NSDEF_NODE | ROXML_PENDING | ROXML_INVALID);
    }
    ...
    if (context->inside_node_state == STATE_INSIDE_ARG) {
      context->inside_node_state = STATE_INSIDE_ARG_BEG;
      if ((context->curr_name_len == 5) &&
          (strncmp(context->curr_name, "xmlns", 5) == 0)) {
        context->candidat_arg->type |= ROXML_NS_NODE;
        context->nsdef = 1;
      } else {
        context->candidat_arg->ns =
            roxml_lookup_nsdef(context->namespaces, context->curr_name);
...
int _func_load_default(char *chunk, void *data) {
  ...
  case STATE_NODE_BEG:
    ...
    while (!ROXML_WHITE(chunk[cur]) && (chunk[cur] != '>') &&
  ...
  case STATE_NODE_ATTR:
    if (context->inside_node_state == STATE_INSIDE_ARG_BEG) {
      if (context->nsdef) {
        ...
        context->candidat_arg = roxml_create_node(
            context->pos - 1, context->src, ROXML_ATTR_NODE | context->type);
      ...
      context->curr_name = chunk;
      while ((chunk[cur] != '=') && (chunk[cur] != '>') &&
             (chunk[cur] != ':') && (chunk[cur] != '\0')) {
        cur++;
      }
      context->curr_name_len = cur;
      if (context->nsdef) {
        roxml_ns_t *ns =
            calloc(1, sizeof(roxml_ns_t) + (1 + context->curr_name_len));
        ns->id = ROXML_NS_ID;
        ...
        context->nsdef = 0;
        if (context->candidat_node->ns) {
          if ((context->candidat_node->ns->type & ROXML_INVALID) ==
              ROXML_INVALID) {
            if (strcmp(context->candidat_arg->prnt->ns->src.buf, ns->alias) ==
                0) {
              roxml_free_node(context->candidat_node->ns);
      ...
      if (context->mode != MODE_COMMENT_NONE) {
        ...
        context->candidat_val = roxml_create_node(
            context->pos + 1, context->src, ROXML_TXT_NODE | context->type);
      } else {
        context->candidat_val = roxml_create_node(
            context->pos, context->src, ROXML_TXT_NODE | context->type);
      }
      context->candidat_val =
          roxml_append_node(context->candidat_arg, context->candidat_val);
      context->inside_node_state = STATE_INSIDE_VAL;
    } else if ((context->inside_node_state == STATE_INSIDE_ARG) &&
               (chunk[0] == '=')) {
