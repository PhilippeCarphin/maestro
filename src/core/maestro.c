/* maestro.c - Main engine of the state machine that is the Maestro sequencer
 * software package.
 */
...
static const char *CATCHUP_DISCR_MSG =
    "DISCRETIONARY: this job is not scheduled to run";
static const char *CATCHUP_UNSUBMIT_MSG =
    "CATCHUP mode: this job will not be submitted";
...
static int go_abort(char *_signal, char *_flow,
                    const SeqNodeDataPtr _nodeDataPtr);
static int go_initialize(char *_signal, char *_flow,
                         const SeqNodeDataPtr _nodeDataPtr);
static int go_begin(char *_signal, char *_flow,
                    const SeqNodeDataPtr nodeDataPtr);
static int go_end(char *_signal, char *_flow,
                  const SeqNodeDataPtr _nodeDataPtr);
static int go_submit(const char *signal, char *_flow,
                     const SeqNodeDataPtr nodeDataPtr, int ignoreAllDeps);
...
static void setAbortState(const SeqNodeDataPtr _nodeDataPtr,
                          char *current_action);
static void setWaitingState(const SeqNodeDataPtr _nodeDataPtr,
                            const char *waited_one, const char *waited_status);
static void clearAllOtherStates(SeqNodeDataPtr _nodeDataPtr, char *fullNodeName,
                                char *originator, char *current_signal);
int isNodeXState(const char *node, const char *loopargs, const char *datestamp,
                 const char *exp, const char *state);
...
static void submitLoopSetNodeList(const SeqNodeDataPtr _nodeDataPtr,
                                  ...
                                  SeqNameValuesPtr set_args_ptr);

/* dependancy related */
static int writeNodeWaitedFile(const SeqNodeDataPtr _nodeDataPtr,
                               const char *_dep_exp_path, const char *_dep_node,
                               const char *_dep_status, const char *_dep_index,
                               const char *_dep_datestamp,
...
char *formatWaitingMsg(SeqDependsScope _dep_scope, const char *_dep_exp,
                       const char *_dep_node, const char *_dep_index,
                       const char *_dep_datestamp);
int prepFEFile(const SeqNodeDataPtr _nodeDataPtr, char *_target_state,
               char *_target_datestamp);
int prepInterUserFEFile(const SeqNodeDataPtr _nodeDataPtr, char *_target_state,
                        char *_target_datestamp);

int processDepStatus(const SeqNodeDataPtr _nodeDataPtr, SeqDepDataPtr dep,
                     const char *_flow);
...
static void (*_CreateLockFile)(int sock, char *filename, char *caller,
                               const char *_seq_exp_home);
static int (*_WriteNWFile)(const char *seq_xp_home, const char *nname,
                           const char *datestamp, const char *loopArgs,
                           const char *filename, const char *StatusFile);
static int (*_WriteInterUserDepFile)(const char *filename, const char *depBuf,
                                     const char *ppwdir,
                                     const char *maestro_version,
                                     const char *datestamp, const char *md5sum);
static int (*_WriteFEFile)(const char *_exp, const char *_node,
                           const char *_datestamp, const char *_target_index,
                           const char *_loopArgs, const char *_filename);

int writeInterUserNodeWaitedFile(const SeqNodeDataPtr _nodeDataPtr,
                                 const char *_dep_name, const char *_dep_index,
                                 char *depIndexPtr, const char *_dep_datestamp,
                                 const char *_dep_status, const char *_dep_exp,
                                 const char *_dep_prot, const char *statusFile,
                                 const char *_flow);
...
static void alarm_handler() {
  ...
  useNFSlocking();
  fprintf(stderr,
          "@@@@ TIMEOUT: connection to mserver killed by SIGALRM @@@@ \n");
}

static void pipe_handler() {
  fprintf(stderr,
          "@@@@ SIGPIPE received: the mserver has probably shutdown!  @@@@ \n");
}

/*
go_abort

This function is used to cause the node to enter an abort state, reacting to the
abort actions defined and handling the container behaviour.
...
static int go_abort(char *_signal, char *_flow,
  ...
  A typical _nodeDataPtr->abort will have two or more elements "rerun stop or
  cont". A lockfile is created for the current action..... Using the above
  example the first time the job aborts it'll create a lockfile with an
  extension "rerun". If it bombs again, the extension will be "stop".
  */
  if (strcmp(_signal, "abort") == 0 && strcmp(_flow, "continue") == 0) {
    ...
    while (tempPtr != NULL) {
      ...
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "maestro.go_abort() checking for action %s on node %s \n",
                    current_action, _nodeDataPtr->name);
      sprintf(filename, "%s/%s/%s.abort.%s", _nodeDataPtr->workdir,
              _nodeDataPtr->datestamp, extName, current_action);
      ...
      if (_access(filename, R_OK, _nodeDataPtr->expHome) == 0) {
        ...
        if (tempPtr != NULL) {
          ...
          SeqUtil_TRACE(TL_FULL_TRACE,
                        "maestro.go_abort() doing action %s on node %s \n",
                        current_action, _nodeDataPtr->name);
        }
        break;
      } else if (tempPtr->nextPtr == NULL) {
        /* no file was found, and we've reached the end of the list, so we must
         * do the first action */
        tempPtr = _nodeDataPtr->abort_actions;
        if ((current_action = (char *)malloc(strlen(tempPtr->data) + 1)) !=
            NULL) {
  ...
  */
  ...
  if (current_action == NULL) {
    if (_nodeDataPtr->abort_actions != NULL) {
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "nodelogger: %s i \"maestro info: abort action list "
                    "finished. Aborting with stop.\" \n",
                    _nodeDataPtr->name);
    }
    current_action = strdup("stop");
  }
  ...
  if (strcmp(current_action, "rerun") == 0) {
    ...
    if (strcmp(_signal, "abort") == 0 && strcmp(_flow, "continue") == 0) {
      ...
      SeqUtil_TRACE(TL_ERROR,
                    "nodelogger: %s X \"BOMBED: it has been resubmitted\"\n",
                    _nodeDataPtr->name);
      nodelogger(_nodeDataPtr->name, "info", _nodeDataPtr->extension, tmpString,
    ...
    if (strcmp(_signal, "abort") == 0 && strcmp(_flow, "continue") == 0) {
      ...
      /* submit the rest of the jobs it's supposed to submit (still missing
    ...
    */
    SeqUtil_TRACE(
        TL_ERROR,
        "invalid abort action flag %s... aborting...nodeabort: %s stop\n",
        current_action, _nodeDataPtr->name);
  ...
  if (strcmp(_nodeDataPtr->container, "") != 0 &&
      _nodeDataPtr->catchup != CatchupDiscretionary) {
...
processContainerAbort
 ...
 Treats the abort state of a node that has a container to see how the signal has
to be propagated up.
...
static void processContainerAbort(const SeqNodeDataPtr _nodeDataPtr) {
  ...
  /* deal with L(i) begin -> beginx of L if none are aborted, or Npass(i) ->
   * Npass */
  if ((char *)SeqLoops_getLoopAttribute(_nodeDataPtr->loop_args,
                                        _nodeDataPtr->nodeName) != NULL) {
    SeqNameValues_deleteItem(&newArgs, _nodeDataPtr->nodeName);
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "********** processContainerAbort() calling maestro -s "
                  "abortx -n %s with loop args=%s\n",
...
Deals with the status files associated with the abort state by clearing the
other states then putting the node in abort state.

Inputs:
  _nodeDataPtr - pointer to the node targetted by the execution
  current_action - pointer to the string declaring the abort action to do
(usually cont, stop, rerun).
...
static void setAbortState(const SeqNodeDataPtr _nodeDataPtr,
  ...
  clearAllOtherStates(_nodeDataPtr, extName, "maestro.setAbortState()",
                      current_action);
  ...
  _CreateLockFile(MLLServerConnectionFid, filename,
                  "maestro.setAbortState() created lockfile",
...
This function is used to cause the node to enter an initialized state, clearing
the node's state, and possibly any child nodes.
...
static int go_initialize(char *_signal, char *_flow,
  ...
  /* clears all the datestamped status files starting from the current node, if
  ...
  if ((strcmp(_signal, "initbranch") == 0) &&
      (_nodeDataPtr->type == Task || _nodeDataPtr->type == NpassTask)) {
    raiseError("maestro -s initbranch can only be used on container type nodes "
               "(not task type). Exiting. \n");
  }
  ...
  if (strcmp(_signal, "initbranch") == 0) {
    ...
    SeqUtil_TRACE(
        TL_FULL_TRACE,
        "maestro.go_initialize() deleting end lockfiles starting at node=%s\n",
        _nodeDataPtr->name);
    sprintf(
        cmd,
        "find %s/%s/%s/%s -name \"*%s.end\" -type f -print -delete -o -name "
        "\"*%s.begin\"  -type f -print -delete  -o -name \"*%s.abort.*\"  "
        "-type f -print -delete -o -name \"*%s.submit\" -type f -print -delete "
        "-o -name \"*%s.waiting*\" -type f -print -delete",
    ...
    sprintf(
        ...
        "find %s/%s/%s/%s -name \"*%s+*.end\"  -type f -print -delete -o -name "
        "\"*%s+*.begin\"  -type f -print -delete -o  -name \"*%s+*.abort.*\"  "
        "-type f -print -delete -o -name \"*%s+*.submit\"  -type f -print "
        "-delete -o -name \"*%s+*.waiting*\"  -type f -print -delete",
        _nodeDataPtr->workdir, _nodeDataPtr->datestamp, _nodeDataPtr->container,
        _nodeDataPtr->nodeName, extName, extName, extName, extName, extName);
    SeqUtil_TRACE(TL_FULL_TRACE, "cmd=%s\n", cmd);
    returnValue = system(cmd);
    ...
    memset(cmd, '\0', sizeof cmd);
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "maestro.go_initialize() deleting waiting.InterUser "
                  "lockfiles starting for node=%s\n",
                  _nodeDataPtr->name);
    sprintf(cmd, "find %s%s%s%s -name \"*.waiting*\" -type f -print -delete",
            _nodeDataPtr->expHome, INTER_DEPENDS_DIR, _nodeDataPtr->datestamp,
...
Deals with the status files associated with the initial state by clearing the
other states.
...
static void setInitState(const SeqNodeDataPtr _nodeDataPtr) {
  ...
  /* this line will take care of task, normal containers (including the loop
   * node) */
  ...
  if (_nodeDataPtr->type == Switch) {
    if (((char *)SeqLoops_getLoopAttribute(_nodeDataPtr->loop_args,
                                           _nodeDataPtr->nodeName)) == NULL) {
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "maestro.go_initialize() Adding to list switch argument "
                    "extension: %s\n",
                    SeqNameValues_getValue(_nodeDataPtr->switchAnswers,
  ...
  while (nodeList != NULL) {
    ...
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "maestro.go_initialize()() Looking for status files: %s\n",
                  extName);
...
This function is used to cause the node to enter the begin state. Containers
submit their implicit dependencies in this state. The function also deals with
the parent containers' begin state if conditions are met.
...
static int go_begin(char *_signal, char *_flow,
  ...
  SeqUtil_TRACE(TL_FULL_TRACE,
                "maestro.go_begin() node=%s signal=%s flow=%s loopargs=%s\n",
                _nodeDataPtr->name, _signal, _flow,
                SeqLoops_getLoopArgs(_nodeDataPtr->loop_args));
  ...
  if (strcmp(_signal, "begin") == 0 && strcmp(_flow, "continue") == 0) {
    ...
    if (_nodeDataPtr->type == Loop || _nodeDataPtr->type == ForEach) {
      ...
      if ((char *)SeqLoops_getLoopAttribute(_nodeDataPtr->loop_args,
        ...
        if (_nodeDataPtr->type == Loop) {
          loopSetArgs = (SeqNameValuesPtr)SeqLoops_getLoopSetArgs(
              _nodeDataPtr, _nodeDataPtr->loop_args, 0);
          SeqUtil_TRACE(TL_FULL_TRACE,
                        "maestro.go_begin() doing loop iteration submissions\n",
                        _nodeDataPtr->name, _signal);
        } else if (_nodeDataPtr->type == ForEach) {
          /* FE ->  check ForEach target for launched iterations, launch
           * corresponding  */
          loopSetArgs = (SeqNameValuesPtr)processForEachTarget(_nodeDataPtr);
          SeqUtil_TRACE(TL_FULL_TRACE,
                        "maestro.go_begin() doing for_each iteration "
                        "submissions or tagging \n",
                        _nodeDataPtr->name, _signal);
        }
        loopArgs = (SeqNameValuesPtr)SeqLoops_getContainerArgs(
            _nodeDataPtr, _nodeDataPtr->loop_args);
    ...
    if (_nodeDataPtr->type == Switch) {
      ...
      if ((char *)SeqLoops_getLoopAttribute(_nodeDataPtr->loop_args,
        ...
        SeqNameValues_insertItem(
            &loopSetArgs, _nodeDataPtr->nodeName,
            SeqNameValues_getValue(_nodeDataPtr->switchAnswers,
                                   _nodeDataPtr->name));
        /*now submit the child nodes */
        SeqUtil_TRACE(
            TL_FULL_TRACE,
            "go_begin calling maestro -n %s -s submit -l %s -f continue\n",
            _nodeDataPtr->name, SeqLoops_getLoopArgs(loopSetArgs));
        maestro(_nodeDataPtr->name, "submit", "continue", loopSetArgs, 0, NULL,
                _nodeDataPtr->datestamp, _nodeDataPtr->expHome);
        SeqNameValues_deleteWholeList(&loopArgs);
        SeqNameValues_deleteWholeList(&loopSetArgs);
      }
  ...
  if (strcmp(_nodeDataPtr->container, "") != 0) {
    ...
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "go_begin():Node %s does not have a container\n",
                  _nodeDataPtr->nodeName);
  }
  actionsEnd(_signal, _flow, _nodeDataPtr->name);
  return (0);
...
Deals with the status files associated with the begin state by clearing the
other states then putting the node in begin state.
...
static void setBeginState(char *_signal, const SeqNodeDataPtr _nodeDataPtr) {
  ...
  SeqUtil_TRACE(TL_FULL_TRACE,
                "maestro.setBeginState() on node:%s extension: %s\n",
                _nodeDataPtr->name, _nodeDataPtr->extension);

  extName = (char *)SeqNode_extension(_nodeDataPtr);

  clearAllOtherStates(_nodeDataPtr, extName, "maestro.setBeginState()",
                      "begin");

  /* begin lock file */
  memset(filename, '\0', sizeof filename);
  sprintf(filename, "%s/%s/%s.begin", _nodeDataPtr->workdir,
  ...
  if (_nodeDataPtr->type != Task && _nodeDataPtr->type != NpassTask) {
    if ((strcmp(_signal, "begin") == 0) ||
        (strcmp(_signal, "beginx") == 0 &&
         !_isFileExists(filename, "setBeginState()", _nodeDataPtr->expHome))) {
  ...
  SeqUtil_TRACE(TL_FULL_TRACE,
                "maestro.setBeginState() checking for lockfile %s\n", filename);
...
int hasArgs(const SeqNodeDataPtr _nodeDataPtr) {
  ...
  if (_nodeDataPtr->type == Loop || _nodeDataPtr->type == NpassTask ||
    ...
    tmpValue = SeqLoops_getLoopAttribute(_nodeDataPtr->loop_args,
                                         _nodeDataPtr->nodeName);
  if (tmpValue != NULL) {
    free(tmpValue);
...
processContainerBegin
 ...
 Treats the begin state of a container by checking the status of the siblings
nodes around the targetted node.
...
static void processContainerBegin(const SeqNodeDataPtr _nodeDataPtr,
  ...
  if (_nodeDataPtr->catchup == CatchupDiscretionary) {
    SeqUtil_TRACE(
        TL_FULL_TRACE,
        "maestro.processContainerBegin() bypassing discrete node:%s\n",
        _nodeDataPtr->name);
    return;
  }

  SeqUtil_TRACE(TL_FULL_TRACE,
                "processContainerBegin():_nodeDataPtr->loopArgs : \n");
  SeqNameValues_printList(_nodeDataPtr->loop_args);
  SeqUtil_TRACE(TL_FULL_TRACE,
                "processContainerBegin():_nodeDataPtr->nodeName : %s\n",
                _nodeDataPtr->nodeName);
  SeqUtil_TRACE(TL_FULL_TRACE,
                "processContainerBegin():_nodeDataPtr->name : %s\n",
                _nodeDataPtr->name);
  SeqUtil_TRACE(TL_FULL_TRACE,
                "processContainerBegin(): _nodeDataPtr->type : %s\n",
                SeqNode_getTypeString(_nodeDataPtr->type));

  /* deal with L(i) begin -> beginx of L if none are aborted, or Npass(i) ->
   * Npass, or Switch(i) -> Switch */
  if (hasArgs(_nodeDataPtr)) {
    SeqUtil_TRACE(TL_FULL_TRACE, "processContainerBegin(): Entered if "
                                 "(SeqLoops_getLoopAttribute() != NULL).\n");
    if ((_nodeDataPtr->type == Loop && !isLoopAborted(_nodeDataPtr)) ||
        (_nodeDataPtr->type == NpassTask && !isNpassAborted(_nodeDataPtr)) ||
        _nodeDataPtr->type == Switch) {
      SeqNameValues_deleteItem(&newArgs, _nodeDataPtr->nodeName);
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "********** processContainerBegin() calling maestro -s "
                    "beginx -n %s with loop args=%s\n",
                    _nodeDataPtr->name, SeqLoops_getLoopArgs(newArgs));
    ...
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "processContainerBegin() container=%s extension=%s\n",
    ...
    if (siblingIteratorPtr != NULL && abortedSibling == 0) {
      ...
      while (siblingIteratorPtr != NULL && abortedSibling == 0) {
        memset(filename, '\0', sizeof filename);
        sprintf(filename, "%s/%s/%s/%s%s.abort.stop", _nodeDataPtr->workdir,
                _nodeDataPtr->datestamp, _nodeDataPtr->container,
                siblingIteratorPtr->data, extWrite);
        abortedSibling = _isFileExists(filename, "processContainerBegin()",
                                       _nodeDataPtr->expHome);
        if (abortedSibling) {
          /* check if it's a discretionary or catchup higher than job's value,
           * bypass if yes */
          memset(tmp, '\0', sizeof tmp);
          sprintf(tmp, "%s/%s", _nodeDataPtr->container,
                  siblingIteratorPtr->data);
          SeqUtil_TRACE(
              TL_FULL_TRACE,
              "maestro.processContainerBegin() getting sibling info: %s\n",
              tmp);
          ...
          if (siblingDataPtr->catchup > catchup) {
            ...
            abortedSibling = 0;
            SeqUtil_TRACE(TL_FULL_TRACE,
                          "maestro.processContainerBegin() bypassing "
                          "discretionary or higher catchup node: %s\n",
                          siblingIteratorPtr->data);
          }
          SeqNode_freeNode(siblingDataPtr);
        }
    ...
    if (abortedSibling == 0) {
      SeqUtil_TRACE(
          TL_FULL_TRACE,
          "processContainerBegin(): Entered if( abortedSibling == 0 )\n");
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "********** processContainerBegin() calling maestro -s "
                    "beginx -n %s with loop args=%s\n",
                    _nodeDataPtr->container, SeqLoops_getLoopArgs(newArgs));
  ...
  SeqUtil_TRACE(TL_FULL_TRACE,
                "maestro.processContainerBegin() abortedSibling value=%d\n",
                abortedSibling);
}
...
This function is used to cause the node to enter the end state. Tasks submit
their implicit dependencies in this state. The function also deals with the
parent containers' end state if conditions are met. See header documentation of
SeqLoops_nextLoopArgs() for explanation of how loops are handled.
...
static int go_end(char *_signal, char *_flow,
  ...
  if ((_nodeDataPtr->type == Task || _nodeDataPtr->type == NpassTask) &&
     ...
     */
    if ((tmpValue = SeqLoops_getLoopAttribute(
             _nodeDataPtr->loop_args, _nodeDataPtr->nodeName)) != NULL &&
        !SeqLoops_isLastIteration(_nodeDataPtr, _nodeDataPtr->loop_args)) {
      newArgs = (SeqNameValuesPtr)SeqLoops_nextLoopArgs(
          _nodeDataPtr, _nodeDataPtr->loop_args, &newDefNumber);
      if ((strcmp(_flow, "continue") == 0)) {
        if (newDefNumber != 0) {
          /* Submit the initial set of a new definition */
          SeqUtil_TRACE(
              TL_MEDIUM,
              "go_end() submitting new definition. newDefNumber = %d\n",
              newDefNumber);
          loopSetArgs = (SeqNameValuesPtr)SeqLoops_getLoopSetArgs(
              _nodeDataPtr, NULL, newDefNumber);
          containerArgs = (SeqNameValuesPtr)SeqLoops_getContainerArgs(
              _nodeDataPtr, _nodeDataPtr->loop_args);
          ...
          if (isEndCnt != 0) {
            ...
            SeqUtil_TRACE(
                TL_FULL_TRACE,
                "go_end() submitting next iteration with maestro call:\n\t\
                        maestro( %s, \"submit\" , %s, %s, 0, NULL, %s, %s \n",
                _nodeDataPtr->name, _flow, newArgs, _nodeDataPtr->datestamp,
                _nodeDataPtr->expHome);
            maestro(_nodeDataPtr->name, "submit", _flow, newArgs, 0, NULL,
                    _nodeDataPtr->datestamp, _nodeDataPtr->expHome);
          } else {
            SeqUtil_TRACE(TL_FULL_TRACE,
                          "maestro.go_end() Skipping submission of next "
                          "iteration -- already @ end state.\n");
          }
        }
      }
    }
  ...
  if (strcmp(_nodeDataPtr->container, "") != 0) {
    if (isEndCnt != 0) {
      ...
      fprintf(
          stderr,
          "maestro.go_end() Skipping end execution, already @ end state. No "
          "dependencies will be submitted, nor containers will be processed. "
          "Clear out end lockfile to be able to run end command.\n");
      return (0);
    }
  }
...
Deals with the status files associated with the end state by clearing the other
states then putting the node in end state.
...
static int setEndState(const char *_signal, const SeqNodeDataPtr _nodeDataPtr) {
  ...
  if (_nodeDataPtr->type != Task && _nodeDataPtr->type != NpassTask) {
    if ((strcmp(_signal, "end") == 0) ||
        ((strcmp(_signal, "endx") == 0) &&
         !_isFileExists(filename, "setEndState()", _nodeDataPtr->expHome))) {
  ...
  */
  ...
  if (_nodeDataPtr->type == NpassTask && _nodeDataPtr->isLastArg) {
    ...
    if ((char *)SeqLoops_getLoopAttribute(_nodeDataPtr->loop_args,
      ...
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "maestro.setEndState() containerLoopExt %s\n",
                    containerLoopExt);
      ...
      sprintf(filename1, "%s/%s/%s.%s.end", _nodeDataPtr->workdir,
              _nodeDataPtr->datestamp, _nodeDataPtr->name, nptExt);
      free(nptExt);
    }
...
Inputs:
  fullNodeName - pointer to the node's full name (i.e.
/testsuite/assimilation/00/randomjob ) originator - pointer to the name of the
originating function that called this
...
static void clearAllOtherStates(const SeqNodeDataPtr _nodeDataPtr,
  ...
  memset(filename, '\0', sizeof filename);
  SeqUtil_TRACE(TL_FULL_TRACE,
                "maestro.clearAllOtherStates() originator=%s node=%s\n",
                originator, fullNodeName);

  /* remove the node begin lock file */
  sprintf(filename, "%s/%s/%s.begin", _nodeDataPtr->workdir,
          _nodeDataPtr->datestamp, fullNodeName);
  if (_access(filename, R_OK, _nodeDataPtr->expHome) == 0 &&
      strcmp(current_state, "begin") != 0) {
    SeqUtil_TRACE(
        TL_FULL_TRACE,
  ...
  memset(filename, '\0', sizeof filename);
  sprintf(filename, "%s/%s/%s.abort.stop", _nodeDataPtr->workdir,
          _nodeDataPtr->datestamp, fullNodeName);
  if (_access(filename, R_OK, _nodeDataPtr->expHome) == 0 &&
  ...
  memset(filename, '\0', sizeof filename);
  /* Notice that clearing submit will cause a concurrency vs NFS problem when we
   * add dependency */
  sprintf(filename, "%s/%s/%s.submit", _nodeDataPtr->workdir,
          _nodeDataPtr->datestamp, fullNodeName);
  if (_access(filename, R_OK, _nodeDataPtr->expHome) == 0 &&
      strcmp(current_state, "submit") != 0) {
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "maestro.clearAllOtherStates() %s removed lockfile %s\n",
                  originator, filename);
    ret = _removeFile(filename, _nodeDataPtr->expHome);
  ...
  sprintf(filename, "%s/%s/%s.waiting", _nodeDataPtr->workdir,
          _nodeDataPtr->datestamp, fullNodeName);
  if (_access(filename, R_OK, _nodeDataPtr->expHome) == 0 &&
      strcmp(current_state, "waiting") != 0) {
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "maestro.clearAllOtherStates() %s removed lockfile %s\n",
                  originator, filename);
    ret = _removeFile(filename, _nodeDataPtr->expHome);
  ...
  if (_nodeDataPtr->type == NpassTask) {
    if ((char *)SeqLoops_getLoopAttribute(_nodeDataPtr->loop_args,
      ...
      SeqUtil_TRACE(
          TL_FULL_TRACE,
          "maestro.clearAllOtherStates() NPASS(i) deleting NPASS.end \n",
          originator, filename);
      newArgs = SeqNameValues_clone(_nodeDataPtr->loop_args);
      SeqNameValues_deleteItem(&newArgs, _nodeDataPtr->nodeName);
      ...
      memset(filename, '\0', sizeof filename);
      sprintf(filename, "%s/%s/%s%s.end", _nodeDataPtr->workdir,
              _nodeDataPtr->datestamp, _nodeDataPtr->name, extension);
      if (_access(filename, R_OK, _nodeDataPtr->expHome) == 0 &&
          strcmp(current_state, "end") != 0) {
        SeqUtil_TRACE(TL_FULL_TRACE,
                      "maestro.clearAllOtherStates() %s removed lockfile %s\n",
  ...
  if (strcmp(current_state, "init") == 0 || strcmp(current_state, "end") == 0 ||
      strcmp(current_state, "stop") == 0) {
    memset(filename, '\0', sizeof filename);
    sprintf(filename, "%s/%s/%s.abort.rerun", _nodeDataPtr->workdir,
            _nodeDataPtr->datestamp, fullNodeName);
    if (_access(filename, R_OK, _nodeDataPtr->expHome) == 0 &&
        strcmp(current_state, "rerun") != 0) {
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "maestro.clearAllOtherStates() %s removed lockfile %s\n",
                    originator, filename);
      ret = _removeFile(filename, _nodeDataPtr->expHome);
    ...
    memset(filename, '\0', sizeof filename);
    sprintf(filename, "%s/%s/%s.abort.cont", _nodeDataPtr->workdir,
            _nodeDataPtr->datestamp, fullNodeName);
    if (_access(filename, R_OK, _nodeDataPtr->expHome) == 0 &&
        strcmp(current_state, "cont") != 0) {
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "maestro.clearAllOtherStates() %s removed lockfile %s\n",
                    originator, filename);
      ret = _removeFile(filename, _nodeDataPtr->expHome);
...
isLoopComplete
 ...
 returns 1 if all iteration of the loop is complete in terms of iteration status
file returns 0 otherwise
...
static int isLoopComplete(const SeqNodeDataPtr _nodeDataPtr) {
  ...
  if (extensions != NULL) {
    while (extensions != NULL && undoneIteration == 0) {
      sprintf(endfile, "%s/%s/%s.%s.end", _nodeDataPtr->workdir,
              _nodeDataPtr->datestamp, _nodeDataPtr->name, extensions->data);
      sprintf(continuefile, "%s/%s/%s.%s.abort.cont", _nodeDataPtr->workdir,
              _nodeDataPtr->datestamp, _nodeDataPtr->name, extensions->data);
      SeqUtil_TRACE(
          TL_FULL_TRACE,
          "maestro.isLoopComplete() loop done? checking for:%s or %s\n",
          endfile, continuefile);
      undoneIteration =
          !(_isFileExists(endfile, "isLoopComplete()", _nodeDataPtr->expHome) ||
            _isFileExists(continuefile, "isLoopComplete()",
                          _nodeDataPtr->expHome));
...
isLoopAborted
 ...
 returns 1 if one loop iteration is in abort state in terms of iteration status
file returns 0 otherwise
...
static int isLoopAborted(const SeqNodeDataPtr _nodeDataPtr) {
  ...
  if (extensions != NULL) {
    while (extensions != NULL && abortedIteration == 0) {
      ...
      sprintf(abortedfile, "%s/%s/%s.%s.abort.stop", _nodeDataPtr->workdir,
              _nodeDataPtr->datestamp, _nodeDataPtr->name, extensions->data);
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "maestro.isLoopAborted() loop has aborted iteration? "
                    "checking for:%s\n",
                    abortedfile);
      abortedIteration =
          _isFileExists(abortedfile, "isLoopAborted()", _nodeDataPtr->expHome);
      extensions = extensions->nextPtr;
    }
  }
  SeqListNode_deleteWholeList(&extensions);
...
isNpassComplete
 ...
 returns 1 if all iteration of the npass task is complete in terms of iteration
status file returns 0 otherwise
...
static int isNpassComplete(const SeqNodeDataPtr _nodeDataPtr) {
  ...
  containerLoopArgsList = (SeqNameValuesPtr)SeqLoops_getContainerArgs(
      _nodeDataPtr, _nodeDataPtr->loop_args);
  if (containerLoopArgsList != NULL) {
    SeqUtil_stringAppend(
        &extension, (char *)SeqLoops_getExtFromLoopArgs(containerLoopArgsList));
  }
  SeqUtil_stringAppend(&extension, "+");

  memset(statePattern, '\0', sizeof statePattern);
  sprintf(statePattern, "%s/%s/%s.%slast.end", _nodeDataPtr->workdir,
          _nodeDataPtr->datestamp, _nodeDataPtr->name, extension);
  undoneIteration =
      !(_globPath(statePattern, GLOB_NOSORT, 0, _nodeDataPtr->expHome));
  if (undoneIteration)
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "maestro.isNpassComplete - last iteration not found\n");

  if (!undoneIteration) {
    /* search for submit states. */
    memset(statePattern, '\0', sizeof statePattern);
    sprintf(statePattern, "%s/%s/%s.%s*.submit", _nodeDataPtr->workdir,
            _nodeDataPtr->datestamp, _nodeDataPtr->name, extension);
    undoneIteration =
        _globPath(statePattern, GLOB_NOSORT, 0, _nodeDataPtr->expHome);
    if (undoneIteration)
      SeqUtil_TRACE(TL_FULL_TRACE, "maestro.isNpassComplete - found submit\n");
  }
  if (!undoneIteration) {
    /* search for begin states. */
    memset(statePattern, '\0', sizeof statePattern);
    sprintf(statePattern, "%s/%s/%s.%s*.begin", _nodeDataPtr->workdir,
            _nodeDataPtr->datestamp, _nodeDataPtr->name, extension);
    undoneIteration =
        _globPath(statePattern, GLOB_NOSORT, 0, _nodeDataPtr->expHome);
    if (undoneIteration)
      SeqUtil_TRACE(TL_FULL_TRACE, "maestro.isNpassComplete - found begin\n");
  }
  if (!undoneIteration) {
    /* search for abort.stop states. */
    memset(statePattern, '\0', sizeof statePattern);
    sprintf(statePattern, "%s/%s/%s.%s*.abort.stop", _nodeDataPtr->workdir,
            _nodeDataPtr->datestamp, _nodeDataPtr->name, extension);
    undoneIteration =
        _globPath(statePattern, GLOB_NOSORT, 0, _nodeDataPtr->expHome);
    if (undoneIteration)
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "maestro.isNpassComplete - found abort.stop\n");
  }
...
isNpassAborted
 ...
 returns 1 if an iteration of the npass task is aborted in terms of iteration
status file returns 0 otherwise

Inputs
  _nodeDataPtr - pointer to the npass node targetted by the execution (it should
have its own iteration defined)
*/
...
static int isNpassAborted(const SeqNodeDataPtr _nodeDataPtr) {
  ...
  abortedIteration =
      _globPath(statePattern, GLOB_NOSORT, 0, _nodeDataPtr->expHome);
  if (abortedIteration)
    SeqUtil_TRACE(TL_FULL_TRACE, "maestro.isNpassAborted - found abort.stop\n");
...
processForEachTarget
 ...
 returns a name-values list of iterations that can be launched by the foreach
container. returns NULL if there are no iteration that can be launched yet. It
will also tag the target(s) to launch iterations.
...
static SeqNameValuesPtr
processForEachTarget(const SeqNodeDataPtr _nodeDataPtr) {
  ...
  target_datestamp = SeqDatesUtil_getPrintableDate(
      _nodeDataPtr->datestamp, 0, atoi(_nodeDataPtr->forEachTarget->hour), 0,
      0);
  targetNodeDataPtr =
      nodeinfo(_nodeDataPtr->forEachTarget->node, NI_SHOW_ALL, NULL,
               _nodeDataPtr->forEachTarget->exp, NULL, target_datestamp, NULL);
  SeqUtil_TRACE(TL_FULL_TRACE,
                "maestro.processForEachTarget() called, targetting exp=%s, "
                "name=%s, index=%s, date=%s\n",
                _nodeDataPtr->forEachTarget->exp,
                _nodeDataPtr->forEachTarget->node,
                _nodeDataPtr->forEachTarget->index, target_datestamp);

  /* Check status of target node's iterations */
  /* search for end states. */
  memset(statePattern, '\0', sizeof statePattern);
  sprintf(statePattern, "%s/%s/%s.%s+*.end", targetNodeDataPtr->workdir,
          targetNodeDataPtr->datestamp, targetNodeDataPtr->name,
  ...
  while (tempIterator != NULL) {
    SeqUtil_TRACE(
        TL_FULL_TRACE,
        "maestro.processForEachTarget() inserting loop name=%s value=%s\n",
        _nodeDataPtr->nodeName, tempIterator->data);
    SeqNameValues_insertItem(&iterationListToReturn, _nodeDataPtr->nodeName,
                             tempIterator->data);
...
int prepFEFile(const SeqNodeDataPtr _nodeDataPtr, char *_target_state,
  ...
  SeqUtil_TRACE(
      TL_FULL_TRACE,
      "maestro.prepFEFile() target exp=%s, target node=%s, target index=%s "
      "target datestamp=%s self.index=%s\n",
      _nodeDataPtr->forEachTarget->exp, _nodeDataPtr->forEachTarget->node,
      _nodeDataPtr->forEachTarget->index, _target_datestamp, loopArgs);

  depBase = (char *)SeqUtil_getPathBase(
      (const char *)_nodeDataPtr->forEachTarget->node);
  /* write in sequencing/status/depends/$datestamp */
  sprintf(filename, "%s/%s/%s/%s", _nodeDataPtr->forEachTarget->exp,
          LOCAL_DEPENDS_DIR, _target_datestamp, depBase);
  ...
  if (_nodeDataPtr->extension != NULL && strlen(_nodeDataPtr->extension) > 0) {
    ...
    sprintf(filename, "%s%s%s/%s.%s.FE_%s", _nodeDataPtr->forEachTarget->exp,
            LOCAL_DEPENDS_DIR, _target_datestamp,
            _nodeDataPtr->forEachTarget->node, _nodeDataPtr->extension,
            _target_state);
  } else {
    sprintf(filename, "%s%s%s/%s.FE_%s", _nodeDataPtr->forEachTarget->exp,
            LOCAL_DEPENDS_DIR, _target_datestamp,
            _nodeDataPtr->forEachTarget->node, _target_state);
  }

  status = _WriteFEFile(_nodeDataPtr->expHome, _nodeDataPtr->name,
                        _nodeDataPtr->datestamp,
                        _nodeDataPtr->forEachTarget->index, loopArgs, filename);
  ...
  case 1:
    raiseError(
        "maestro had problems writing to the following file: %s (prepFEFile)\n",
        filename);
    break;
  case 9:
    status = 0;
...
int prepInterUserFEFile(const SeqNodeDataPtr _nodeDataPtr, char *_target_state,
  ...
  if (loopArgs != NULL && strlen(loopArgs) != 0) {
    snprintf(depFile, sizeof(depFile), "%s_%s%s_%s.%s", _nodeDataPtr->datestamp,
             _nodeDataPtr->expHome, _nodeDataPtr->name, loopArgs,
             _target_state);
  } else {
    snprintf(depFile, sizeof(depFile), "%s_%s%s.%s", _nodeDataPtr->datestamp,
             _nodeDataPtr->expHome, _nodeDataPtr->name, _target_state);
  }
  ...
  strftime(registration_time, sizeof(registration_time), "%Y%m%d-%H:%M:%S",
           ptm);
  ...
  if ((maestro_shortcut = getenv("SEQ_MAESTRO_SHORTCUT")) == NULL) {
    raiseError("Could not get maestro shortcut from SEQ_MAESTRO_SHORTCUT env "
               "variable.\n");
  }
  ...
  sprintf(filename, "%s/%s/%s/%s", _nodeDataPtr->expHome, INTER_DEPENDS_DIR,
          _nodeDataPtr->datestamp, depBase);
  ...
  if (loopArgs != NULL && strlen(loopArgs) != 0) {
    sprintf(filename, "%s%s%s%s_%s.waiting.interUser.%s", _nodeDataPtr->expHome,
            INTER_DEPENDS_DIR, _nodeDataPtr->datestamp, _nodeDataPtr->name,
            loopArgs, _target_state);
  } else {
    sprintf(filename, "%s%s%s%s.waiting.interUser.%s", _nodeDataPtr->expHome,
            INTER_DEPENDS_DIR, _nodeDataPtr->datestamp, _nodeDataPtr->name,
            _target_state);
  }
  ...
  ret =
      !_WriteInterUserDepFile(filename, depBuf, current_passwd->pw_dir,
                              maestro_version, _nodeDataPtr->datestamp, md5sum);

  /* check if server is active, if not, send warning that dep will not launch
   * until server is active */
  if (MLLServerConnectionFid < 0) {
    nodelogger(_nodeDataPtr->name, "info", _nodeDataPtr->extension,
               "ForEach node registered to mserver but mserver inactive. Run "
               "mserver to be able to launch future iterations.",
               _nodeDataPtr->datestamp, _nodeDataPtr->expHome);
  }
...
processContainerEnd
 ...
 Treats the end of a container by checking the status of the siblings around the
targetted node.
...
static void processContainerEnd(const SeqNodeDataPtr _nodeDataPtr,
                                char *_flow) {
  ...
  SeqNameValuesPtr newArgs = SeqNameValues_clone(_nodeDataPtr->loop_args);
  SeqUtil_TRACE(TL_FULL_TRACE,
                "********** processContainerEnd() calling maestro -s endx -n "
                "%s with loop args=%s\n",
                _nodeDataPtr->name, SeqLoops_getLoopArgs(newArgs));

  SeqUtil_TRACE(TL_FULL_TRACE,
                "processContainerEnd() :_nodeDataPtr->loopArgs : \n");
  SeqNameValues_printList(_nodeDataPtr->loop_args);
  SeqUtil_TRACE(TL_FULL_TRACE,
                "processContainerEnd() :_nodeDataPtr->nodeName : %s\n",
                _nodeDataPtr->nodeName);
  SeqUtil_TRACE(TL_FULL_TRACE,
                "processContainerEnd() :_nodeDataPtr->name : %s\n",
                _nodeDataPtr->name);
  SeqUtil_TRACE(TL_FULL_TRACE,
                "processContainerEnd(): _nodeDataPtr->type : %s\n",
                SeqNode_getTypeString(_nodeDataPtr->type));
  /* deal with L(i) ending -> end of L if all iterations are done, or Npass(i)
   * -> Npass */
  if (hasArgs(_nodeDataPtr)) {
    SeqUtil_TRACE(TL_FULL_TRACE, "processContainerEnd(): Entered if "
                                 "(SeqLoops_getLoopAttribute() != NULL).\n");
    if ((_nodeDataPtr->type == Loop && isLoopComplete(_nodeDataPtr)) ||
        (_nodeDataPtr->type == NpassTask && isNpassComplete(_nodeDataPtr)) ||
        _nodeDataPtr->type == Switch) {
      SeqNameValues_deleteItem(&newArgs, _nodeDataPtr->nodeName);
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "********** processContainerEnd() calling maestro -s endx "
                    "-n %s with loop args=%s\n",
                    _nodeDataPtr->name, SeqLoops_getLoopArgs(newArgs));
    ...
    siblingIteratorPtr = _nodeDataPtr->siblings;
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "processContainerEnd() container=%s extension=%s\n",
                  _nodeDataPtr->container, _nodeDataPtr->extension);
    if (strlen(_nodeDataPtr->extension) > 0) {
      SeqUtil_stringAppend(&extWrite, ".");
      SeqUtil_stringAppend(&extWrite, _nodeDataPtr->extension);
    ...
    if (siblingIteratorPtr != NULL) {
      ...
      while (siblingIteratorPtr != NULL && undoneChild == 0) {
        memset(endfile, '\0', sizeof endfile);
        memset(continuefile, '\0', sizeof continuefile);
        sprintf(endfile, "%s/%s/%s/%s%s.end", _nodeDataPtr->workdir,
                _nodeDataPtr->datestamp, _nodeDataPtr->container,
                siblingIteratorPtr->data, extWrite);
        sprintf(continuefile, "%s/%s/%s/%s%s.abort.cont", _nodeDataPtr->workdir,
                _nodeDataPtr->datestamp, _nodeDataPtr->container,
                siblingIteratorPtr->data, extWrite);
        undoneChild = !(_isFileExists(endfile, "processContainerEnd()",
                                      _nodeDataPtr->expHome) ||
                        _isFileExists(continuefile, "processContainerEnd()",
                                      _nodeDataPtr->expHome));
        if (undoneChild) {
          /* check if it's a discretionary or catchup higher than job's value,
           * bypass if yes */
          ...
          SeqUtil_TRACE(
              TL_FULL_TRACE,
              "maestro.processContainerEnd() getting sibling info: %s\n", tmp);
          siblingDataPtr = nodeinfo(tmp, NI_SHOW_TYPE | NI_SHOW_RESOURCE, NULL,
                                    _nodeDataPtr->expHome, NULL,
                                    _nodeDataPtr->datestamp, NULL);
          if (catchup != CatchupStop) {
            if (siblingDataPtr->catchup > catchup) {
              ...
              undoneChild = 0;
              SeqUtil_TRACE(TL_FULL_TRACE,
                            "maestro.processContainerEnd() bypassing "
                            "discretionary or higher catchup node: %s\n",
                            siblingIteratorPtr->data);
            }
          } else {
...
This function is used to cause the node to enter the submit state. Checks
dependencies of the node then submits to batch system.

Inputs:
  signal - pointer to the value of the signal given to the binary (-s option)
  flow - pointer to the value of the flow given to the binary ( -f option)
  _nodeDataPtr - pointer to the node targetted by the execution
  ignoreAllDeps - integer representing True or False whether dependencies are
ignored
...
static int go_submit(const char *_signal, char *_flow,
  ...
  SeqUtil_TRACE(
      TL_FULL_TRACE,
      "maestro.go_submit() node=%s signal=%s flow=%s ignoreAllDeps=%d\n ",
  ...
  sprintf(nodeFullPath, "%s/modules/%s.tsk", _nodeDataPtr->expHome,
          _nodeDataPtr->taskPath);
  sprintf(listingDir, "%s/sequencing/output/%s", _nodeDataPtr->expHome,
          _nodeDataPtr->container);
  sprintf(defFile, "%s/resources/resources.def", _nodeDataPtr->expHome);
  _SeqUtil_mkdir(listingDir, 1, _nodeDataPtr->expHome);

  if (!(strlen(_nodeDataPtr->extension) > 0)) {
    sprintf(submissionDir, "%s/sequencing/output%s.submission.%s.pgmout%d",
            _nodeDataPtr->expHome, _nodeDataPtr->name, _nodeDataPtr->datestamp,
            getpid());
  } else {
    sprintf(submissionDir, "%s/sequencing/output%s.%s.submission.%s.pgmout%d",
            _nodeDataPtr->expHome, _nodeDataPtr->name, _nodeDataPtr->extension,
            _nodeDataPtr->datestamp, getpid());
  }
  ...
  SeqUtil_TRACE(
      TL_FULL_TRACE,
      "node catchup= %d , exp catchup = %d , discretionary catchup = %d  \n",
  ...
  if (_nodeDataPtr->catchup > catchup && !ignoreAllDeps) {
    if (_nodeDataPtr->catchup == CatchupDiscretionary) {
      SeqUtil_TRACE(TL_FULL_TRACE, "nodelogger: -n %s -l %s \"%s\"\n",
                    _nodeDataPtr->name, _nodeDataPtr->extension,
                    CATCHUP_DISCR_MSG);
      nodelogger(_nodeDataPtr->name, "discret", _nodeDataPtr->extension,
                 CATCHUP_DISCR_MSG, _nodeDataPtr->datestamp,
                 _nodeDataPtr->expHome);
    } else {
      SeqUtil_TRACE(TL_FULL_TRACE, "nodelogger: -n %s -l %s \"%s\"\n",
                    _nodeDataPtr->name, _nodeDataPtr->extension,
                    CATCHUP_UNSUBMIT_MSG);
      nodelogger(_nodeDataPtr->name, "catchup", _nodeDataPtr->extension,
                 CATCHUP_UNSUBMIT_MSG, _nodeDataPtr->datestamp,
                 _nodeDataPtr->expHome);
    }
    return (0);
  }
  ...
  if (ignoreAllDeps == 1 || validateDependencies(_nodeDataPtr, _flow) == 0) {
    ...
    SeqUtil_stringAppend(&tmpCfgFile, _nodeDataPtr->datestamp);
    SeqUtil_stringAppend(&tmpCfgFile, ".");
    ...
    if ((prefix = SeqUtil_getdef(defFile, "SEQ_JOBNAME_PREFIX",
                                 _nodeDataPtr->expHome)) != NULL) {
    ...
    if (_nodeDataPtr->type == Task || _nodeDataPtr->type == NpassTask) {
      ...
      /* check if it's work_unit SEQ_WORKER_PATH is not 0 -> immediate mode in
       * ord_soumet (with jobtar option) else it's normal submission  */
      if (strcmp(_nodeDataPtr->workerPath, "") != 0) {
        /* create tar of the job in the worker's path */
        if ((tmpTarPath = malloc(
                 strlen(_nodeDataPtr->expHome) +
                 strlen("/sequencing/tmpfile/") +
                 strlen(_nodeDataPtr->datestamp) + strlen("/work_unit_depot/") +
                 strlen(_nodeDataPtr->workerPath) + strlen("/") +
                 strlen(_nodeDataPtr->container) + 1)) != NULL) {
          sprintf(tmpTarPath, "%s/sequencing/tmpfile/%s/work_unit_depot/%s/%s",
                  _nodeDataPtr->expHome, _nodeDataPtr->datestamp,
                  _nodeDataPtr->workerPath, _nodeDataPtr->container);
        } else {
        ...
        if ((tarFile = malloc(strlen(tmpTarPath) + strlen("/") +
                              strlen(extName) + strlen(".tar") + 1)) != NULL) {
          sprintf(tarFile, "%s/%s.tar", tmpTarPath, extName);
        } else {
          raiseError("OutOfMemory exception in maestro.go_submit()\n");
        }
        if ((movedTarFile =
                 malloc(strlen(tmpTarPath) + strlen("/") + strlen(extName) +
                        strlen(".tmp") + strlen(".tar") + 1)) != NULL) {
          sprintf(movedTarFile, "%s/%s.tmp.tar", tmpTarPath, extName);
        } else {
          raiseError("OutOfMemory exception in maestro.go_submit()\n");
        }
        if ((movedTmpName = malloc(strlen(tmpTarPath) + strlen("/") +
                                   strlen(extName) + strlen(".tmp") + 1)) !=
            NULL) {
          sprintf(movedTmpName, "%s/%s.tmp", tmpTarPath, extName);
        } else {
          raiseError("OutOfMemory exception in maestro.go_submit()\n");
        }
        if ((readyFile = malloc(strlen(tmpTarPath) + strlen("/") +
                                strlen(extName) + strlen(".tar.ready") + 1)) !=
            NULL) {
        ...
        SeqUtil_TRACE(
            TL_FULL_TRACE,
            "maestro.go_submit() checking for worker's extension: %s\n",
            workerDataPtr->extension);

        if (strlen(workerDataPtr->extension) > 0) {
          sprintf(workerEndFile, "%s/%s/%s.%s.end", _nodeDataPtr->workdir,
                  _nodeDataPtr->datestamp, _nodeDataPtr->workerPath,
                  workerDataPtr->extension);
        } else {
          sprintf(workerEndFile, "%s/%s/%s.end", _nodeDataPtr->workdir,
                  _nodeDataPtr->datestamp, _nodeDataPtr->workerPath);
        }

        if (strlen(workerDataPtr->extension) > 0) {
          sprintf(workerAbortFile, "%s/%s/%s.%s.abort.stop",
                  _nodeDataPtr->workdir, _nodeDataPtr->datestamp,
                  _nodeDataPtr->workerPath, workerDataPtr->extension);
        } else {
          sprintf(workerAbortFile, "%s/%s/%s.abort.stop", _nodeDataPtr->workdir,
                  _nodeDataPtr->datestamp, _nodeDataPtr->workerPath);
        }

        SeqUtil_TRACE(TL_FULL_TRACE,
                      "maestro.go_submit() checking for worker status %s or %s",
                      workerEndFile, workerAbortFile);
        ...
        if ((_access(workerEndFile, R_OK, _nodeDataPtr->expHome) == 0) ||
          ...
          maestro(_nodeDataPtr->workerPath, "submit", "continue",
                  workerDataPtr->loop_args, 0, NULL, _nodeDataPtr->datestamp,
                  _nodeDataPtr->expHome);
        }
      ...
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "Temporarily sending submission output to %s\n",
                    submissionDir);
      strcat(cmd, " > \"");
      strcat(cmd, submissionDir);
      strcat(cmd, "\" 2>&1");
      ...
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "maestro.go_submit() ord return status: %d \n",
                    error_status);
      if (strcmp(_nodeDataPtr->workerPath, "") != 0) {
        rename(movedTarFile, tarFile);
        SeqUtil_TRACE(
            TL_FULL_TRACE,
            "maestro.go_submit() moving temporary tar file %s to %s \n",
      ...
      /*
         ...
         AT any time, we will only have 1 dependency file (for any task) in
         inter_depend_dir, we should remove the dep. file and mserver will
         remove link ($datestamp_$key) in
         $HOME/.suites/maestrod/dependencies/polling/$version/ when the link
         became a dangling link. I have hardcoded depstatus to "end", to
         minimize code and cuse this is the status that we are depending on Now.
      */
      if (ignoreAllDeps == 1) {
        memset(tmpfile, '\0', sizeof tmpfile);
        if (loopArgs != NULL && strlen(loopArgs) != 0) {
          snprintf(tmpfile, sizeof(tmpfile),
                   "%s%s%s%s_%s.waiting.interUser.end", _nodeDataPtr->expHome,
                   INTER_DEPENDS_DIR, _nodeDataPtr->datestamp,
                   _nodeDataPtr->name, loopArgs);
        } else {
          snprintf(tmpfile, sizeof(tmpfile), "%s%s%s%s.waiting.interUser.end",
                   _nodeDataPtr->expHome, INTER_DEPENDS_DIR,
                   _nodeDataPtr->datestamp, _nodeDataPtr->name);
        }
      ...
      memset(tmpfile, '\0', sizeof tmpfile);
      sprintf(tmpfile, "%s/sequencing/tmpfile/container.tsk",
              _nodeDataPtr->expHome);
      if (_touch(tmpfile, _nodeDataPtr->expHome) != 0)
        raiseError("Cannot create lockfile: %s\n", tmpfile);

      if (((_nodeDataPtr->type == Switch) || _nodeDataPtr->type == Loop) &&
          ((char *)SeqLoops_getLoopAttribute(_nodeDataPtr->loop_args,
                                             _nodeDataPtr->nodeName) == NULL)) {
        strcpy(noendwrap, "-noendwrap");
      } else { /* All other containers need to check if it submits nothing*/
        _nodeDataPtr->submits == NULL ? strcpy(noendwrap, "")
                                      : strcpy(noendwrap, "-noendwrap");
      }

      containerMethod = SeqUtil_getdef(defFile, "SEQ_CONTAINER_METHOD",
                                       _nodeDataPtr->expHome);
      if ((containerMethod == NULL) ||
          (strcmp(containerMethod, "immediate") == 0)) {
    ...
    if (nodetracer_status) {
      SeqUtil_TRACE(
          TL_CRITICAL,
          "Problem with nodetracer call, listing may not be available in the "
          "listings directory, possibly in $SEQ_EXP_HOME/sequencing/output.\n");
    }
  }
...
Deals with the status files associated with the submit state by clearing the
other states then putting the node in submit state.
...
static void setSubmitState(const SeqNodeDataPtr _nodeDataPtr) {
  ...
  clearAllOtherStates(_nodeDataPtr, extName, "maestro.setSubmitState()",
                      "submit");
...
static void submitNodeList(const SeqNodeDataPtr _nodeDataPtr) {
  ...
  while (listIteratorPtr != NULL) {
    ...
    maestro(listIteratorPtr->data, "submit", "continue",
            _nodeDataPtr->loop_args, 0, NULL, _nodeDataPtr->datestamp,
            _nodeDataPtr->expHome);
    /*flood control, wait CONTAINER_FLOOD_TIMER seconds between sets of
     * submits*/
...
static void submitLoopSetNodeList(const SeqNodeDataPtr _nodeDataPtr,
                                  SeqNameValuesPtr container_args_ptr,
                                  SeqNameValuesPtr loopset_index) {
  SeqNameValuesPtr myLoopArgsPtr = loopset_index;
  SeqNameValuesPtr cmdLoopArg = NULL;
  int counter = 0;
  SeqUtil_TRACE(TL_FULL_TRACE,
                "maestro.submitLoopSetNodeList() container_args_ptr=%s "
                "loopset_index=%s\n",
                SeqLoops_getLoopArgs(container_args_ptr),
                SeqLoops_getLoopArgs(loopset_index));
  while (myLoopArgsPtr != NULL) {
    ...
    /* then add the current loop argument */
    SeqNameValues_insertItem(&cmdLoopArg, myLoopArgsPtr->name,
                             myLoopArgsPtr->value);
    /*now submit the child nodes */
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "submitLoopSetNodeList calling maestro -n %s -s submit -l %s "
                  "-f continue\n",
...
Deals with the status files associated with the waiting state by clearing the
other states then putting the node in waiting state.
...
static void submitDependencies(const SeqNodeDataPtr _nodeDataPtr,
                               const char *_signal, const char *_flow) {
  ...
  char depExp[256] = {'\0'}, depNode[256] = {'\0'}, depArgs[256] = {'\0'},
       depDatestamp[20] = {'\0'};
  char waited_filename[SEQ_MAXFIELD] = {'\0'}, submitCmd[SEQ_MAXFIELD] = {'\0'},
       statusFile[SEQ_MAXFIELD] = {'\0'};
  ...
  SeqUtil_TRACE(TL_FULL_TRACE,
                "maestro.submitDependencies() executing for %s\n",
                _nodeDataPtr->nodeName);

  loopArgsPtr = _nodeDataPtr->loop_args;
  if (_nodeDataPtr->isLastArg) {
    tmpValue = SeqUtil_striplast(
        SeqNameValues_getValue(loopArgsPtr, _nodeDataPtr->nodeName));
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "SeqLoops_submitDependencies Found ^last argument, replacing "
                  "%s for %s for node %s \n",
                  SeqNameValues_getValue(loopArgsPtr, _nodeDataPtr->nodeName),
                  tmpValue, _nodeDataPtr->nodeName);
  ...
  /* local dependencies (same exp) are fetched from
     sequencing/status/depends/$datestamp,
     remote dependencies are fetched from
     sequencing/status/remote_depends/$datestamp */
  for (count = 0; count < 2; count++) {
    if (count == 0) {
      /* local dependencies */
      sprintf(waited_filename, "%s%s%s/%s.waited_%s", _nodeDataPtr->expHome,
              LOCAL_DEPENDS_DIR, _nodeDataPtr->datestamp, extName, _signal);
    } else {
      /* remote dependencies */
      sprintf(waited_filename, "%s%s%s/%s.waited_%s", _nodeDataPtr->expHome,
              REMOTE_DEPENDS_DIR, _nodeDataPtr->datestamp, extName, _signal);
    }
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "maestro.submitDependencies() looking for waited file=%s\n",
                  waited_filename);

    if (_access(waited_filename, R_OK, _nodeDataPtr->expHome) == 0) {
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "maestro.submitDependencies() found waited file=%s\n",
                    waited_filename);
      if ((waitedFilePtr = _fopen(waited_filename, MLLServerConnectionFid)) !=
          NULL) {
        if (strcmp(_flow, "continue") == 0) {
          ...
          while (current_dep_line != NULL) {
            ...
            SeqUtil_TRACE(
                TL_FULL_TRACE,
                "maestro.submitDependencies() from waited file line: %s\n",
                current_dep_line->data);
            sscanf(current_dep_line->data,
                   "exp=%255s node=%255s datestamp=%19s args=%255s", depExp,
                   depNode, depDatestamp, depArgs);
            SeqUtil_TRACE(TL_FULL_TRACE,
                          "maestro.submitDependencies() waited file data "
                          "depExp:%s depNode:%s depDatestamp:%s depArgs:%s\n",
                          depExp, depNode, depDatestamp, depArgs);
            /* is the dependant still waiting? if not, don't submit. */
            if (!isNodeXState(depNode, depArgs, depDatestamp, depExp,
                              "waiting")) {
              SeqUtil_TRACE(TL_FULL_TRACE,
                            "maestro.submitDependencies() dependant node not "
                            "currently in wait state, skipping submission. \n");
              goto next;
            }
            ...
            SeqUtil_TRACE(
                TL_FULL_TRACE,
                "submitDependencies(): Using system calls to submit %s\n",
                depNode);
            if (strlen(depArgs) > 0) {
            ...
            sprintf(submitCmd,
                    "maestro -e %s -d %s -s submit -f continue -n %s %s",
                    depExp, depDatestamp, depNode, submitDepArgs);
            SeqUtil_TRACE(TL_FULL_TRACE,
                          "submitDependencies(): Running system command: %s\n",
                          submitCmd);
...
#else
            SeqUtil_TRACE(TL_FULL_TRACE,
                          "submitDependencies(): Using function call to submit "
                          "dependency %s\n",
                          depNode);
            SeqUtil_TRACE(TL_FULL_TRACE,
                          "submitDependencies calling maestro:\n\t\
                           maestro(%s, \"submit\", \"continue\", %s, 0, NULL, %s, %s );\n",
                          depNode, depArgs, depDatestamp, depExp);
            SeqLoops_parseArgs(&submitDepArgs, depArgs);
            submitCode = maestro(depNode, "submit", "continue", submitDepArgs,
                                 0, NULL, depDatestamp, depExp);
            ...
            if (submitCode != 0) {
              ...
              _WriteNWFile(depExp, depNode, depDatestamp, depArgs,
                           waited_filename, statusFile);
              SeqUtil_TRACE(TL_ERROR,
                            "Error submitting node %s of experiment %s \n",
                            depNode, depExp);
              sprintf(nodelogger_msg,
                      "An error occurred while submitting dependant node %s in "
                      "experiment %s",
                      depNode, depExp);
              nodelogger(_nodeDataPtr->name, "info", _nodeDataPtr->extension,
                         nodelogger_msg, _nodeDataPtr->datestamp,
                         _nodeDataPtr->expHome);
            }
          next:
            depExp[0] = '\0';
          ...
          /* warn if file empty ... */
          if (line_count == 0)
            raiseError("waited_end file:%s (submitDependencies) EMPTY !!!! \n",
                       waited_filename);
        } else {
          /* Flow is not "continue" : Simply emit a nodelogger messge for every
           * line in the file */
          while (fgets(line, sizeof(line), waitedFilePtr) != NULL) {
            sscanf(line, "exp=%255s node=%255s datestamp=%19s args=%255s",
                   depExp, depNode, depDatestamp, depArgs);
            SeqUtil_TRACE(TL_FULL_TRACE,
                          "maestro.submitDependencies() read in line exp=%s "
                          "node=%s datestamp=%s args=%s \n",
                          depExp, depNode, depDatestamp, depArgs);
            if ((depArgs != NULL) && (strlen(depArgs) > 0) &&
                (SeqLoops_parseArgs(&depNVArgs, depArgs) != -1)) {
              depExtension = SeqLoops_getExtFromLoopArgs(depNVArgs);
              SeqNameValues_deleteWholeList(&depNVArgs);
            }
            sprintf(nodelogger_msg,
                    "Node was waiting on node %s%s of experiment %s which "
                    "ended with flow set to stop : remaining in waiting state.",
                    _nodeDataPtr->name, _nodeDataPtr->extension,
                    _nodeDataPtr->expHome);
            nodelogger(depNode, "info", depExtension, nodelogger_msg,
                       depDatestamp, depExp);
            free(depExtension);
            depExtension = NULL;
          }
...
Returns an integer saying whether the targetted node is in a given state. 1 if
node is in the desired state, 0 if not.

Inputs:
const char * node - target node
const char * loopargs - what is the target node's loop index in csv name=value
format const char * datestamp - what datestamp are we verifying const char * exp
- what experiment is the node in const char * state - what state are we
verifying
...
int isNodeXState(const char *node, const char *loopargs, const char *datestamp,
  ...
  SeqUtil_TRACE(
      TL_FULL_TRACE,
      "isNodeXState node=%s, loopargs=%s, datestamp=%s, exp=%s, state=%s \n",
  ...
  sprintf(stateFile, "%s/sequencing/status/%s/%s%s.%s", exp, datestamp, node,
          extension, state);
  ...
  if (result)
    ...
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "isNodeXState file=%s not found. Returning 0.\n", stateFile);
...
Writes the dependency lockfile in the directory of the node that this current
node is waiting for.
...
static int writeNodeWaitedFile(const SeqNodeDataPtr _nodeDataPtr,
  ...
  SeqUtil_TRACE(TL_FULL_TRACE,
                "maestro.writeNodeWaitedFile() _dep_exp_path=%s, _dep_node=%s, "
                "_dep_index=%s _dep_datestamp=%s\n",
  ...
  if (_dep_scope == IntraSuite) {
    /* write in sequencing/status/depends/$datestamp */
    sprintf(filename, "%s/%s/%s/%s", _dep_exp_path, LOCAL_DEPENDS_DIR,
            _dep_datestamp, depBase);
    _SeqUtil_mkdir(filename, 1, _nodeDataPtr->expHome);
    if (_dep_index != NULL && strlen(_dep_index) > 0 && _dep_index[0] != '.') {
      /* add extra dot for extension */
      sprintf(filename, "%s%s%s/%s.%s.waited_%s", _dep_exp_path,
              LOCAL_DEPENDS_DIR, _dep_datestamp, _dep_node, _dep_index,
              _dep_status);
    } else {
      sprintf(filename, "%s%s%s/%s%s.waited_%s", _dep_exp_path,
              LOCAL_DEPENDS_DIR, _dep_datestamp, _dep_node, _dep_index,
    ...
    sprintf(filename, "%s%s%s/%s", _dep_exp_path, REMOTE_DEPENDS_DIR,
            _dep_datestamp, depBase);
    _SeqUtil_mkdir(filename, 1, _nodeDataPtr->expHome);
    if (_dep_index != NULL && strlen(_dep_index) > 0 && _dep_index[0] != '.') {
      /* add extra dot for extension */
      sprintf(filename, "%s%s%s/%s.%s.waited_%s", _dep_exp_path,
              REMOTE_DEPENDS_DIR, _dep_datestamp, _dep_node, _dep_index,
              _dep_status);
    } else {
      sprintf(filename, "%s%s%s/%s%s.waited_%s", _dep_exp_path,
              REMOTE_DEPENDS_DIR, _dep_datestamp, _dep_node, _dep_index,
  ...
  status =
      _WriteNWFile(_nodeDataPtr->expHome, _nodeDataPtr->name,
                   _nodeDataPtr->datestamp, loopArgs, filename, statusfile);
  ...
  case 1:
    raiseError(
        "maestro cannot write Node Waited file: %s (writeNodeWaitedFile)\n",
        filename);
    break;
  case 9:
    status = 0;
...
char *formatWaitingMsg(SeqDependsScope _dep_scope, const char *_dep_exp,
  ...
  if (_dep_scope == IntraSuite) {
    sprintf(waitMsg, "node=%s%s datestamp=%s", _dep_node, _dep_index,
            _dep_datestamp);
  } else {
    sprintf(waitMsg, "exp=%s node=%s%s datestamp=%s", _dep_exp, _dep_node,
            _dep_index, _dep_datestamp);
  }
...
void setDepExpScope(SeqNodeDataPtr ndp, SeqDepDataPtr dep) {
  ...
  if (_access(seqPath, W_OK, ndp->expHome) == 0) {
    ...
    if (get_Inode(ndp->expHome) == get_Inode(dep->exp)) {
      ...
      dep->exp_scope = IntraSuite;
    } else {
      dep->exp_scope = IntraUser;
    }
  } else {
...
void setDepDatestamp(SeqNodeDataPtr ndp, SeqDepDataPtr dep) {
  ...
  if (dep->time_delta != NULL && strlen(dep->time_delta) > 0) {
    ...
    dep->datestamp = SeqDatesUtil_addTimeDelta(ndp->datestamp, dep->time_delta);
  } else if (dep->hour != NULL && strlen(dep->hour) > 0) {
    /* calculate relative datestamp based on the current one */
    dep->datestamp =
        SeqDatesUtil_getPrintableDate(ndp->datestamp, 0, atoi(dep->hour), 0, 0);
  } else {
...
out:
  ...
  SeqUtil_TRACE(TL_FULL_TRACE,
                "check_depIsInScope() end : dep->isInScope = %d\n",
                dep->isInScope);
}
 ...
 * satisfied.
 ********************************************************************************/
static int validateSingleDep(SeqNodeDataPtr ndp, SeqDepDataPtr dep,
                             const char *_flow) {
  ...
  default:
    SeqUtil_TRACE(TL_FULL_TRACE, "validateDependencies() unprocessed "
                                 "nodeinfo_depend_type=%d depsPtr->type\n");
  }
 ...
 ********************************************************************************/
static int validateDependencies(SeqNodeDataPtr ndp, const char *_flow) {
...
/* this function is used to process dependency status files. It does the
 * following:
...
int checkTargetedIterations(SeqNodeDataPtr _nodeDataPtr,
                            ...
                            const char *_flow);
int processWaitingState(SeqNodeDataPtr _nodeDataPtr, SeqDepDataPtr dep,
                        const char *_flow, char *extension, char *statusFile);
int writeWaitedFile(SeqNodeDataPtr _nodeDataPtr, SeqDepDataPtr dep,
                    const char *extension, const char *_flow,
                    const char *statusFile);
...
int processDepStatus(const SeqNodeDataPtr _nodeDataPtr, SeqDepDataPtr dep,
  ...
  SeqNodeDataPtr depNodeDataPtr = nodeinfo(
      dep->node_name, NI_SHOW_ALL, NULL, dep->exp, NULL, dep->datestamp, NULL);
  /* check catchup value of the node */
...
int checkTargetedIterations(SeqNodeDataPtr _nodeDataPtr,
  ...
  for (itr = extensions; itr != NULL; itr = itr->nextPtr) {
    ...
    if (!_isFileExists(statusFile, "maestro.checkTargetedIterations()",
                       _nodeDataPtr->expHome)) {
      /* create waiting file and set state to waiting */
      retval = processWaitingState(_nodeDataPtr, dep, _flow, lastCheckedIndex,
                                   statusFile);
...
/********************************************************************************
 * Will create the waiting message and file for a given node caused by an
 *unsatisfied dependency The input is the node, the dependant node and waiting
 *extension the output is proceesing the waiting state being completed.
 ********************************************************************************/
...
int processWaitingState(SeqNodeDataPtr _nodeDataPtr, SeqDepDataPtr dep,
  ...
  free(waitingMsg);
  writeStatus =
      writeWaitedFile(_nodeDataPtr, dep, extension, _flow, statusFile);
  if (writeStatus == 0) {
    raiseError("processWaitingState() couldn't write dependency into %s in "
               "maestro.processDepStatus()\n",
               statusFile);
  }
  SeqUtil_TRACE(TL_FULL_TRACE, "processWaitingState() begin\n");
...
int writeWaitedFile(SeqNodeDataPtr _nodeDataPtr, SeqDepDataPtr dep,
  ...
  if (dep->exp_scope == InterUser) {
    ...
    writeStatus = writeNodeWaitedFile(_nodeDataPtr, dep->exp, dep->node_name,
                                      dep->status, extension, dep->datestamp,
                                      dep->exp_scope, statusFile);
  }
  return writeStatus;
}
...
int writeInterUserNodeWaitedFile(const SeqNodeDataPtr _nodeDataPtr,
  ...
  if (loopArgs != NULL && strlen(loopArgs) != 0) {
    snprintf(depFile, sizeof(depFile), "%s_%s%s_%s.%s", _nodeDataPtr->datestamp,
             _nodeDataPtr->expHome, _nodeDataPtr->name, loopArgs, _dep_status);
  } else {
    snprintf(depFile, sizeof(depFile), "%s_%s%s.%s", _nodeDataPtr->datestamp,
             _nodeDataPtr->expHome, _nodeDataPtr->name, _dep_status);
  }
  ...
  snprintf(
      depParam, sizeof(depParam),
      " <xp>%s</xp>\n <node>%s</node>\n <indx>%s</indx>\n <xdate>%s</xdate>\n <status>%s</status>\n <largs>%s</largs>\n \
<sxp>%s</sxp>\n <snode>%s</snode>\n <sxdate>%s</sxdate>\n <slargs>%s</slargs>\n <lock>%s</lock>\n <container>%s</container>\n \
<mdomain>%s</mdomain>\n <mversion>%s</mversion>\n <regtime date=\"%s\" epoch=\"%ld\" />\n <flow>%s</flow>\n<key>%s</key>\n",
      _dep_exp, _dep_name, depIndexPtr, _dep_datestamp, _dep_status, _dep_index,
      _nodeDataPtr->expHome, _nodeDataPtr->name, _nodeDataPtr->datestamp,
      loopArgs, statusFile, _nodeDataPtr->container, maestro_shortcut,
      maestro_version, registration_time, current_time, _flow, md5sum);
  ...
  if (MLLServerConnectionFid < 0) {
    nodelogger(_nodeDataPtr->name, "info", _nodeDataPtr->extension,
               "Dependency registered to mserver but mserver inactive. Run "
               "mserver to be able to launch dependency.",
               _nodeDataPtr->datestamp, _nodeDataPtr->expHome);
  }
...
Main function. Creates the node data by calling nodeinfo on the arguments
passed. Validates input as well. Calls the appropriate state function for the
desired action.
...
Inputs:
  ...
  ignoreAllDeps - integer representing true or false whether to ignore all
dependencies and catchup extraArgs - pointer to extra soumet arguments provided

*/

int maestro(char *_node, char *_signal, char *_flow, SeqNameValuesPtr _loops,
            int ignoreAllDeps, char *_extraArgs, char *_datestamp,
            char *_seq_exp_home) {
  char buffer[SEQ_MAXFIELD] = {'\0'};
  char tmpdir[256];
  char *seq_soumet = NULL, *tmp = NULL, *logMech = NULL, *defFile = NULL,
       *windowAverage = NULL, *runStats = NULL, *shortcut = NULL,
       *loopArgs = NULL;
  char *loopExtension = NULL, *nodeExtension = NULL, *extension = NULL,
       *tmpFullOrigin = NULL, *tmpLoopExt = NULL, *tmpJobID = NULL,
       *tmpNodeOrigin = NULL, *tmpHost = NULL, *fixedPath;
  ...
  loopArgs = SeqLoops_getLoopArgs(_loops);
  fprintf(stderr,
          "maestro: node=%s signal=%s flow=%s loop_args=%s extraArgs=%s "
          "expHome=%s\n",
          _node, _signal, _flow, loopArgs, _extraArgs, _seq_exp_home);

  if (_loops != NULL) {
    SeqUtil_stringAppend(&tmp, " -l ");
    SeqUtil_stringAppend(&tmp, loopArgs);
  }
  if (ignoreAllDeps) {
  ...
  if (_seq_exp_home != NULL) {
    ...
    raiseError("maestro(): argument _seq_exp_home of maestro() function must "
               "be supplied \n");
  }
  ...
  SeqUtil_TRACE(TL_FULL_TRACE, "maestro() using submit script=%s\n",
                submit_tool);
  nodeDataPtr = nodeinfo(_node, NI_SHOW_ALL, _loops, _seq_exp_home, _extraArgs,
                         _datestamp, NULL);
  ...
  if ((logMech = SeqUtil_getdef(defFile, "SEQ_LOGGING_MECH",
                                nodeDataPtr->expHome)) != NULL) {
    free(defFile);
    defFile = NULL;
  } else {
    logMech = strdup("nfs");
  ...
  /* Install handler for
   *   SIGALRM to be able to time out socket routines This handler must be
   * installed only once SIGPIPE : in case of socket closed */
  ...
  if (ServerConnectionStatus == 1) {
    if (strcmp(logMech, "server") == 0) {
      MLLServerConnectionFid =
          OpenConnectionToMLLServer(_node, _signal, _seq_exp_home);
      if (MLLServerConnectionFid > 0) {
        ServerConnectionStatus = 0;
        SeqUtil_TRACE(
            TL_CRITICAL,
            "##Server Connection Open And Login Accepted Signal:%s ##\n",
            _signal);
        useSVRlocking();
      } else {
        ...
        case -1:
          fprintf(stderr,
                  ">>>>>Warning could not open connection with server<<<<<<\n");
          break;
        case -2:
  ...
  sprintf(tmpdir, "%s/%s/%s", nodeDataPtr->workdir, nodeDataPtr->datestamp,
          nodeDataPtr->container);
  _SeqUtil_mkdir(tmpdir, 1, nodeDataPtr->expHome);

  if ((strcmp(_signal, "end") == 0) || (strcmp(_signal, "endx") == 0) ||
      (strcmp(_signal, "endmodel") == 0) ||
      (strcmp(_signal, "endloops") == 0) ||
      (strcmp(_signal, "endmodelloops") == 0) ||
      (strncmp(_signal, "endloops_", 9) == 0) ||
      (strncmp(_signal, "endmodelloops_", 14) == 0)) {
    ...
    if (strcmp(nodeDataPtr->container, "") == 0) {
      if ((defFile = malloc(strlen(nodeDataPtr->expHome) +
                            strlen("/resources/resources.def") + 1)) != NULL) {
        sprintf(defFile, "%s/resources/resources.def", nodeDataPtr->expHome);
      } else {
        raiseError("OutOfMemory exception in maestro()\n");
      }
      if ((runStats = SeqUtil_getdef(defFile, "SEQ_RUN_STATS_ON",
                                     nodeDataPtr->expHome)) != NULL) {
        if ((strncmp(runStats, "off", 3) == 0) ||
            (strncmp(runStats, "0", 1) == 0)) {
          SeqUtil_TRACE(TL_FULL_TRACE, "maestro() skipping job statictics.\n");
        } else {
          SeqUtil_TRACE(TL_FULL_TRACE, "maestro() running job statictics.\n");
          logreader(NULL, NULL, nodeDataPtr->expHome, nodeDataPtr->datestamp,
                    LR_SHOW_STATS, 0, 0);
          if ((windowAverage = SeqUtil_getdef(defFile, "SEQ_AVERAGE_WINDOW",
                                              nodeDataPtr->expHome)) != NULL) {
            SeqUtil_TRACE(TL_FULL_TRACE, "maestro() running averaging.\n");
            logreader(NULL, NULL, nodeDataPtr->expHome, nodeDataPtr->datestamp,
                      LR_CALC_AVG, atoi(windowAverage), 0);
          }
        }
      }
      free(defFile);
  ...
  if (strcmp(_signal, "submit") == 0) {
    ...
    if (shortcut == NULL) {
      raiseError(
          "SEQ_MAESTRO_SHORTCUT environment variable not set but required for "
          "submissions. Please load the maestro package properly.\n");
    }
