...
FlowVisitorPtr Flow_newVisitor(const char *nodePath, const char *seq_exp_home,
  ...
  char *xmlFilename =
      (char *)malloc(strlen(seq_exp_home) + strlen(postfix) + 1);
  if (xmlFilename == NULL) {
  ...
  new_flow_visitor->switch_args =
      (switch_args && strlen(switch_args) ? strdup(switch_args) : NULL);
...
int Flow_parsePath(FlowVisitorPtr _flow_visitor, SeqNodeDataPtr _nodeDataPtr,
  ...
  for_tokens(pathToken, _nodePath, "/", sp) {
    ...
    if (Flow_doNodeQuery(_flow_visitor, pathToken, count == 0) ==
        FLOW_FAILURE) {
      retval = FLOW_FAILURE;
      goto out;
    }
...
int Flow_changeModule(FlowVisitorPtr _flow_visitor, const char *module) {
  ...
  sprintf(xmlFilename, "%s%s%s%s", _flow_visitor->expHome, infix, module,
          postfix);
  ...
  if ((result = XmlUtils_getnodeset("(/MODULE)", _flow_visitor->context)) ==
      NULL) {
    retval = FLOW_FAILURE;
    goto out;
  } else {
 ...
 ********************************************************************************/
int Flow_updatePaths(FlowVisitorPtr _flow_visitor, const char *pathToken,
                     const int container) {
  ...
  if (container) {
    ...
    case ForEach:
      ...
      free(_flow_visitor->intramodulePath);
      _flow_visitor->intramodulePath = NULL;
      SeqUtil_stringAppend(&(_flow_visitor->intramodulePath),
...
char *Flow_findSwitchArg(FlowVisitorPtr fv) {
  SeqUtil_TRACE(
      TL_FULL_TRACE,
      "Flow_findSwitchArg() begin,fv->nodePath:%s fv->switch_args=%s\n",
 ...
 * Returns FLOW_FAILURE otherwise.
 ********************************************************************************/
int Flow_parseSwitchAttributes(FlowVisitorPtr fv, SeqNodeDataPtr _nodeDataPtr,
                               int isLast) {
  ...
  SeqNameValues_insertItem(&(_nodeDataPtr->switchAnswers), fixedSwitchPath,
                           switchValue);
  ...
  if (Flow_findSwitchItem(fv, switchValue) == FLOW_FAILURE) {
    SeqUtil_TRACE(
        TL_FULL_TRACE,
        "Flow_parseSwitchAttributes(): no SWITCH_ITEM found containing "
        "value=%s and no SWITCH_ITEM found containing value=%s\n",
        switchValue);
    retval = FLOW_FAILURE;
    goto out_free;
  }
...
int Flow_findSwitchItem(FlowVisitorPtr _flow_visitor, const char *switchValue) {
  ...
  if (Flow_findSwitchItemWithValue(_flow_visitor, switchValue) ==
      FLOW_SUCCESS) {
    retval = FLOW_SUCCESS;
    goto out;
 ...
 ********************************************************************************/
int Flow_findSwitchItemWithValue(FlowVisitorPtr _flow_visitor,
  ...
  if ((switchItemResult =
           XmlUtils_getnodeset("(child::SWITCH_ITEM[not(@name='default')])",
                               _flow_visitor->context)) == NULL) {
    retval = FLOW_FAILURE;
    goto out;
  }
...
int Flow_findDefaultSwitchItem(FlowVisitorPtr _flow_visitor) {
  ...
  if ((switchItemResult =
           XmlUtils_getnodeset("(child::SWITCH_ITEM[@name='default'])",
                               _flow_visitor->context)) == NULL) {
    retval = FLOW_FAILURE;
    goto out;
  } else {
...
/********************************************************************************
 * Returns FLOW_TRUE if the SWITCH_ITEM currentNodePtr has the argument
 *switchValue as one of the tokens in it's name attribute. Note that we use the
 *XML XPath context to perform the attribute search.  For this, we need to
 *change the context's current node temporarily and restore it upon exiting the
 *function.
 ********************************************************************************/
int Flow_switchItemHasValue(const FlowVisitorPtr _flow_visitor,
  ...
  if ((attributesResult =
           XmlUtils_getnodeset("(@name)", _flow_visitor->context)) == NULL)
    raiseError(
        "Flow_switchItemHasValue(): SWITCH_ITEM with no name attribute\n");
 ...
 ********************************************************************************/
int Flow_checkWorkUnit(FlowVisitorPtr _flow_visitor,
  ...
  if ((attributesResult = XmlUtils_getnodeset("(@work_unit)", context)) ==
      NULL) {
    goto out;
  } else {
  ...
  if (_flow_visitor->currentNodeType == Task ||
    ...
    SeqUtil_TRACE(TL_CRITICAL,
                  "Work unit mode is only for containers (single_reserv=1)");
    retval = FLOW_FAILURE;
    goto out_free;
  }
  if (_flow_visitor->currentFlowNode == NULL) {
    SeqUtil_TRACE(
        TL_CRITICAL,
        "Work unit mode cannot be on the root node (single_reserv=1)");
    retval = FLOW_FAILURE;
    goto out_free;
  }

  Resource_parseWorkerPath(_flow_visitor->currentFlowNode,
                           _flow_visitor->expHome, _nodeDataPtr);
 ...
 ********************************************************************************/
int Flow_setPathData(FlowVisitorPtr _flow_visitor,
                     SeqNodeDataPtr _nodeDataPtr) {
  ...
  SeqUtil_TRACE(
      TL_FULL_TRACE,
      "nodeinfo.getFlowInfo() *********** Node Information **********\n");
  ...
  SeqUtil_TRACE(TL_FULL_TRACE,
                "nodeinfo.getFlowInfo() intramodule_container=%s\n",
 ...
 * _nodeDataPtr->container.
 ********************************************************************************/
int Flow_setPathToModule(FlowVisitorPtr _flow_visitor,
                         SeqNodeDataPtr _nodeDataPtr) {
  ...
  if (_flow_visitor->intramodulePath != NULL && !entryModule) {
    int lengthDiff = strlen(_nodeDataPtr->container) -
                     strlen(_flow_visitor->intramodulePath);
 ...
 ********************************************************************************/
int Flow_setSuiteName(FlowVisitorPtr _flow_visitor,
                      SeqNodeDataPtr _nodeDataPtr) {
 ...
 ********************************************************************************/
int Flow_parseDependencies(FlowVisitorPtr _flow_visitor,
                           SeqNodeDataPtr _nodeDataPtr) {
 ...
 ********************************************************************************/
int Flow_parseSubmits(FlowVisitorPtr _flow_visitor,
                      SeqNodeDataPtr _nodeDataPtr) {
  SeqUtil_TRACE(TL_FULL_TRACE, "Flow_parseSubmits begin\n");
  SeqUtil_TRACE(TL_FULL_TRACE,
                "nodeinfo.getFlowInfo() *********** submits **********\n");
 ...
 ********************************************************************************/
int Flow_parseSiblings(FlowVisitorPtr _flow_visitor,
                       SeqNodeDataPtr _nodeDataPtr) {
  SeqUtil_TRACE(TL_FULL_TRACE, "Flow_parseSiblings begin\n");
  SeqUtil_TRACE(
      TL_FULL_TRACE,
      "nodeinfo.getFlowInfo() *********** node siblings **********\n");
  int switchItemFound = 0;
  switchItemFound =
      (strcmp(_flow_visitor->context->node->name, "SWITCH_ITEM") == 0);
  xmlXPathObjectPtr result = NULL;
  xmlXPathContextPtr context = NULL;
  char query[SEQ_MAXFIELD] = {'\0'};
  ...
  if (result) {
    SeqUtil_TRACE(TL_FULL_TRACE, "nodeinfo.getFlowInfo() *********** preceding "
                                 "siblings found**********\n");
  }
  ...
  result = XmlUtils_getnodeset(query, context);
  if (result) {
    SeqUtil_TRACE(TL_FULL_TRACE, "nodeinfo.getFlowInfo() *********** following "
                                 "siblings found**********\n");
  }
  parseNodeSiblings(result, _nodeDataPtr);
 ...
 * visitor.
 ********************************************************************************/
int Flow_parseSpecifics(FlowVisitorPtr _flow_visitor,
                        SeqNodeDataPtr _nodeDataPtr) {
