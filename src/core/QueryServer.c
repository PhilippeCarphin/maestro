...
int Query_L2D2_Server(int sock, ServerActions action, const char *buf,
                      const char *buf2, const char *_seq_exp_home) {
  ...
  default:
    fprintf(stderr,
            "@@@@@@@@@@@ ERROR Unrecognized action for the Server:%d "
            "@@@@@@@@@@@ \n",
            action);
    return (-1);
    break;
  ...
  if ((bytes_sent =
           send_socket(sock, buffer, sizeof(buffer),
  ...
  memset(Rbuffer, '\0', sizeof(Rbuffer));
  if ((bytes_read =
           recv_socket(sock, Rbuffer, sizeof(Rbuffer),
                       SeqUtil_getEnvOrDefaultI("SEQ_TIMEOUT_CLIENT",
                                                SOCK_TIMEOUT_CLIENT))) <= 0) {
  ...
  default: /* something happend ...
    ...
    fprintf(
        stderr,
        "====== Reverting to nfs Routines Answer for action=%d:>%s< ====== \n",
        action, Rbuffer);
    ret_nfs = revert_nfs(buf, action, buf2, _seq_exp_home);
    return (ret_nfs);
  }
...
int revert_nfs(const char *buf, ServerActions action, const char *buf2,
  ...
  int ret = 0;
  char sfile[1024], filename[1024], pwname[1024], seq_xp_home[1024],
      nname[1024], datestamp[1024], loopArgs[1024];
  ...
  case SVR_WRITE_WNF:
    ...
    ret = sscanf(buf,
                 "sfile=%1023s wfile=%1023s exp=%1023s node=%1023s "
                 "datestamp=%1023s args=%1023s",
                 sfile, filename, seq_xp_home, nname, datestamp, loopArgs);
    ret = WriteNodeWaitedFile_nfs(seq_xp_home, nname, datestamp, loopArgs,
                                  filename, sfile);
    break;
  case SVR_WRITE_USERDFILE: /* have to be reviewed , if server shutdon we may
                               only have the first chunk of transmission
    ...
    ret = sscanf(
        buf,
        "K file=%1023s Dbf=%1023s pwd=%1023s mv=%127s m5s=%127s dstmp=%1023s",
        filename, sfile, pwname, mversion, md5sum, datestamp);
    ret = WriteInterUserDepFile_nfs(filename, sfile, pwname, mversion,
                                    datestamp, md5sum);
    break;
  default:
    fprintf(stderr,
            "@@@@@@@@@@@ ERROR Inrecognized action for the Server:%d Buffer=%s "
            "@@@@@@@@@@@ \n",
            action, buf);
    return (-1);
  }
 ...
 * rev.     : 1.0
 */
int OpenConnectionToMLLServer(const char *node, const char *signal,
                              const char *_seq_exp_home) {
  unsigned int pid;
  ...
  if (Auth_token == NULL) {
    ...
    sscanf(Auth_token, "seqpid=%u seqhost=%19s seqip=%19s seqport=%d", &pid,
           htserver, ipserver, &port);
    fprintf(
        stderr,
        "maestro L2D2 server parameters are: <pid=%u  host=%s ip=%s port=%d>\n",
  ...
  if ((sock = connect_to_host_port_by_ip(ipserver, port)) < 1) {
    fprintf(stderr,
            "Cannot connect to host=%s ip=%s and port=%d  ... exiting\n",
            htserver, ipserver, port);
    return (-1);
  }
  ...
  ret = do_Login(sock, pid, node, _seq_exp_home, signal, passwdEnt->pw_name,
                 &m5sum);
  free(m5sum);
...
void CloseConnectionWithMLLServer(int con) {
  ...
  if ((bytes_sent =
           send_socket(con, "S \0", 3,
                       SeqUtil_getEnvOrDefaultI("SEQ_TIMEOUT_CLIENT",
                                                SOCK_TIMEOUT_CLIENT))) <= 0) {
    fprintf(stderr, "%%%%%%%%%%%% CloseConnectionWithMLLServer: socket closed "
                    "at send %%%%%%%%%%%%%%\n");
  } else
    close(con);
}
