...
PathArgNodePtr getNodeList(const char *seq_exp_home, const char *datestamp) {
  ...
  const char *basePath =
      (const char *)xmlGetProp(fv->context->node, (const xmlChar *)"name");
...
int PathArgNode_pushFront(PathArgNodePtr *list_head, const char *path,
  ...
  if ((newNode = malloc(sizeof *newNode)) == NULL) {
    SeqUtil_TRACE(TL_CRITICAL,
                  "PathArgPair_pushFront() No memory available.\n");
    return 1;
  }
...
void getNodeList_internal(FlowVisitorPtr fv, PathArgNodePtr *pathArgList,
  ...
  xmlXPathObjectPtr results =
      XmlUtils_getnodeset((const xmlChar *)"(child::FAMILY|child::TASK\
  ...
  for_results(xmlNode, results) {
    ...
    if (strcmp(xmlNode->name, "TASK") == 0 ||
        strcmp(xmlNode->name, "NPASS_TASK") == 0) {
      ...
      if (fv->datestamp != NULL) {
         ...
         */
        raiseError("ERROR: A datestamp was specified but getNodeList_internal "
                   "reached a SWITCH_ITEM\n");
      }
...
void gnl_switch_item(FlowVisitorPtr fv, PathArgNodePtr *pathArgList,
  ...
  if (first_comma != NULL)
    *first_comma = '\0';
  sprintf(switch_args, "%s%s=%s,", baseSwitchArgs, switch_name,
          switch_item_name);
