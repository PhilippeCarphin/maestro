/* tictac_main.c - Command-line API for datestamp utility tictac, part of the
...
static void printUsage() {
  char *usage =
      "DESCRIPTION: Date accessor/modifier interface for experiments\n\
\n\
USAGE:\n\
    \n\
  ...
  if (argc >= 2) {
    ...
    if (expHome == NULL) {
      if ((expHome = getenv("SEQ_EXP_HOME")) == NULL) {
        fprintf(stderr, "SEQ_EXP_HOME must be set either through the "
                        "environment or with -e (--exp)\n");
        exit(1);
      }
    }
