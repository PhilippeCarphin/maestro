...
static void show_usage() {
  char *usage = "For complete and up to date information on this command, see "
                "the man page by typing 'man mserver'.";
  puts(usage);
}
...
void DependencyManager(_l2d2server l2d2) {
  ...
  snprintf(ControllerAlive, sizeof(ControllerAlive), "%s/CTR_%d", l2d2.tmpdir,
           l2d2.pid);
  ...
  if (sigaction(SIGUSR1, &sa, NULL) != 0)
    fprintf(dmlg, "error in sigactions  SIGUSR1\n");
  if (sigaction(SIGUSR2, &sa, NULL) != 0)
    fprintf(dmlg, "error in sigactions  SIGUSR2\n");
  ...
  if (access(l2d2.web_dep, R_OK) == 0 || stat(l2d2.web_dep, &st) == 0 ||
      st.st_size > 0) {
  ...
  /* for heartbeat */
  snprintf(DependencyMAlive, sizeof(DependencyMAlive), "%s/DM_%d", l2d2.tmpdir,
           getpid());
  ...
  while (running == 0) {
    ...
    while ((pd = readdir(dp))) {
      ...
      snprintf(ffilename, sizeof(ffilename), "%s/%s", l2d2.dependencyPollDir,
               pd->d_name);
      ...
      if (stat(ffilename, &st) != 0) {
        ...
        linkname[r] = '\0';
        fprintf(dmlg,
                "DependencyManager():%s inter-dependency file not there, "
                "removing link at:%s ... \n",
                linkname, Time);
      ...
      case 'r':
        /* skip hidden files */
        if (filename[0] == '.')
          break; /* should examine if a left over file ... */
                 /* test format */
        ...
        if (nb == 3) {
          ...
          if ((depXp = ParseXmlDepFile(linkname, dmlg)) == NULL) {
            get_time(Time, 1);
            fprintf(dmlg,
                    "DependencyManager(): %s Problem parsing xml file:%s\n",
                    Time, linkname);
          } else {
            /* Is dependant node still in waiting state? If not, do not submit.
             */
            if (l2d2_Util_isNodeXState(depXp->xpd_snode, depXp->xpd_slargs,
                                       depXp->xpd_sxpdate, depXp->xpd_sname,
                                       "waiting") == 0) {
              get_time(Time, 2);
              fprintf(dmlg,
                      "%s Removing dependency (waiting state of dependant "
                      "gone) ffilename=%s ; linkname=%s\n",
            ...
            if (strcmp(depXp->xpd_slargs, "") != 0)
              ...
              strcpy(largs, "");
            /* before trying to access lock file, see if dependency is still
             * active */
            epoch_diff =
                (int)(current_epoch - atoi(depXp->xpd_regtimepoch)) / 3600;
            if (epoch_diff >= l2d2.dependencyTimeOut) {
              ...
              snprintf(buf, sizeof(buf),
                       "Dependency on exp:%s node:%s from exp:%s and node:%s "
                       "Timed out. Removed by mserver at:%s",
                       depXp->xpd_name, depXp->xpd_node, depXp->xpd_sname,
                       depXp->xpd_snode, Time);
              snprintf(
                  cmd, sizeof(cmd),
                  "exec >/dev/null 2>&1;%s; export SEQ_EXP_HOME=%s; export "
                  "SEQ_DATE=%s; nodelogger -n %s %s -s info -m \"%s\" ",
                  l2d2.mshortcut, depXp->xpd_sname, depXp->xpd_sxpdate,
                  depXp->xpd_snode, largs, buf);
              ret = system(cmd);
              ...
              snprintf(cmd, sizeof(cmd),
                       "%s; export SEQ_EXP_HOME=%s; export SEQ_DATE=%s; "
                       "maestro -s initnode -n %s %s",
                       l2d2.mshortcut, depXp->xpd_sname, depXp->xpd_sxpdate,
                       depXp->xpd_snode, largs);
              ret = system(cmd);
              ...
              memset(listings, '\0', sizeof(listings));
              snprintf(listings, sizeof(listings), "%s/listings/%s%s",
                       depXp->xpd_sname, l2d2.host, depXp->xpd_container);
              if (access(listings, R_OK) != 0)
                ret = r_mkdir(listings, 1, dmlg);
              if (ret != 0)
                fprintf(dmlg, "DM:Could not create directory:%s\n", listings);
              memset(listings, '\0', sizeof(listings));
              if (strcmp(depXp->xpd_slargs, "") != 0) {
                snprintf(listings, sizeof(listings),
                         "%s/listings/%s/%s/%s_%s.submit.mserver.%s.%s",
                         depXp->xpd_sname, l2d2.host, depXp->xpd_container,
                         pleaf, depXp->xpd_slargs, depXp->xpd_sxpdate, Time);
              } else {
                snprintf(listings, sizeof(listings),
                         "%s/listings/%s/%s/%s.submit.mserver.%s.%s",
                         depXp->xpd_sname, l2d2.host, depXp->xpd_container,
                         pleaf, depXp->xpd_sxpdate, Time);
              }
              /* build command */
              snprintf(cmd, sizeof(cmd),
                       "%s >/dev/null 2>&1; export SEQ_EXP_HOME=%s; export "
                       "SEQ_DATE=%s; maestro -s submit -n %s %s -f %s >%s 2>&1",
                       l2d2.mshortcut, depXp->xpd_sname, depXp->xpd_sxpdate,
                       depXp->xpd_snode, largs, depXp->xpd_flow, listings);
              fprintf(dmlg, "dependency submit cmd=%s\n", cmd);
              /* take account of concurrency here ie multiple dependency
               * managers! */
              snprintf(buf, sizeof(buf), "%s/.%s", l2d2.dependencyPollDir,
                       filename);
            ...
            snprintf(buf, sizeof(buf),
                     "<td><table><tr><td><font "
                     "color=\"red\">SRC_EXP</font></td><td>%s</td>\n",
                     depXp->xpd_sname);
            fwrite(buf, 1, strlen(buf), fp);
            snprintf(
                buf, sizeof(buf),
                "<tr><td><font color=\"red\">SRC_NODE</font></td><td>%s</td>\n",
                depXp->xpd_snode);
            fwrite(buf, 1, strlen(buf), fp);
            snprintf(buf, sizeof(buf),
                     "<tr><td><font "
                     "color=\"red\">DEP_ON_EXP</font></td><td>%s</td>\n",
                     depXp->xpd_name);
            fwrite(buf, 1, strlen(buf), fp);
            snprintf(
                buf, sizeof(buf),
                "<tr><td><font color=\"red\">Key</font></td><td>%s_%s</td>\n",
                depXp->xpd_xpdate, depXp->xpd_key);
            fwrite(buf, 1, strlen(buf), fp);
            snprintf(buf, sizeof(buf),
                     "<tr><td><font "
                     "color=\"red\">LOCK</font></td><td>%s</td></table></td>\n",
                     depXp->xpd_lock);
            fwrite(buf, 1, strlen(buf), fp);
            free(depXp);
            depXp = NULL;
    ...
    if (KILL_SERVER) {
      ...
      if (g_lres == 0 && g_LogFiles.gl_pathc > 0) {
        for (p = g_LogFiles.gl_pathv, cnt = g_LogFiles.gl_pathc; cnt;
             p++, cnt--) {
          if ((ret = sscanf(*p, "%[^EW_]%[EW_]%d", buf, ThreeChar, &epid)) ==
              3) {
            fprintf(dmlg,
                    "Killing Eternal worker having pid:%d from Dependency "
                    "Manager Process\n",
                    epid);
            ret = kill(epid, 9);
          } else
            fprintf(dmlg,
                    "Trying to kill Eternal Worker=%s, Parsing Error buf=%s "
                    "pid=%d num=%d\n",
                    *p, buf, epid, ret);
        }
      }
    ...
    if ((diff_t = difftime(current_epoch, start_epoch)) >= 120) {
      ...
      if (strncmp(tlog, Time, 8) != 0) {
        ...
        fclose(dmlg);
        snprintf(buf, sizeof(buf), "%s_%.8s_%d", l2d2.dmlog, Time,
                 l2d2.depProcPid);
        if ((dmlg = fopen(buf, "w+")) == NULL) {
          /* write somewhere */
    ...
    if (l2d2.clean_times.clean_flag == 1 &&
        (diff_t = difftime(current_epoch, start_epoch_cln)) >= 1800) {
...
static void l2d2SelectServlet(int listen_sd, TypeOfWorker tworker) {
  ...
  memset(buf, '\0', sizeof(buf));
  if (tworker == ETERNAL)
    snprintf(buf, sizeof(buf), "%s/meworker_%.8s_%d", L2D2.logdir, Stime,
             getpid());
  else
    snprintf(buf, sizeof(buf), "%s/mtworker_%.8s_%d", L2D2.logdir, Stime,
             getpid());
  ...
  /* for heartbeat */
  if (tworker == ETERNAL) {
    snprintf(heartbeatFile, sizeof(heartbeatFile), "%s/EW_%d", L2D2.tmpdir,
             getpid());
  } else {
    snprintf(heartbeatFile, sizeof(heartbeatFile), "%s/TRW_%d", L2D2.tmpdir,
             getpid());
  }
  ...
  /* get reference time to manage sending of signals to add workers to main
   * server */
  time(&sig_sent);
  ...
  do {
    ...
    if (rc < 0) {
      if (mlog != NULL)
        fprintf(mlog, "select() failed: Worker end \n");
      break;
    }
      ...
      if (FD_ISSET(i, &working_set)) {
        ...
        if (i == listen_sd) {
          ...
          do {
            ...
            if (!(fcntl(new_sd, F_GETFL) & O_NONBLOCK)) {
              if (fcntl(new_sd, F_SETFL, fcntl(new_sd, F_GETFL) | O_NONBLOCK) <
                  0) {
                if (mlog != NULL)
                  fprintf(mlog,
                          "Could not put the socket in non-blocking mode\n");
              }
            }

            FD_SET(new_sd, &master_set);
            if (new_sd > max_sd)
              max_sd = new_sd; /* keep track of the max */

            ceiling++;
            if (ceiling >= L2D2.maxClientPerProcess) {
              time(&now);
              /* wait SPAWNING_DELAY_TIME secondes btw sending of signals to
                 main server,this is for signal flood control Note : Only
                 Eternal worker is able to send signal for adding workers */
              if ((delay = difftime(now, sig_sent)) >= SPAWNING_DELAY_TIME) {
                ...
                if (tworker == ETERNAL) {
                  if (mlog != NULL)
                    fprintf(
                        mlog,
                        "Signal sent at:%s to main server to add a worker\n",
                        Stime);
        ...
        /* This is not the listening socket, therefore an
          ...
          do {
            ...
            if (rc < 0) {
              if (errno != EWOULDBLOCK) {
                get_time(Stime, 3);
                if (mlog != NULL)
                  logZone(L2D2.dzone, L2D2.dzone, mlog,
                          "recv() failed with Host:%s AT:%s Exp=%s Node:%s "
                          "Signal:%s\n",
                          l2d2client[i].host, Stime, l2d2client[i].xp,
                          l2d2client[i].node, l2d2client[i].signal);
                close_conn = TRUE;
              }
              break;
            }
            ...
            if (rc == 0) {
              get_time(Stime, 3);
              if (mlog != NULL)
                logZone(_ZONE_, L2D2.dzone, mlog, "%s Closed:%s TrNum=%u\n",
            ...
            case 'I': /* accept session */
              ...
              ret = sscanf(&buff[2], "%d %s %s %s %s %s %s %s", &pidSent,
                           expInode, expName, node, signal, hostname, username,
                           m5);
              get_time(Stime, 3);
              if (ret != 8) {
                send_reply(i, 1);
                if (mlog != NULL)
                  fprintf(mlog,
                          "Got wrong number of parameters at LOGIN, number=%d "
                          "instead of 8 buff=>%s<\n",
                          ret, buff);

                ret = shutdown(i, SHUT_WR);
                ceiling--;
                snprintf(l2d2client[i].Open_str, sizeof(l2d2client[i].Open_str),
                         "Session Refused with Host:%s AT:%s Exp=%s Node=%s "
                         "Signal=%s ... Wrong number of arguments ",
                         hostname, Stime, expName, node, signal);
              } else if (pidTken == pidSent && strcmp(m5, L2D2.m5sum) == 0) {
                send_reply(i, 0);
                snprintf(
                    l2d2client[i].Open_str, sizeof(l2d2client[i].Open_str),
                    "OpenConHost:%s At:%s Xp=%s Node=%s Signal=%s NumCon=%d ",
                    hostname, Stime, expName, node, signal, ceiling);
              } else {
                send_reply(i, 1);
                ...
                snprintf(l2d2client[i].Open_str, sizeof(l2d2client[i].Open_str),
                         "Session Refused with Host:%s AT:%s Exp=%s Node=%s "
                         "Signal=%s pid_svr=%u pid_sent=%u m5_client=%s ",
                         hostname, Stime, expName, node, signal, pidTken,
                         pidSent, m5);
              }
            ...
            case 'K': /* write Inter user dep file : Not used */
              break;
            case 'L': /* Log the node under the proper experiment ie: grab a
                         lock on a file  */
              /* Generate a key based on xp's Inode & user id. Same for all
                 process ,we are not using ftok Note : For this release we
                 discard using the datestamp in generating the key */

              memset(buf, '\0', sizeof(buf));

              /* regexp for checking existence of TRansient Workers . If there
                 are any , we must use the
                 locking mechanism. if none (only the Eternel worker write
                 directly in nodelog file */
              snprintf(buf, sizeof(buf), "%s/TRW_*", L2D2.tmpdir);
              g_result = glob(buf, 0, NULL, &g_AliveFiles);

              if (tworker == TRANSIENT ||
                  (g_result == 0 && g_AliveFiles.gl_pathc > 0)) {

                globfree(&g_AliveFiles);
                log_key = (getuid() & 0xff) << 24 | (atoi(expInode) & 0xffff)
                                                        << 16;
                memset(buf, '\0', sizeof(buf));
                snprintf(buf, sizeof(buf), "%s/NodeLogLock_0x%x", L2D2.tmpdir,
                         log_key);
                /* Open a file descriptor to the file.  what if +1 open at the
                 * same time? */
                ...
                /* NOTE: F_SETLKW -> fnc suspend until lock be placed,
                         F_SETLK  -> fnc returns immedialtly if lock cannot be
                   placed */
                got_lock = FALSE;
                ...
                if (got_lock == FALSE) {
                  ...
                  if (mlog != NULL)
                    fprintf(mlog,
                            "ERROR Cannot get lock, owned by pid:%d exp=%s "
                            "node=%s\n",
            ...
            case 'S': /* Client has sent a Stop. OK Close this connection
                     Note: if we use close(i) here , the rcv() call will fail,
                     we need to close the server write side and let the client
            ...
            default:
              if (mlog != NULL)
                fprintf(mlog,
                        "Unrecognized Token>%s< from Host=%s Exp=%s node=%s "
                        "signal=%s \n",
                        buff, l2d2client[i].host, l2d2client[i].xp,
                        l2d2client[i].node, l2d2client[i].signal);
              send_reply(i, 1);
              break;
            }
          ...
          if (close_conn) {
            ...
            memset(l2d2client[i].Open_str, '\0',
                   sizeof(l2d2client[i].Open_str));
            memset(l2d2client[i].Close_str, '\0',
                   sizeof(l2d2client[i].Close_str));
            ceiling--;
          }
...
static void maestro_l2d2_main_process_server(int fserver) {
  ...
  if (sigaction(SIGUSR1, &adm, NULL) != 0)
    fprintf(smlog, "ERROR in sigaction signal :USR1");
  if (sigaction(SIGUSR2, &adm, NULL) != 0)
    fprintf(smlog, "ERROR in sigaction signal :USR2");
  if (sigaction(SIGHUP, &adm, NULL) != 0)
    fprintf(smlog, "ERROR in sigaction signal :HUP");
  ...
  if ((pid_eworker = fork()) == 0) {
    ...
    fprintf(smlog,
            "Failed to fork the First Eternal Worker ... mserver exiting\n");
    close(fserver);
  ...
  snprintf(isAliveFile, sizeof(isAliveFile), "%s/CTR_%d", L2D2.tmpdir,
           getpid());
  ...
  for (;;) {
    ...
    if (sig_admin_AddWorker == 1) {
      ...
      if (ProcessCount < L2D2.maxNumOfProcess && ProcessCount < MAX_PROCESS) {
        if ((ChildPids[ProcessCount] = fork()) == 0) {
          ...
          fprintf(
              smlog,
              "One worker generated with pid=%u at:%s Actual ProcessCount=%d\n",
    ...
    if (sig_child == 1) {
      ...
      if ((ret = kill(pid_eworker, 0)) != 0) {
        ...
        if (ew_regenerated == 3) {
          fprintf(smlog,
                  "Eternal Worker has been Re-generated 2 times already .. "
                  "exiting at :%s\n",
                  Time);
          close(fserver);
          for (j = 0; j < MAX_PROCESS; j++) {
            ChildPids[j] == kill(ChildPids[j], 9);
          }
          kill(L2D2.depProcPid, 9);
          /* send email notice */
          snprintf(message, sizeof(message),
                   ...
                   L2D2.pid, Time);
          ret =
              sendmail(L2D2.emailTO, L2D2.emailTO, L2D2.emailCC,
                       "maestro server (mserver) failure", &message[0], smlog);
          l2d2server_shutdown(pid_eworker, smlog);
        } else {
          get_time(Time, 2);
          fprintf(smlog,
                  "Eternal Worker is dead (pid=%d) ... trying to spawn a new "
                  "one (count=%d) at:%s\n",
                  pid_eworker, ew_regenerated, Time);
          snprintf(filename, sizeof(filename), "%s/EW_%d", L2D2.tmpdir,
                   pid_eworker);
          ret = unlink(filename);
          if ((pid_eworker = fork()) == 0) {
            fclose(smlog);
      ...
      if ((ret = kill(L2D2.depProcPid, 0)) != 0) {
        ...
        if (dm_regenerated == 2) {
          fprintf(smlog,
                  "Dependency manager has been Re-generated once already .. "
                  "exiting at:%s\n",
                  Time);
          kill(pid_eworker, 9);
          for (j = 0; j < MAX_PROCESS; j++) {
          ...
          fprintf(smlog,
                  "Dependency Manager is dead (pid=%d) ... trying to spawn a "
                  "new one at:%s\n",
                  L2D2.depProcPid, Time);
          snprintf(filename, sizeof(filename), "%s/DM_%d", L2D2.tmpdir,
                   L2D2.depProcPid);
          ret = unlink(filename);
      ...
      for (j = 0; j < MAX_PROCESS; j++) {
        if (ChildPids[j] != 0 && (ret = kill(ChildPids[j], 0)) != 0) {
          ...
          fprintf(
              smlog,
              "Worker process pid:%u has terminated time=%s ProcessCount=%d\n",
    ...
    /* Heartbeat:
       both of [E|T]Worker and DManager update a file on tmpdir, This section
       examine the modif time of the file to see if processes are still alive,
       this redundency is added in case we miss signals Note : the Files TW_*
       (transient worker) are not examined */

    current_epoch = time(NULL);
    /* Eternal worker       */
    snprintf(filename, sizeof(filename), "%s/EW_%d", L2D2.tmpdir, pid_eworker);
    if (stat(filename, &st) == 0) {
      if ((diff_t = abs(difftime(current_epoch, st.st_mtime))) >
          NOTIF_TIME_INTVAL_EW) {
        if ((ret = kill(pid_eworker, 0)) != 0) {
          sig_child = 1;
          ret = unlink(filename);
    ...
    snprintf(authorization_file, sizeof(authorization_file),
             ".maestro_server_%s", L2D2.mversion);
    Auth_token =
        get_Authorization(authorization_file, passwdEnt->pw_name, &m5sum);
    if (Auth_token != NULL) {
      if (strcmp(m5sum, L2D2.m5sum) != 0) {
        get_time(Time, 1);
        fprintf(smlog,
                "mserver::Error md5sum has changed File=%s new=%s old=%s ... "
                "at:%s killing server\n",
    ...
    if (L2D2.clean_times.clean_flag == 1 &&
        (diff_t = difftime(current_epoch, epoch_cln)) >= 3600) {
      ...
      if (strncmp(tlog, Time, 8) != 0) {
        ...
        fclose(smlog);
        snprintf(filename, sizeof(filename), "%s_%.8s_%d", L2D2.mlog, Time,
                 L2D2.pid);
        if ((smlog = fopen(filename, "w+")) == NULL) {
          /* write somewhere */
          exit(1); /* could we have another alt. */
...
int main(int argc, char *argv[]) {
  ...
  // mserver has no arguments, so if some are given, assume it is -h or --help
  // or the user needs a usage page.
  if (argc > 1) {
    show_usage();
    exit(1);
  }
  ...
  snprintf(buf, sizeof(buf), "%s/.suites/maestrod/dependencies/polling/v%s",
           passwdEnt->pw_dir, L2D2.mversion);
  strcpy(L2D2.dependencyPollDir, buf);
  if (access(buf, R_OK) != 0) {
    if ((status = r_mkdir(buf, 1, stderr)) != 0) {
      fprintf(stderr,
              "maestro_server(),Could not create dependencies directory:%s\n",
              buf);
    }
  }

  /* get authorization param */
  snprintf(authorization_file, sizeof(authorization_file), ".maestro_server_%s",
           L2D2.mversion);
  snprintf(L2D2.auth, sizeof(L2D2.auth), "%s/.suites/%s", passwdEnt->pw_dir,
           authorization_file);
  Auth_token =
      get_Authorization(authorization_file, passwdEnt->pw_name, &m5sum);
  if (Auth_token != NULL) {
    fprintf(stderr,
            "maestro_server(): Found mserver process information file %s: "
            "removing and replacing with new one.\n",
            L2D2.auth);
  ...
  if (status != 0) {
    fprintf(stderr,
            "maestro_server(),could not change directory to / ... exiting");
    exit(1);
  }
  ...
  /* parent writes host:port and exits after disconnecting stdin, stdout, and
   * stderr */
  ...
  if ((ret = r_mkdir(buf, 1, stderr)) != 0) {
    fprintf(stderr,
            "Cannot create server working tmpdir directory:%s ... exiting\n",
            buf);
    exit(1);
  }
  ...
  set_Authorization(mypid, hostname, ip, server_port, authorization_file,
                    passwdEnt->pw_name, &L2D2.m5sum);
  Auth_token =
      get_Authorization(authorization_file, passwdEnt->pw_name, &m5sum);
  sscanf(Auth_token, "seqpid=%u seqhost=%s seqip=%s seqport=%d", &pidTken,
         hostTken, ipTken, &portTken);
  snprintf(buf, sizeof(buf), "%s/.suites/.maestro_server_%s", passwdEnt->pw_dir,
           L2D2.mversion);
  ...
  if (sigaction(SIGCHLD, &chd, 0) != 0)
    fprintf(stderr, "ERROR sigaction for SIGCHLD");
