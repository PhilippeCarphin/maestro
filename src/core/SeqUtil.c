...
void SeqUtil_setTraceEnv() {
  ...
  if (seq_trace_level == NULL) {
    SeqUtil_TRACE(TL_FULL_TRACE, "SeqUtil_setTraceEnv(): Unable to find "
                                 "SEQ_TRACE_LEVEL environment variable \n");
    return;
  }
 ...
 *******************************************************************************/
int SeqUtil_getTraceLevel() { return traceFlags.importance; }
...
int touch_nfs(const char *filename, const char *_seq_exp_home) {
  ...
  if ((actionfile = fopen(filename, "r")) != NULL) {
    fclose(actionfile);
    utime(filename,
          NULL); /*set the access and modification times to current time */
  } else {
...
int isFileExists_nfs(const char *lockfile, const char *caller,
  ...
  if (access(lockfile, R_OK) == 0) {
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "SeqUtil.isFileExists_nfs() caller:%s found lock file=%s\n",
                  caller, lockfile);
    return 1;
  }
  SeqUtil_TRACE(TL_MEDIUM,
                "SeqUtil.isFileExists_nfs() caller:%s missing lock file=%s\n",
                caller, lockfile);
  return 0;
}
 ...
 * pathnames matching
 */
int globPath_nfs(const char *pattern, int flags,
  ...
  case GLOB_NOSPACE:
    SeqUtil_TRACE(TL_ERROR,
                  "SeqUtil.globPath_nfs() Glob running out of memory \n");
    return (0);
    break;
  case GLOB_ABORTED:
...
/*
 * nfs Wrapper to glob function to return list of extensions found by pattern
 * with a * wildcard.
 */
LISTNODEPTR globExtList_nfs(const char *pattern, int flags,
                            int (*errfunc)(const char *epath, int eerrno)) {
  ...
  SeqUtil_TRACE(TL_FULL_TRACE,
                "SeqUtil.globExtList_nfs() looking for pattern %s \n", pattern);

  wildcardPtr = strchr(pattern, '*');
  if (wildcardPtr == NULL)
    return NULL;
  /* location used to know where the new patterns will start in the glob return
   * strings */
  ...
  case GLOB_NOSPACE:
    SeqUtil_TRACE(TL_MEDIUM,
                  "SeqUtil.globExtList_nfs() Glob running out of memory \n");
    return (0);
    break;
  case GLOB_ABORTED:
  ...
  case GLOB_NOMATCH:
    SeqUtil_TRACE(TL_MEDIUM,
                  "SeqUtil.globExtList_nfs() Glob no found matches \n");
    globfree(&glob_p);
    return (0);
    break;
  }
  ...
  for (filecounter = 0; filecounter < totalfiles; ++filecounter) {
    /*file return format should be /path/to/files/filename.*.some_state, and *
     * should replace a "+index" where index can be a string or a number */

    SeqUtil_TRACE(
        TL_FULL_TRACE,
        "SeqUtil.globExtList_nfs() iteration found, Target: %s (length=%d); "
        "wildcardLocation=%d, wildcardPtr=%s (length %d); \n",
        glob_p.gl_pathv[filecounter], strlen(glob_p.gl_pathv[filecounter]),
        wildcardLocation, wildcardPtr, strlen(wildcardPtr));

    tmpString =
        strndup(glob_p.gl_pathv[filecounter] + wildcardLocation,
                strlen(glob_p.gl_pathv[filecounter]) - strlen(pattern) + 1);
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "SeqUtil.globExtList_nfs() iteration found. Extension: %s \n",
                  tmpString);
...
char *SeqUtil_cpuCalculate(const char *npex, const char *npey, const char *omp,
                           const char *cpu_multiplier) {
  char *chreturn = NULL;
  int nMpi = 1;
  if (!(chreturn = malloc(strlen(npex) + (npey == NULL || strlen(npey)) +
                          (omp == NULL || strlen(omp)) +
                          strlen(cpu_multiplier) + 1))) {
  ...
  if (omp != NULL) {
    sprintf(chreturn, "%sx%s", chreturn, omp);
  }
  return (chreturn);
}

char *SeqUtil_cpuCalculate_new(const char *npex, const char *npey,
                               const char *omp, const char *cpu_multiplier) {
  char *chreturn = NULL;
  int nMpi = 1, totalsize = 0;

  if (npex != NULL)
    totalsize += strlen(npex);
  if (npey != NULL)
    totalsize += strlen(npey);
  if (omp != NULL)
    totalsize += strlen(omp);
  if (cpu_multiplier != NULL)
    totalsize += strlen(omp);

  if (!(chreturn = malloc(totalsize + 1))) {
    SeqUtil_TRACE(TL_CRITICAL,
                  "SeqUtil_cpuCalculate(): malloc: Out of memory!\n");
    return (NULL);
  }
  nMpi = atoi(npex) * atoi(cpu_multiplier);
  if (npey != NULL) {
  ...
  if (data != NULL) {
    if (*source != NULL) {
      if (!(newDataPtr = malloc(strlen(*source) + strlen(data) + 1))) {
        SeqUtil_TRACE(TL_CRITICAL,
                      "SeqUtil_stringAppend malloc: Out of memory!\n");
        return;
      }
...
char *SeqUtil_resub(const char *regex_text, const char *repl_text,
                    const char *str) {
...
/* waits for the presence of a file (checking every timeInterval) or max time
 * elapses */
...
char *SeqUtil_getdef(const char *filename, const char *key,
                     const char *_seq_exp_home) {
  char *retval = NULL, *home = NULL, *ovpath = NULL,
       *ovext = "/.suites/overrides.def", *defpath = NULL,
       *defext = "/.suites/default_resources.def";
  ...
  if (passwdEnt == NULL)
    raiseError("SeqUtil_getdef err:%s while getting HOME value for uid: %lu\n",
  ...
  SeqUtil_TRACE(TL_FULL_TRACE,
                "SeqUtil_getdef(): looking for definition of %s in %s\n", key,
                ovpath);
 ...
 ********************************************************************************/
int SeqUtil_getmappedfile(const char *filename, char **filestart,
                          char **fileend) {
  ...
  for (i = 0; i < nbMappedFiles; ++i) {
    ...
    if (strcmp(mappedFiles[i].filename, filename) == 0) {
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "getmappedfile(): File %s was found in mmapped files\n",
                    filename);
  ...
  if (fd == -1) {
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "SeqUtil_getmappedfile(): Could not open file %s for "
                  "definition lookup \n",
                  filename);
    return 1;
  }
  ...
  if (fstat(fd, &fileStat) == -1) {
    SeqUtil_TRACE(TL_ERROR,
                  "SeqUtil_getmappedfile(): Error getting stats for file %s \n",
                  filename);
    return 1;
  }
  ...
  if (addr == MAP_FAILED) {
    SeqUtil_TRACE(
        TL_ERROR,
        "SeqUtil_getmappedfile(): Error mapping file %s into memory \n",
        filename);
    return 1;
  }
  SeqUtil_TRACE(
      TL_FULL_TRACE,
      "getmappedfile(): Mapped file %s into memory using file descriptor %d \n",
      filename, fd);
...
void SeqUtil_unmapfiles() {
  ...
  while (nbMappedFiles-- > 0) {
    filesize = mappedFiles[nbMappedFiles].fileend -
               mappedFiles[nbMappedFiles].filestart;
...
int readline(char *line, int size, char *source) {
  ...
  while ((c = source[i]) != '\n' && c != '\0') {
    ...
    if (i >= size) {
      raiseError(
          "readline(): Buffer is too small to receive line from source\n");
      return -1;
    }
  }
  line[i] = '\0';
...
char *SeqUtil_parsedef(const char *filename, const char *key) {
  ...
  if (status == 1) {
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "SeqUtil_parsdef failed to open/map file %s \n", filename);
    return NULL;
  }

  char *current = filestart;
  /* long int to help the compiler because we will be adding it to a 64 bit
   * pointer */
  long int readsize = 0;
  ...
  do {
    ...
    if (sscanf(line, " %[^= \t] = %[^\n \t]", defkey, defval) == 2) {
      if (strcmp(key, defkey) == 0) {
        ...
        SeqUtil_TRACE(TL_FULL_TRACE,
                      "SeqUtil_parsedef(): found definition %s=%s in %s\n",
...
/* Substitutes a ${.} formatted keyword in a string. To use a definition file
   (format defined by SeqUtils_getdef(), provide the _deffile name; a NULL value
   passed to _deffile causes the resolver to search in the environment for the
   key definition.  If
   _srcfile is NULL, information about the str source is not printed in case of
   an error.*/
char *SeqUtil_keysub(const char *_str, const char *_deffile,
                     const char *_srcfile, const char *_seq_exp_home) {
  char *strtmp = NULL, *substr = NULL, *var_name = NULL, *var_value = NULL,
       *post = NULL, *source = NULL;
  ...
  SeqUtil_TRACE(
      TL_FULL_TRACE,
      "XmlUtils_resolve(): performing %s replacements in string \"%s\"\n",
      source, _str);
  ...
  while (substr != NULL) {
    ...
    if (var_value == NULL) {
      ...
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "XmlUtils_resolve(): replacing %s with %s value \"%s\"\n",
...
/*removes first occurance of the substring from a string, modifies the string
 * argument */
void SeqUtil_stripSubstring(char **string, char *substring) {
  ...
  SeqUtil_TRACE(TL_FULL_TRACE,
                "SeqUtil_stripSubstring(): resulting string %s \n", string);
}

/* returns an memory allocated string of a path whose ".", ".." and "..."
 * keywords have been replaced with the following . -> current container path
...
char *SeqUtil_relativePathEvaluation(char *path, SeqNodeDataPtr _nodeDataPtr) {
  ...
  char *returnString = NULL, *tmpString = NULL, *tmpstrtok = NULL,
       *prevPtr = NULL, *newString = NULL;
  ...
  if (strstr(path, "..") != NULL || strstr(path, "./") != NULL) {
    ...
    if ((tmpString = strstr(path, "...")) != NULL) {
      if (!(returnString = malloc(strlen(path) - 3 +
                                  strlen(_nodeDataPtr->pathToModule) + 1))) {
      ...
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "SeqUtil_relativePathEvaluation(): module keyword "
                    "replacement: replacing %s with %s\n",
                    path, returnString);
      if (strstr(returnString, "...") != NULL) {
        raiseError("SeqUtil_relativePathEvaluation(): \"...\" keyword should "
                   "only occur once. Not permitted to go beyond a single "
                   "module level. Check your dep_name = %s",
                   path);
      }
      ...
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "SeqUtil_relativePathEvaluation(): parent keyword "
                    "replacement: replacing %s with %s\n",
                    path, returnString);
      free(tmpString);
    } else if ((tmpString = strstr(path, "./")) != NULL) {

      if (!(returnString = malloc(strlen(path) - 1 +
                                  strlen(_nodeDataPtr->container) + 1))) {
        raiseError("SeqUtil_stripSubstring malloc: Out of memory!\n");
      }
      strcpy(returnString, _nodeDataPtr->container);
      strcat(returnString, path + 1);
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "SeqUtil_relativePathEvaluation(): container replacement: "
                    "replacing %s with %s\n",
                    path, returnString);

      if (strstr(returnString, "./") != NULL) {
        raiseError("SeqUtil_relativePathEvaluation(): \"./\" keyword should "
                   "only occur once. Check your dep_name = %s",
                   path);
      }
    }

  } else {
    /* doesn't contain keywords, just create a copy of the input string and
     * return */
 ...
 * Writes (nfs) the dependency lockfile in the directory of the node that this
 * current node is waiting for.
 *
 * Inputs:
...
int WriteNodeWaitedFile_nfs(const char *seq_xp_home, const char *nname,
  ...
  fprintf(
      stderr,
      "maestro.writeNodeWaitedFile(): Using WriteNodeWaitedFile_nfs routine\n");
  ...
  /* be carfull here, argument a will position both read and write pointer at
     the end, while a+ will position read at beginning and write at the end.
     NOTE: the file is open through NFS from different machines, we dont know
     the exact behaviour of the append command */
  ...
  while (fgets(line, SEQ_MAXFIELD, waitingFile) != NULL) {
    ...
    if ((Linode = get_Inode(Lexp)) < 0) {
      fprintf(stderr,
              "writeNodeWaitedFile_nfs: Cannot get Inode of registred xp=%s\n",
              Lexp);
      continue;
    }
    if (Linode == inode && strcmp(Lnode, nname) == 0 &&
        strcmp(Ldatestamp, datestamp) == 0 &&
        strcmp(LloopArgs, loopArgs) == 0) {
      found = 1;
      break;
    }
  }
  ...
  if (!found) {
    snprintf(tmp_line, sizeof(tmp_line),
             "exp=%s node=%s datestamp=%s args=%s\n", seq_xp_home, nname,
             datestamp, loopArgs);

    num = fwrite(tmp_line, sizeof(char), strlen(tmp_line), waitingFile);
    if (num != strlen(tmp_line))
      fprintf(stderr, "writeNodeWaitFile Error: written:%zu out of:%zd \n", num,
              strlen(tmp_line));
  }
...
int WriteInterUserDepFile_nfs(const char *filename, const char *DepBuf,
                              const char *ppwdir, const char *maestro_version,
   ...
   * Note: multiple client could try to create this when  do not exist. Give a
   * static inode for this action */
  snprintf(buff, sizeof(buff), "%s/.suites/maestrod/dependencies/polling/v%s",
           ppwdir, maestro_version);

  if (access(buff, R_OK) != 0) {
    if (SeqUtil_mkdir_nfs(buff, 1, NULL) != 0) {
      raiseError(
          "WriteInterUserDepFile_nfs: Could not create dependency directory:%s",
          buff);
    }
  }

  /* build dependency filename and link it to true dependency file under the xp.
   * tree */
  snprintf(DepFileName, sizeof(DepFileName),
           "%s/.suites/maestrod/dependencies/polling/v%s/%s_%s", ppwdir,
 ...
 * Writes (nfs) the for each lockfile in the directory of the node that this
 * current node is waiting for.
 *
 * Inputs:
 ...
 *    _target_index       - the loop index name expected to be filled up by FE
 * target _loopArgs          - the name=value,name=value string of loop
 * arguments belonging to the forEach node. _filename          - filename where
 * info will be located Output: int status file created in target area
 */

int WriteForEachFile_nfs(const char *_exp, const char *_node,
  ...
  SeqUtil_TRACE(
      TL_FULL_TRACE,
      "WriteNodeWaitedFile(): Using WriteForEachFileFile_nfs routine\n");
  memset(tmp_line, '\0', sizeof(tmp_line));
  memset(line, '\0', sizeof(line));
  memset(Lexp, '\0', sizeof(Lexp));
  memset(Lnode, '\0', sizeof(Lnode));
  ...
  while (fgets(line, SEQ_MAXFIELD, waitingFile) != NULL) {
    ...
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "writeForEachFile_nfs checking inodes: %zu vs %zu, %s vs %s, "
                  "%s vs %s, %s vs %s, %s vs %s\n",
                  Linode, inode, Lnode, _node, Ldatestamp, _datestamp,
                  LloopArgs, _loopArgs, Lindex, _target_index);
    if (Linode == inode && strcmp(Lnode, _node) == 0 &&
        strcmp(Ldatestamp, _datestamp) == 0 &&
        strcmp(LloopArgs, _loopArgs) == 0 &&
        strcmp(Lindex, _target_index) == 0) {
      found = 1;
      break;
    }
  }
  if (!found) {
    snprintf(tmp_line, sizeof(tmp_line),
             "exp=%s node=%s datestamp=%s index_to_add=%s args=%s\n", _exp,
             _node, _datestamp, _target_index, _loopArgs);
    num = fwrite(tmp_line, sizeof(char), strlen(tmp_line), waitingFile);
    if (num != strlen(tmp_line)) {
...
int lock_nfs(const char *filename, const char *datestamp,
  ...
  if (access(lpath, R_OK) != 0)
    ret = SeqUtil_mkdir_nfs(lpath, 1, NULL);

  snprintf(src, sizeof(src), "%s/END_TASK_LOCK", lpath);
  if (access(src, R_OK) != 0) {
    if ((ret = touch_nfs(src, _seq_exp_home)) != 0)
      fprintf(stderr, "cannot touch file:lock on %s \n", lpath);
  }
...
/* Returns the average of the input array, but sorting it, and removing the
 * removal_quantity of extreme values from each side. */

int SeqUtil_basicTruncatedMean(int *unsorted_int_array, int elements,
                               int removal_quantity) {
  int total = 0;
  int i;
  if (elements <= 2 * removal_quantity)
    return 0;
...
/* taken from https://gist.github.com/starwing/2761647 . Will not remove
 * trailing slashes (so can be used in conjunction with SeqUtil_fixPath) */
/* will return a normalized path, removing relative notations and additional
 * slashes */
...
const char *SeqUtil_resourceDefFilename(const char *_seq_exp_home) {
  ...
  char *defFile = (char *)malloc(
      sizeof(char) * (strlen(_seq_exp_home) + strlen(resource_postfix) + 1));
 ...
 ********************************************************************************/
int SeqUtil_sprintStatusFile(char *dst, const char *exp_home,
                             const char *node_name, const char *datestamp,
                             const char *extension, const char *status) {

  if ((exp_home == NULL) || (datestamp == NULL) || (status) == NULL ||
      (dst == NULL)) {
    raiseError("SeqUtil_sprintStatusFile() input arguments (status, exp_home, "
               "datestamp or dst) containing NULL\n");
  }

  SeqUtil_TRACE(TL_FULL_TRACE, "SeqUtil_sprintStatusFile() begin\n");
  if (extension != NULL && strlen(extension) > 0) {
    sprintf(dst, "%s/sequencing/status/%s/%s.%s.%s", exp_home, datestamp,
            node_name, extension, status);
  } else {
    sprintf(dst, "%s/sequencing/status/%s/%s.%s", exp_home, datestamp,
            node_name, status);
  }
  SeqUtil_TRACE(TL_FULL_TRACE,
                "SeqUtil_sprintStatusFile() end constructed filename:\n   %s\n",
                dst);
 ...
 ********************************************************************************/
int SeqUtil_sprintContainerTaskPath(char *dst, const char *exp_home,
...
/********************************************************************************
 * Reference
 *http://www.microhowto.info/howto/capture_the_output_of_a_child_process_in_c.html#idp146944
...
char *SeqUtil_getScriptOutput(const char *script_path, size_t buffer_size) {
  ...
  while (1) {
    ssize_t count = read(switch_script_fd[0], &(buffer[totalCount]),
                         sizeof(buffer) - totalCount);
    if (count == -1) {
      if (errno == EINTR) {
        continue;
      } else {
 ...
 ********************************************************************************/
const char *SeqUtil_popenGetScriptOutput(const char *script_path,
                                         size_t buffer_size) {
  FILE *fp;
  int status;
  char buffer[buffer_size];
...
/*
 * Returns the environment variable's integer value if it is defined, or the
