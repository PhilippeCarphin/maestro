...
char *absolutePath(const char *relativePath) {
  ...
  char *absPath =
      (char *)malloc(strlen(testDir) + 1 + strlen(relativePath) + 1);
...
int test_xml_fallback() {
  ...
  sprintf(cmd, "rm -f %s", badFile);
  system(cmd);
  sprintf(cmd, "touch %s", badFile);
  system(cmd);
  ndp->type = Task;
