/* SeqNode.c - Node construct definitions and utility functions used by the
...
void SeqNode_setPathToModule(SeqNodeDataPtr node_ptr,
                             const char *pathToModule) {
...
void SeqNode_setIntramoduleContainer(SeqNodeDataPtr node_ptr,
                                     const char *intramodule_container) {
  if (intramodule_container != NULL) {
    free(node_ptr->intramodule_container);
    if ((node_ptr->intramodule_container =
             malloc(strlen(intramodule_container) + 1)) != NULL) {
      strcpy(node_ptr->intramodule_container, intramodule_container);
    } else {
      raiseError(
          "OutOfMemory exception in SeqNode_setIntramoduleContainer()\n");
    }
  }
}
...
void SeqNode_setCpu_new(SeqNodeDataPtr node_ptr, const char *cpu) {
  ...
  if (cpu != NULL) {
    ...
    for (x_count = 0; strPtr[x_count];
         strPtr[x_count] == 'x' ? x_count++ : *(strPtr++))
      ;

    SeqUtil_TRACE(TL_FULL_TRACE,
                  "SeqNode_setCpu() cpu=%s, x-separator count=%d\n", cpu,
                  x_count);

    switch (x_count) {
    case 0:
      if (sscanf(cpu, "%d", &value1) == 1) {
        /* 1 value matching, so value1 = OMP when not mpi, npex when mpi,
         * resetting the other value in case*/
        if (node_ptr->mpi == 0) {
          free(node_ptr->npex);
          free(node_ptr->omp);
          (node_ptr->omp = malloc(10)) != NULL
              ? snprintf(node_ptr->omp, 10, "%d", value1)
              : raiseError("OutOfMemory Exception");
          node_ptr->npex = strdup("1");
        } else {
          free(node_ptr->npex);
          free(node_ptr->omp);
          (node_ptr->npex = malloc(10)) != NULL
              ? snprintf(node_ptr->npex, 10, "%d", value1)
              : raiseError("OutOfMemory Exception");
          node_ptr->omp = strdup("1");
        }
      } else
        raiseError("Format error in cpu %s. Should be 1x1x1, 1x1 or 1.\n", cpu);
      break;
    case 1:
      if (sscanf(cpu, "%dx%d", &value1, &value2) == 2) {
        ...
        (node_ptr->npex = malloc(10)) != NULL
            ...
            : raiseError("OutOfMemory Exception");
        (node_ptr->omp = malloc(10)) != NULL
            ? snprintf(node_ptr->omp, 10, "%d", value2)
            : raiseError("OutOfMemory Exception");
      } else
        raiseError("Format error in cpu %s. Should be 1x1x1, 1x1 or 1.\n", cpu);
      break;
    case 2:
      if (sscanf(cpu, "%dx%dx%d", &value1, &value2, &value3) == 3) {
        ...
        (node_ptr->npex = malloc(10)) != NULL
            ? snprintf(node_ptr->npex, 10, "%d", value1)
            : raiseError("OutOfMemory Exception");
        (node_ptr->npey = malloc(10)) != NULL
            ? snprintf(node_ptr->npey, 10, "%d", value2)
            : raiseError("OutOfMemory Exception");
        (node_ptr->omp = malloc(10)) != NULL
            ? snprintf(node_ptr->omp, 10, "%d", value3)
            : raiseError("OutOfMemory Exception");
      } else
        raiseError("Format error in cpu %s. Should be 1x1x1, 1x1 or 1.\n", cpu);
      break;
...
void SeqNode_setCpuMultiplier(SeqNodeDataPtr node_ptr,
...
void SeqNode_setSubmitOrigin(SeqNodeDataPtr node_ptr,
                             const char *submitOrigin) {
...
/* default numerical loop with start, step, set, end or an expression with
 * START1:END1:STEP1:SET1,STARTN:ENDN:STEPN:SETN,... */

void SeqNode_addNumLoop(SeqNodeDataPtr node_ptr, char *loop_name, char *start,
                        char *step, char *set, char *end, char *expression) {
  SeqLoopsPtr loopsPtr = NULL;
  ...
  SeqUtil_TRACE(TL_FULL_TRACE,
                "SeqNode_addNumLoop() input loop_name=%s, start=%s, step=%s, "
                "set=%s, end=%s, expression=%s, \n",
                loop_name, start, step, set, end, expression);

  defFile = malloc(strlen(node_ptr->expHome) +
                   strlen("/resources/resources.def") + 1);
  if (defFile == NULL)
    raiseError("OutOfMemory exception in SeqNode_addNumLoop()\n");

  sprintf(defFile, "%s/resources/resources.def", node_ptr->expHome);

  if (strcmp(tmpExpression, "") == 0) {
    if (strstr(start, "${") != NULL) {
      if ((value = SeqUtil_keysub(start, defFile, NULL, node_ptr->expHome)) !=
          NULL) {
        strcpy(tmpStart, value);
      }
    }
    if (strstr(step, "${") != NULL) {
      if ((value = SeqUtil_keysub(step, defFile, NULL, node_ptr->expHome)) !=
          NULL) {
        strcpy(tmpStep, value);
      }
    }
    if (strstr(set, "${") != NULL) {
      if ((value = SeqUtil_keysub(set, defFile, NULL, node_ptr->expHome)) !=
          NULL) {
        strcpy(tmpSet, value);
      }
    }
    if (strstr(end, "${") != NULL) {
      if ((value = SeqUtil_keysub(end, defFile, NULL, node_ptr->expHome)) !=
          NULL) {
        strcpy(tmpEnd, value);
      }
    }
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "SeqNode_addNumLoop() resulting loop_name=%s, start=%s, "
                  "step=%s, set=%s, end=%s, \n",
    ...
    if (strstr(expression, "${") != NULL) {
      if ((value = SeqUtil_keysub(expression, defFile, NULL,
                                  node_ptr->expHome)) != NULL) {
        strcpy(tmpExpression, value);
      }
    }
    SeqUtil_TRACE(
        TL_FULL_TRACE,
        "SeqNode_addNumLoop() resulting loop_name=%s, expression:%s, \n",
        loop_name, tmpExpression);
    SeqLoops_validateNumLoopExpression(tmpExpression);
  }

  if ((loopsPtr = SeqLoops_findLoopByName(
           node_ptr->loops, SeqUtil_getPathLeaf(loop_name))) == NULL) {
...
void SeqNode_addSwitch(SeqNodeDataPtr _nodeDataPtr, const char *switchName,
                       const char *switchType, const char *returnValue) {
  SeqLoopsPtr loopsPtr = NULL;
  SeqUtil_TRACE(
      TL_FULL_TRACE,
      "SeqNode_addSwitch() switchName=%s switchType=%s returnValue=%s\n",
...
void SeqNode_addSpecificData(SeqNodeDataPtr node_ptr, const char *name,
                             const char *value) {
  char *tmp = NULL;
  int count = 0;
...
void SeqNode_setForEachTarget(SeqNodeDataPtr nodePtr, const char *t_node,
                              const char *t_index, const char *t_exp,
                              const char *t_hour) {
  ...
  if (t_node != NULL)
    nodePtr->forEachTarget->node = strdup(t_node);
  if (t_index != NULL)
    nodePtr->forEachTarget->index = strdup(t_index);
  if (t_exp != NULL)
    nodePtr->forEachTarget->exp = strdup(t_exp);
  if (t_hour != NULL)
    nodePtr->forEachTarget->hour = strdup(t_hour);
}
...
void SeqNode_printNode(SeqNodeDataPtr node_ptr, unsigned int filters,
                       const char *filename) {
  ...
  if (filters & NI_SHOW_NODE) {
    (node_ptr->extension == NULL || strlen(node_ptr->extension) == 0)
        ? SeqUtil_printOrWrite(tmpFile, "node.fullnode=%s\n", node_ptr->name)
        : SeqUtil_printOrWrite(tmpFile, "node.fullnode=%s.%s\n", node_ptr->name,
                               node_ptr->extension);
  }
  ...
  if ((filters & (NI_SHOW_ALL | NI_SHOW_TASKPATH)) &&
      (node_ptr->type == Task || node_ptr->type == NpassTask)) {
  ...
  if (filters & NI_SHOW_ALL) {
    ...
    SeqUtil_printOrWrite(tmpFile,
                         "node.flow=${SEQ_EXP_HOME}/modules/%s/flow.xml\n",
                         node_ptr->module);
  ...
  if (filters & NI_SHOW_DEP)
    SeqNode_printDependencies(node_ptr, tmpFile, 1);
...
void printField(FILE *fp, const char *prefix, const char *name,
                const char *field) {
  if (strlen(field) > 0)
...
void SeqNode_printDependencies(SeqNodeDataPtr _nodeDataPtr, FILE *tmpFile,
                               int isPrettyPrint) {
  ...
  for (; current != NULL; current = current->nextPtr) {
    if (isPrettyPrint)
      SeqUtil_printOrWrite(tmpFile, "Dependency #%d\n", count);
...
Inputs:
  ...
  filename - char * pointer to where the file must be generated, if null will be
output to stdout

*/
void SeqNode_generateConfig(const SeqNodeDataPtr _nodeDataPtr, const char *flow,
                            const char *filename) {
  char *extName = NULL;
  int stringLength = 0;

  char shortdate[11] = {'\0'};
  char *tmpdir = NULL, *loopArgs = NULL, *containerLoopArgs = NULL,
       *containerLoopExt = NULL, *tmpValue = NULL, *tmp2Value = NULL;
  FILE *tmpFile = NULL;
  ...
  if (containerLoopArgsList != NULL) {
    ...
    containerLoopExt =
        (char *)SeqLoops_getExtFromLoopArgs(containerLoopArgsList);
  }
  ...
  if (_nodeDataPtr->isLastArg) {
    ...
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "SeqLoops_GenerateConfig Found ^last argument, replacing %s "
                  "for %s for node %s \n",
