...
#include "l2d2_roxml-parse-engine.h"
...
int roxml_is_number(char *input) {
  ...
  if ((end == NULL) || (roxml_is_separator(end[0])) || (end[0] == '"') ||
      (end[0] == '\'') || (end[0] == '\0')) {
    is_number = 1;
  }
...
void ROXML_INT roxml_process_begin_node(roxml_load_ctx_t *context,
  ...
  if (context->candidat_txt) {
    ...
    if (context->empty_text_node == 0) {
      ...
      node_t *to_be_closed = roxml_create_node(position, context->src,
                                               ROXML_TXT_NODE | context->type);
      context->candidat_txt =
          roxml_append_node(context->current_node, context->candidat_txt);
  ...
  context->candidat_node =
      roxml_create_node(position, context->src, ROXML_ELM_NODE | context->type);
}
...
node_t *ROXML_INT roxml_load(node_t *current_node, FILE *file, char *buffer) {
  ...
  xpath_tok_table_t *table =
      (xpath_tok_table_t *)calloc(1, sizeof(xpath_tok_table_t));
  ...
  if (file) {
    ...
    do {
      int ret = 0;
      int chunk_len = 0;
      int_len = fread(int_buffer + circle, 1, ROXML_BULK_READ - circle, file) +
                circle;
...
node_t *ROXML_INT roxml_lookup_nsdef(node_t *nsdef, char *ns) {
  ...
  while (nsdef) {
    if (nsdef->priv &&
        strcmp(namespace, ((roxml_ns_t *)nsdef->priv)->alias) == 0) {
      break;
    }
    nsdef = nsdef->next;
  }
...
void ROXML_INT roxml_set_type(node_t *n, int type) {
  n->type &= ~(ROXML_ATTR_NODE | ROXML_ELM_NODE | ROXML_TXT_NODE |
               ROXML_CMT_NODE | ROXML_PI_NODE);
  n->type |= type;
}
...
xpath_node_t *ROXML_INT roxml_set_axes(xpath_node_t *node, char *axes,
  ...
  } else if (strncmp(ROXML_L_DESC_O_SELF, axes, strlen(ROXML_L_DESC_O_SELF)) ==
             0) {
    /* ROXML_L_DESC_O_SELF */
    node->axes = ROXML_ID_DESC_O_SELF;
...
int ROXML_INT roxml_parse_xpath(char *path, xpath_node_t **xpath, int context) {
  ...
  parser =
      roxml_append_parser_item(parser, ROXML_COND_OR, _func_xpath_condition_or);
  parser = roxml_append_parser_item(parser, ROXML_COND_AND,
                                    _func_xpath_condition_and);
  parser = roxml_append_parser_item(parser, ROXML_FUNC_POS_STR,
                                    _func_xpath_position);
  parser =
      roxml_append_parser_item(parser, ROXML_FUNC_FIRST_STR, _func_xpath_first);
  parser =
      roxml_append_parser_item(parser, ROXML_FUNC_LAST_STR, _func_xpath_last);
  parser =
      roxml_append_parser_item(parser, ROXML_FUNC_NSURI_STR, _func_xpath_nsuri);
...
int ROXML_INT roxml_validate_axes(node_t *root, node_t *candidat, node_t ***ans,
  ...
  if ((valid) && (xn->xp_cond)) {
    ...
    if (condition->func == ROXML_FUNC_STRCOMP) {
      ...
      char strval[ROXML_BASE_LEN];
      iarg1 =
          atof(roxml_get_content(candidat, strval, ROXML_BASE_LEN, &status));
      if (status >= ROXML_BASE_LEN) {
    ...
    if (roxml_add_to_pool(root, candidat, req_id)) {
      if (ans) {
        if ((*nb) >= (*max)) {
          int new_max = (*max) * 2;
          node_t **new_ans =
              roxml_malloc(sizeof(node_t *), new_max, PTR_NODE_RESULT);
...
void ROXML_INT roxml_check_node(xpath_node_t *xp, node_t *root, node_t *context,
  ...
  case ROXML_ID_CHILD: {
    ...
    while (current) {
      validate_node =
          roxml_validate_axes(root, current, ans, nb, max, xp, req_id);
      if (validate_node) {
        roxml_check_node(xp->next, root, current, ans, nb, max, ROXML_DIRECT,
                         req_id);
      }
      current = current->sibl;
    }
  ...
  case ROXML_ID_DESC_O_SELF: {
    xp = xp->next;
    validate_node =
        roxml_validate_axes(root, context, ans, nb, max, xp, req_id);
    if (validate_node) {
      roxml_check_node(xp->next, root, context, ans, nb, max, ROXML_DIRECT,
                       req_id);
    }
    roxml_check_node(xp, root, context, ans, nb, max, ROXML_DESC_ONLY, req_id);
  } break;
  ...
  case ROXML_ID_PARENT: {
    if (context->prnt) {
      validate_node =
          roxml_validate_axes(root, context->prnt, ans, nb, max, xp, req_id);
      roxml_check_node(xp->next, root, context->prnt, ans, nb, max,
                       ROXML_DIRECT, req_id);
    } else {
      validate_node = 0;
    }
  ...
  case ROXML_ID_ATTR: {
    ...
    while (attribute) {
      validate_node =
          roxml_validate_axes(root, attribute, ans, nb, max, xp, req_id);
      if (validate_node) {
        roxml_check_node(xp->next, root, context, ans, nb, max, ROXML_DIRECT,
                         req_id);
      }
  ...
  case ROXML_ID_NEXT: {
    ...
    while (current) {
      ...
      while (following) {
        validate_node =
            roxml_validate_axes(root, following, ans, nb, max, xp, req_id);
        if (validate_node) {
          roxml_check_node(xp->next, root, following, ans, nb, max,
                           ROXML_DIRECT, req_id);
        } else {
  ...
  case ROXML_ID_PREV: {
    ...
    while (current && current->prnt) {
      ...
      while (preceding != current) {
        validate_node =
            roxml_validate_axes(root, preceding, ans, nb, max, xp, req_id);
        if (validate_node) {
          roxml_check_node(xp->next, root, preceding, ans, nb, max,
                           ROXML_DIRECT, req_id);
        } else {
          xp->axes = ROXML_ID_CHILD;
  ...
  case ROXML_ID_NS: {
    validate_node =
        roxml_validate_axes(root, context->ns, ans, nb, max, xp, req_id);
    if (validate_node) {
      roxml_check_node(xp->next, root, context, ans, nb, max, ROXML_DIRECT,
                       req_id);
    }
...
node_t *ROXML_INT roxml_parent_node(node_t *parent, node_t *n, int position) {
  ...
  if (n->type & ROXML_ATTR_NODE) {
    ...
    nb = roxml_get_nodes_nb(n->prnt, ROXML_PI_NODE | ROXML_CMT_NODE |
                                         ROXML_TXT_NODE | ROXML_ELM_NODE |
                                         ROXML_DOCTYPE_NODE);
  }
...
void roxml_compute_and(node_t *root, node_t **node_set, int *count,
                       int cur_req_id, int prev_req_id) {
  int i = 0;
  for (i = 0; i < *count; i++) {
    if ((!roxml_in_pool(root, node_set[i], cur_req_id)) ||
        (!roxml_in_pool(root, node_set[i], prev_req_id))) {
      (*count)--;
...
void roxml_compute_or(node_t *root, node_t **node_set, int *count, int req_id,
                      int glob_id) {
  int i = 0;
  for (i = 0; i < *count; i++) {
...
node_t **roxml_exec_xpath(node_t *root, node_t *n, xpath_node_t *xpath,
                          int index, int *count) {
  int path_id;
  int max_answers = 1;
  int glob_id = 0;
  ...
  node_t **node_set =
      roxml_malloc(sizeof(node_t *), max_answers, PTR_NODE_RESULT);
  ...
  *count = 0;
  ...
  for (path_id = 0; path_id < index; path_id++) {
    ...
    if ((cur_xpath->rel == ROXML_OPERATOR_AND) ||
        ((next_xpath) && (next_xpath->rel == ROXML_OPERATOR_AND))) {
      ...
      roxml_check_node(cur_xpath, root, orig, &node_set, count, &max_answers,
                       ROXML_DIRECT, req_id);
