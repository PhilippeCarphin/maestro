...
void printValidityData(ValidityDataPtr val) {
  if (val->dow != NULL)
    SeqUtil_TRACE(TL_FULL_TRACE, " val->dow = %s\n", val->dow);
  if (val->hour != NULL)
    SeqUtil_TRACE(TL_FULL_TRACE, " val->hour = %s\n", val->hour);
  if (val->time_delta != NULL)
    SeqUtil_TRACE(TL_FULL_TRACE, " val->time_delta = %s\n", val->time_delta);
  if (val->valid_hour != NULL)
    SeqUtil_TRACE(TL_FULL_TRACE, " val->valid_hour = %s\n", val->valid_hour);
  if (val->valid_dow != NULL)
    SeqUtil_TRACE(TL_FULL_TRACE, " val->valid_dow = %s\n", val->valid_dow);
  if (val->local_index != NULL)
    SeqUtil_TRACE(TL_FULL_TRACE, " val->local_index = %s\n", val->local_index);
}
...
int checkValidity(SeqNodeDataPtr _nodeDataPtr, ValidityDataPtr val) {
  ...
  const char *incrementedDatestamp = SeqDatesUtil_getIncrementedDatestamp(
      _nodeDataPtr->datestamp, val->hour, val->time_delta);

  /* Check local_index */
  if (val->local_index != NULL && localArgs != NULL &&
      strcmp(val->local_index, localArgs) != 0) {
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "checkValidity(): local_args mismatch:local_index=%s, "
                  "ndp->loop_args=%s\n",
...
ResourceVisitorPtr newResourceVisitor(SeqNodeDataPtr _nodeDataPtr,
  ...
  SeqUtil_TRACE(TL_FULL_TRACE,
                "newResourceVisitor() begin, nodePath=%s expHome=%s\n",
  ...
  rv->context =
      Resource_createContext(_nodeDataPtr, rv->xmlFile, rv->defFile, nodeType);
  if (rv->context != NULL)
...
const char *xmlResourceFilename(const char *_seq_exp_home, const char *nodePath,
  ...
  size_t pathLength = strlen(_seq_exp_home) + strlen(infix) + strlen(nodePath) +
                      strlen(xml_postfix) + 1;
...
xmlXPathContextPtr Resource_createContext(SeqNodeDataPtr _nodeDataPtr,
  ...
  if (access(xmlFile, R_OK) != 0) {
    if (nodeType == Loop || nodeType == ForEach) {
      raiseError(
          "createResourceContext(): Cannot access mandatory resource file %s\n",
          xmlFile);
    } else {
      context = NULL;
      goto out;
  ...
  while (found == 0 && currentNodePtr != NULL) {
    SeqUtil_TRACE(TL_FULL_TRACE, "Resource_createContext() current node: %s\n",
                  currentNodePtr->name);
    if (strcmp((const char *)currentNodePtr->name, NODE_RES_XML_ROOT_NAME) ==
        0) {
      found = 1;
      ...
      /* Delete preceeding nodes from document so that further queries are
       * accurate */
      ...
      if ((currentNodePtr) == NULL)
        raiseError("xml file %s must contain tag %s \n", xmlFile,
                   NODE_RES_XML_ROOT_NAME);
    }
  }
...
xmlDocPtr xml_fallbackDoc(const char *xmlFile, SeqNodeType nodeType) {
  ...
  if (!pxml)
    raiseError("Permission to write in %s\n", xmlFile);
  ...
  if (xmlSize == 0) {
    ...
    const char *loop_node =
        "\t<LOOP \t<LOOP start=\"0\" set=\"1\" end=\"1\" step=\"1\"/>\n";
    SeqUtil_TRACE(
        TL_FULL_TRACE,
        "xml_fallbackDoc(): File %s is empty, writing mandatory tags\n",
        xmlFile);
...
syntax_err:
  raiseError("xml_fallbackDoc(): Syntax error in xml file %s.  Search listing "
             "for \"parser error\" \n",
             xmlFile);
  return NULL; /* In case we change raiseError for something that doesn' halt
                  the program, we should still return something. */
}
...
int Resource_parseNodeDFS(ResourceVisitorPtr rv, SeqNodeDataPtr _nodeDataPtr,
  ...
  if (rv->context != NULL) {
    retval = Resource_parseNodeDFS_internal(rv, _nodeDataPtr,
                                            rv->context->doc->children, nf, 0);
  }
...
int Resource_parseNodeDFS_internal(ResourceVisitorPtr rv,
  ...
  SeqUtil_TRACE(TL_FULL_TRACE,
                "Resource_parseNodeDFS_internal() begin, depth = %d\n", depth);
  int retval = RESOURCE_SUCCESS;
  ...
  xmlXPathObjectPtr validityResults =
      XmlUtils_getnodeset((const xmlChar *)"(child::VALIDITY)", rv->context);
  {
    for_results(valNode, validityResults) {
      ...
      if (isValid(_nodeDataPtr, valNode)) {
        retval = Resource_parseNodeDFS_internal(rv, _nodeDataPtr, valNode, nf,
                                                depth + 1);
      }
    }
  }
...
int Resource_getLoopAttributes(ResourceVisitorPtr rv,
  ...
  xmlXPathObjectPtr result =
      XmlUtils_getnodeset((const xmlChar *)"(child::LOOP/@*)", rv->context);
  if (result != NULL) {
...
/********************************************************************************
 * NodeFunction: Queries the context for the attributes of the FOR_EACH child of
 *the current node which may be the root node or a VALIDITY node.
 ********************************************************************************/
int Resource_getForEachAttributes(ResourceVisitorPtr rv,
  ...
  xmlXPathObjectPtr result =
      XmlUtils_getnodeset((const xmlChar *)"(child::FOR_EACH/@*)", rv->context);
  if (result != NULL) {
...
/********************************************************************************
 * NodeFunction: Queries the context for the attributes of the BATCH child of
 *the current node which may be the root node or a VALIDITY node.
 ********************************************************************************/
int Resource_getBatchAttributes(ResourceVisitorPtr rv,
  ...
  xmlXPathObjectPtr result =
      XmlUtils_getnodeset((const xmlChar *)"(child::BATCH/@*)", rv->context);
  if (result != NULL) {
...
int Resource_getDependencies(ResourceVisitorPtr rv,
  ...
  xmlXPathObjectPtr result =
      XmlUtils_getnodeset((const xmlChar *)"(child::DEPENDS_ON)", rv->context);
  if (result != NULL)
...
int Resource_getAbortActions(ResourceVisitorPtr rv,
  ...
  xmlXPathObjectPtr result = XmlUtils_getnodeset(
      (const xmlChar *)"(child::ABORT_ACTION/@*)", rv->context);
  if (result != NULL) {
    parseAbortActions(result, _nodeDataPtr);
    rv->abortActionFound = RESOURCE_TRUE;
  } else if (rv->defFile != NULL &&
             (abortValue =
                  SeqUtil_getdef(rv->defFile, "SEQ_DEFAULT_ABORT_ACTION",
...
int Resource_getContainerLoopAttributes(ResourceVisitorPtr rv,
  ...
  if ((result = XmlUtils_getnodeset((const xmlChar *)"(child::LOOP/@*)",
                                    rv->context)) != NULL) {
...
int Resource_getWorkerPath(ResourceVisitorPtr rv, SeqNodeDataPtr _nodeDataPtr) {
  ...
  xmlXPathObjectPtr result =
      XmlUtils_getnodeset((const xmlChar *)"(child::WORKER)", rv->context);
  if (result != NULL) {
    workerPath =
        xmlGetProp(result->nodesetval->nodeTab[0], (const xmlChar *)"path");
...
int Resource_setWorkerData(ResourceVisitorPtr rv, SeqNodeDataPtr _nodeDataPtr) {
  ...
  ResourceVisitorPtr worker_rv = newResourceVisitor(
      _nodeDataPtr, _nodeDataPtr->expHome, _nodeDataPtr->workerPath, Task);
...
int Resource_validateMachine(ResourceVisitorPtr rv,
  ...
  if (strcmp(_nodeDataPtr->machine, "") == 0) {
    /* get default machine*/
    if ((value = SeqUtil_getdef(rv->defFile, "SEQ_DEFAULT_MACHINE",
                                _nodeDataPtr->expHome)) != NULL) {
      SeqNode_setMachine(_nodeDataPtr, value);
    } else {
      raiseError(
          " ERROR: Required machine attribute of BATCH tag not found for node "
          "%s and SEQ_DEFAULT_MACHINE not found in definition file %s\n",
...
int Resource_setShell(ResourceVisitorPtr rv, SeqNodeDataPtr _nodeDataPtr) {
  ...
  /* get default shell*/
  if (rv->defFile != NULL &&
      ((shellValue = SeqUtil_getdef(rv->defFile, "SEQ_DEFAULT_SHELL",
                                    _nodeDataPtr->expHome)) != NULL)) {
...
int getNodeResources(SeqNodeDataPtr _nodeDataPtr, const char *expHome,
  ...
  ResourceVisitorPtr rv =
      newResourceVisitor(_nodeDataPtr, expHome, nodePath, _nodeDataPtr->type);
 ...
 ********************************************************************************/
void getNodeLoopContainersAttr(SeqNodeDataPtr _nodeDataPtr, const char *expHome,
                               const char *loopNodePath) {
  SeqUtil_TRACE(TL_FULL_TRACE, "getNodeLoopContainersAttr() begin\n");
  ResourceVisitorPtr rv =
      newResourceVisitor(_nodeDataPtr, expHome, loopNodePath, Loop);
...
int Resource_parseWorkerPath(const char *pathToNode, const char *_seq_exp_home,
  ...
  int retval = RESOURCE_SUCCESS;
  ResourceVisitorPtr rv =
      newResourceVisitor(_nodeDataPtr, _seq_exp_home, pathToNode, Loop);
