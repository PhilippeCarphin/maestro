...
extern void tictac_setDate(char *_expHome, char *datestamp) {
  ...
  if (tmpfromaestro == NULL) {
    if ((MLLServerConnectionFid =
             OpenConnectionToMLLServer(_expHome, job, _expHome)) < 0) {
      _touch = touch_nfs;
    } else {
      _touch = touch_svr;
    }
  ...
  SeqUtil_TRACE(TL_FULL_TRACE, "maestro.tictac_setDate() setting date to=%s\n",
                datestamp);
  fprintf(stderr, "Warning: use of tictac -s $datestamp is deprecated. Please "
                  "use SEQ_DATE environment variable or a -d $datestamp "
                  "argument to maestro or expbegin calls.\n");
  ...
  if (_touch(dateFileName, _expHome) != 0)
    raiseError("Cannot touch log file: %s\n", dateFileName);
...
extern char *tictac_getDate(char *_expHome, char *format, char *datestamp) {
  ...
  if (datestamp != NULL) {
    ...
    if (glob_logs.gl_pathc == 0) {
      SeqUtil_TRACE(TL_MEDIUM,
                    "Warning: No latest datestamp available in %s/logs. "
                    "Datestamp used is 197001010000 (epoch).\n",
                    _expHome);
  ...
  SeqUtil_TRACE(TL_FULL_TRACE,
                "tictac_getDate() checking validity of dateValue ... \n");
...
extern void checkValidDatestamp(char *datestamp) {
  ...
  if (dateLength < 8 || dateLength > 14)
    raiseError("ERROR: Datestamp must contain between 8 and 14 characters "
               "(YYYYMMDD[HHMMSS]).\n");
  ...
  if ((tmpDateString = (char *)malloc(5)) != NULL) {
    ...
    raiseError(
        "ERROR: Unable to allocate memory in tictac_checkValidDatestamp()\n");
  }
