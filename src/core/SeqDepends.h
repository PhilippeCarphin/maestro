...
typedef enum _SeqDependsType { NodeDependancy, DateDependancy } SeqDependsType;
 ...
 *            time_delta fields (formerly depDatestamp in
 *validateDependencies()). ext, local_ext: are the extensions corresponding to
 *the index and local_index fields respectively. isInScope: Is calculated in
 *processDepStatus to decide whether the dependency is on a node that exists in
 *the current context. (formerly isDepInScope in validateDependencies())
