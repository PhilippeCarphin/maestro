/* l2d2_socket.c - Network functions for server code of the Maestro sequencer
 * software package.
 */
...
char *get_Authorization(char *filename, char *username, char **m5sum) {
  ...
  if ((auth_buf = (char *)malloc(1024)) == NULL) {
    ...
    memset(auth_buf, '\0',
           1024); /* this is very important, if removed login can fail */
  }
 ...
 * file will contain the location of the mserver process for clients
 * (maestro/nodelogger) to connect to.
 */
void set_Authorization(unsigned int pid, char *hostn, char *hip, int port,
...
/**
 * accept connections on the bound server socket return socket for incoming
 * connection bind_sock_to_port must have been called before connection can be
 * accepted
 */
...
/**
 * bind an existing socket to a free port within the port range, return port
 * number existing socket usually created by get_sock_net
 */
int bind_sock_to_port(int s, int min_port, int max_port) {
  ...
  if ((min_port < 0) || (max_port < 0)) {
    fprintf(stderr,
            "Invalid port range. Min port %d. Max port %d. Must be >= 0 \n",
            min_port, max_port);
    return (-1);
  }
...
/**
 * set buffer sizes (recv and send) for a newly created socket (always returns
 * 0)
 */
int set_socket_opt(int s) {
  ...
  if (b0 != 0)
    fprintf(stderr, "Error setting SEND BUFFER: SO_SNDBUF size \n");
  ...
  if (b0 != 0)
    fprintf(stderr, "Error setting RECEIVE BUFFER: SO_RCVBUF size \n");
...
int connect_to_hostport(char *target2) {
  ...
  if (NULL == strstr(target, ":")) { /* no host specified, use local host */
    portno = target;
    if (GetHostName(buf, sizeof buf)) {
      fprintf(stderr, "Can't find hostname\n");
      return (-1);
 ...
 * We should normaly add a test on ret < 0 and  put close_conn=TRUE, but we test
 * at the beginning of the loop the client connection and we set up the variable
...
int do_Login(int sock, unsigned int pid, const char *node, const char *xpname, const char *signl,
  ...
  /* build authentication string check first length of strings, this can make
   * sscanf on server do a memory fault !*/
  if (strlen(xpname) >= 256 || strlen(node) >= 256 || strlen(signl) >= 256 ||
      strlen(host) >= 256 || strlen(username) >= 256) {
    fprintf(stderr,
            "do_Login: length of string sent to server is above value 256\n");
    return (1);
  }

  snprintf(bLogin, sizeof(bLogin), "I %u %ld %s %s %s %s %s %s", pid,
           (long)fileStat.st_ino, xpname, node, signl, host, username, *m5);
  if ((bytes_sent =
           send_socket(sock, bLogin, sizeof(bLogin),
                       SeqUtil_getEnvOrDefaultI("SEQ_TIMEOUT_CLIENT",
                                                SOCK_TIMEOUT_CLIENT))) <= 0) {
    fprintf(stderr,
            "LOGIN FAILED (Timeout sending) with %s Maestro server from "
            "host=%s node=%s signal=%s\n",
            username, host, node, signl);
    return (1);
  }

  if ((bytes_read =
           recv_socket(sock, buffer, sizeof(buffer),
                       SeqUtil_getEnvOrDefaultI("SEQ_TIMEOUT_CLIENT",
                                                SOCK_TIMEOUT_CLIENT))) <= 0) {
    fprintf(stderr,
            "LOGIN FAILED (Timeout receiving) with %s Maestro server from "
            "host=%s node=%s signal=%s\n",
            username, host, node, signl);
    return (1);
  }
  ...
  if (buffer[0] != '0')
    fprintf(
        stderr,
        "LOGIN FAILED with %s Maestro server from host=%s node=%s signal=%s\n",
        username, host, node, signl);
...
int recv_full(int sock, char *buff, int rsize) {
  ...
  while (received < rsize) {
    r = recv_socket(sock, buff + received, rsize - received,
                    SOCK_TIMEOUT_CLIENT);
    if (r <= 0) {
