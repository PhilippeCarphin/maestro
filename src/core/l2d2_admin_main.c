/* l2d2_admin.c - Server administrative tool, part of the Maestro sequencer
 * software package.
 */
...
typedef enum { DEP_KEY, DEP_EXP, DPE_EXP, DEP_ALL } DepOption;
...
void print_usage(FILE *stream, int exit_code) {
  fprintf(stream, "For complete and up to date information on this command, "
                  "see the man page by typing 'man madmin'.\n");
  exit(exit_code);
}
...
int main(int argc, char *argv[]) {
  ...
  if (Auth_token == NULL) {
    ...
    sscanf(Auth_token, "seqpid=%u seqhost=%31s seqip=%31s seqport=%u", &pid,
  ...
  */
  ...
  snprintf(depdir, sizeof(depdir), "%s/maestrod/dependencies/polling/v%s",
           buffer, mversion);

  answer =
      do_Login(sock, pid, node, exp_home, signal, passwdEnt->pw_name, &m5sum);
  ...
  case LIST_DEPENDENCIES:
    ...
    case DEP_ALL: /* insert list_ptr, DEPENDER xp, depender node, dependee xp
                     name, dependee node, depender date, dependee date, depender
                     loop args, dependee loop ars, file, link */
      for (LP = getDependencyFiles(depdir, "all", fp, "depender"); LP != NULL;
           LP = LP->next) {
        fprintf(stdout,
                "node:%s depend_on_exp:%s node:%s loop_args:%s key:%s \n",
      ...
      break;
    case DEP_EXP:
      for (LP = getDependencyFiles(depdir, xp, fp, "depender"); LP != NULL;
           LP = LP->next) {
        fprintf(stdout,
                "node=%s depend_on_exp=%s node:%s loop_args:%s key:%s\n",
                LP->snode, LP->depOnXp, LP->depOnNode, LP->depOnLargs, LP->key);
    ...
    case DEP_KEY:
      ...
      free_list(PRT_listdep);
      break;
    case DPE_EXP: /* insert list_ptr, DEPENDEE xp, depender node, dependee xp
                     name, dependee node, depender date, dependee date, depender
                     loop args, dependee loop ars, file, link */
      for (LP = getDependencyFiles(depdir, xp, fp, "dependee"); LP != NULL;
           LP = LP->next) {
  ...
  case REMOVE_DEPENDENCIES:
    ...
    case DEP_KEY: /* one dependency given by the key */
      snprintf(cmdBuf, sizeof(cmdBuf),
               "%s/.suites/maestrod/dependencies/polling/v%s/%s",
               passwdEnt->pw_dir, mversion, dpkey);
      r = readlink(cmdBuf, linkname, 1023);
      linkname[r] = '\0';
      answer = unlink(linkname);
      /* The server will remove the dangling link: we are doing this to have a
    ...
    case DEP_EXP: /* dependencies for all this xp */
      ...
      for (LP = getDependencyFiles(depdir, xp, fp, "depender"); LP != NULL;
        ...
        /* waitfile is the file in inter_depends (xml file), it is cleared by
           maestro when doing initnode. We should probably erase Lp->link , but
           we will have no listing for this. this is why we let the server
           remove (dangling link) and log action in mdpmanager_$date_$pid
        ...
        */
        ...
        if ((ret = putenv(cmdBuf)) != 0)
          fprintf(stderr,
                  "Error adding variable seq_exp_home in environement \n");
        ...
        if ((buf2 = (char *)malloc(512 * sizeof(char))) != NULL) {
          ...
          sprintf(
              buf2,
              "Inter-user dependency deleted (madmin) at:%s, will init node",
              Time);
        } else {
        ...
        /* wild card in loop ?? */
        snprintf(cmdBuf, sizeof(cmdBuf),
                 "%s >/dev/null 2>&1; export SEQ_EXP_HOME=%s; nodelogger -n %s "
                 "-s \"initnode\" %s -m \"%s\" -d %s",
                 mshortcut, LP->sxp, LP->snode, lpargs, buf2, LP->sdstmp);
        ret = system(cmdBuf);
        ...
        snprintf(cmdBuf, sizeof(cmdBuf),
                 ...
                 "SEQ_DATE=%s;maestro -s initnode -n %s %s -f stop ",
                 mshortcut, LP->sxp, LP->sdstmp, LP->snode, lpargs);
        ret = system(cmdBuf);
    ...
    case DEP_ALL:
      for (LP = getDependencyFiles(depdir, "all", fp, "depender"); LP != NULL;
           LP = LP->next) {
        fprintf(stdout,
                "Removing dependency node:%s on exp:%s node:%s loop_args:%s\n",
  ...
  case IS_ALIVE:
    ...
    if (bytes_read <= 0) {
      fprintf(stderr,
              "Could not read from the mserver. Timed out... bytes_read=%d\n",
              bytes_read);
      break;
    }
