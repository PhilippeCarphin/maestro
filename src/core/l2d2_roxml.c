...
char *ROXML_API roxml_get_name(node_t *n, char *buffer, int size) {
  ...
  if (buffer == NULL) {
    buffer = (char *)roxml_malloc(sizeof(char), strlen(tmp_name) - offset + 1,
                                  PTR_CHAR);
...
node_t *roxml_add_node(node_t *parent, int position, int type, char *name,
                       char *value) {
  int name_l = 0;
  int end_node = 0;
  int content_l = 0;
  ...
  if (type & ROXML_ATTR_NODE) {
    ...
    if (type & ROXML_NS_NODE) {
      ...
      buffer =
          (char *)malloc(sizeof(char) * (name_l + content_l + xmlns_l + 4));
  ...
  if (((type & ROXML_ELM_NODE) && content_l) || (type & ROXML_ATTR_NODE)) {
    node_t *new_txt = roxml_create_node(
        content_pos, buffer, ROXML_TXT_NODE | ROXML_PENDING | ROXML_BUFF);
  ...
  if (parent == NULL) {
    ...
    parent =
        roxml_create_node(0, NULL, ROXML_ELM_NODE | ROXML_PENDING | ROXML_BUFF);
    parent->end = 1;
