/* l2d2_Util.c - Utility functions for server code of the Maestro sequencer
 * software package.
 */
...
/**
 * Name        : removeFile
 * Removes a file, returns the removal status. This functions should be
 * deprecated.
 */
...
/**
 * Name        : CreateLock
 * Description : touch a file (calls touch) if file not there. Returns 0 if file
 * exists or was unable to be created.
 */
...
/*
 * Name        : isFileExists
 * Author      : copied form SeqUtil.c  -> deprecate and use util one if
 * existing Description : check if file exists
 */
...
/**
 * creates one or several directories levels, one at a time to deal with
 * possible race conditions Logs failures in mserver log.
 */
 ...
 */
int globPath(char *pattern, int flags,
             int (*errfunc)(const char *epath, int eerrno), FILE *mlog) {
  glob_t glob_p;
  int ret;
...
/**
 * copied from nodelogger_svr (should be removed and included if duplicate
 * exists Returns all but last element of a "/" delimited string. /path/to/leaf
 * -> /path/to
 */
...
/**
 * Name        : NodeLogr
 * Description : write in .../logs/YYYYMMDDHH0000_nodelog file, logs errors in
 * mserver log file
 */
int NodeLogr(char *nodeLogerBuffer, int pid, FILE *mlog) {
  ...
  if ((NodeLogfile = open(firsin, O_WRONLY | O_APPEND | O_CREAT, 00666)) !=
    ...
    fprintf(mlog,
            "NodeLogr: Could not Open nodelog file for Experiment:%s pid=%d "
            "logBuffer:%s\n",
 ...
 *               we examine duplicate in  wait file before inserting a
 * dependency Return value: 0 success, 1 failure
 */
int writeNodeWaitedFile(const char *string, FILE *mlog) {
  FILE *waitingFile;
  char line[1024];
  char this_line[1024];
  char statusFile[1024], waitfile[1024], exp[256], node[256], datestamp[25],
      loopArgs[128];
  ...
  n = sscanf(string,
             "sfile=%1023s wfile=%1023s exp=%255s node=%255s datestamp=%24s "
             "args=%127s",
             statusFile, waitfile, this_exp, this_node, this_datestamp,
             this_loopArgs);

  /* check if we have the right number of tokens */
  if ((n <= 4) || (n == 5 && strlen(this_loopArgs) != 0)) {
    fprintf(mlog,
            "writeNodeWaitFile: Wrong number of argument given by sscanf "
            "got:%d should be 6\n",
            n);
    return (1);
  }

  /* if could not get inode, forget about this dependency, log it in mserver
   * logs */
  if ((this_inode = get_Inode(this_exp)) < 0) {
    fprintf(mlog,
            "writeNodeWaitFile: Cannot get inode of xp, dependency not written "
            "to file string=%s\n",
            string);
    return (1);
  }
  ...
  /* be carfull here, argument a will position both read and write pointer at
     ...
     NOTE: the file is open through NFS from the same machine, probably the
     append mode will be atomic in case of concurrent append. Concurrent append
     will happen when we will have multiple workers and if in the future we have
     issues with this we could use locking (same as for Logging to nodelog
     files) to resolve this */
  if ((waitingFile = fopen(waitfile, "a+")) == NULL) {
    fprintf(mlog,
            "writeNodeWaitedFile: Cannot open file:%s in appending mode\n",
            waitfile);
    return (1);
  }

  while (fgets(line, 1024, waitingFile) != NULL) {
    n = sscanf(line, "exp=%255s node=%255s datestamp=%24s args=%127s", exp,
               node, datestamp, loopArgs);
    if ((inode = get_Inode(exp)) < 0) {
      fprintf(mlog, "writeNodeWaitFile: Cannot get inode of registred xp=%s\n",
              exp);
      continue;
    }
    if (inode == this_inode && strcmp(node, this_node) == 0 &&
        strcmp(datestamp, this_datestamp) == 0 &&
        strcmp(loopArgs, this_loopArgs) == 0) {
      found = 1;
      break;
    }
  }
  ...
  if (!found) {
    snprintf(this_line, sizeof(this_line),
             "exp=%s node=%s datestamp=%s args=%s\n", this_exp, this_node,
             this_datestamp, this_loopArgs);

    num = fwrite(this_line, sizeof(char), strlen(this_line), waitingFile);
    if (num != strlen(this_line))
      fprintf(mlog, "writeNodeWaitFile Error: written:%zu out of:%ld \n", num,
              strlen(this_line));
  }
  fclose(waitingFile);
  return (0);
...
int writeInterUserDepFile(const char *tbuffer, FILE *mlog) {
  ...
  char filename[1024], DepBuf[2048], ppwdir[1024], mversion[128], md5sum[128],
      datestamp[128];
  char *token, *saveptr1;
  char *tmpString;
  ...
  if ((fp = fopen(filename, "w")) == NULL) {
    fprintf(mlog,
            "maestro server cannot write to interUser dependency file:%s\n",
            filename);
    return (1);
  }
  ...
  if (stat(filename, &st) != 0) {
    ...
    fprintf(mlog, "size of InterUserDepFile is :%ld\n", st.st_size);
 ...
 */
char *page_start_dep = "<html>\n"
                       " <body>\n"
                       " <meta http-equiv=\"refresh\" content=\"10\">\n"
                       "  <center><b>Dependencies</b></center>\n"
                       "  <table cellpadding=\"4\" cellspacing=\"0\" "
                       "border=\"1\" width=\"100%\" bgcolor=\"#E6E6FA\">\n"
                       "   <thead>\n"
                       "    <tr>\n"
 ...
 */
char *page_end_dep = "   </tbody>\n"
                     "  </table>\n"
                     " </body>\n"
                     "</html>\n";

char *page_start_blocked = "  <table cellpadding=\"4\" cellspacing=\"0\" "
                           "border=\"1\" width=\"100%\" bgcolor=\"#E6E6FF\">\n"
                           "  <thead>\n"
                           "   <tr>\n"
                           "    <th>Host</th>\n"
                           "    <th>User</th>\n"
...
int ParseXmlConfigFile(char *filename, _l2d2server *pl2d2) {
  ...
  if ((doc = fopen(filename, "r")) == NULL) {
    fprintf(stderr,
            "Cannot Open XML Configuration file for mserver:%s ... Setting "
            "Defaults\n",
            filename);

    sprintf(pl2d2->logdir, "%s/.suites/log/v%s", getenv("HOME"),
            pl2d2->mversion);
    ...
    sprintf(pl2d2->mlog, "%s/.suites/log/v%s/mcontroller", getenv("HOME"),
            pl2d2->mversion);
    sprintf(pl2d2->dmlog, "%s/.suites/log/v%s/mdpmanager", getenv("HOME"),
            pl2d2->mversion);
    ...
    pl2d2->dzone = 0;
    fprintf(stderr,
            "Setting Defaults for maxNumOfProcess:%d maxClientPerProcess:%d\n",
  ...
  if (strcmp(bf, "mserver") == 0) {
    ...
    if (log_n != NULL && log_txt != NULL &&
        (c = roxml_get_content(log_txt, bf, sizeof(bf), &size)) != NULL &&
        size > 0) {
      ...
      snprintf(pl2d2->mlog, sizeof(pl2d2->mlog), "%s/mcontroller",
               pl2d2->logdir);
      /* set files */
      snprintf(pl2d2->dmlog, sizeof(pl2d2->dmlog), "%s/mdpmanager",
               pl2d2->logdir);
    ...
    if (web_n != NULL && web_txt != NULL &&
        (c = roxml_get_content(web_txt, bf, sizeof(bf), &size)) != NULL &&
        size > 0) {
      ...
      sprintf(pl2d2->web, "%s/public_html/v%s", getenv("HOME"),
              pl2d2->mversion);
      status = r_mkdir(pl2d2->web, 1, stderr);
      if (status != 0) {
        fprintf(stderr, "Could not create web directory=%s\n", pl2d2->web);
    ...
    if (pparam_n != NULL) {
      ...
      if (prc_n != NULL &&
          (c = roxml_get_content(prc_n, bf, sizeof(bf), &size)) != NULL &&
          size > 0) {
      ...
      if (pmc_n != NULL &&
          (c = roxml_get_content(pmc_n, bf, sizeof(bf), &size)) != NULL &&
          size > 0) {
    ...
    if (param_n != NULL) {
      ...
      if (pfq != NULL &&
          (c = roxml_get_content(pfq, bf, sizeof(bf), &size)) != NULL &&
          size > 0) {
        if ((pl2d2->pollfreq = atoi(bf)) <= 15 ||
            (pl2d2->pollfreq = atoi(bf)) > 120) {
          pl2d2->pollfreq = 30;
      ...
      if (dto != NULL &&
          (c = roxml_get_content(dto, bf, sizeof(bf), &size)) != NULL &&
          size > 0) {
        if ((pl2d2->dependencyTimeOut = atoi(bf)) <= 0 ||
            (pl2d2->dependencyTimeOut = atoi(bf)) > 168) {
          pl2d2->dependencyTimeOut = 24;
          fprintf(stderr,
                  "dependencyTimeOut invalid or too high. Forcing "
                  "dependencyTimeOut=%d\n",
                  pl2d2->dependencyTimeOut);
        } else {
          pl2d2->dependencyTimeOut = atoi(bf);
          fprintf(stderr,
                  "In xml Config file found dependency time out period=%d\n",
                  pl2d2->dependencyTimeOut);
        }
      } else {
        pl2d2->dependencyTimeOut = 24;
    ...
    if (dbz_n != NULL &&
        (c = roxml_get_content(dbz_txt, bf, sizeof(bf), &size)) != NULL &&
        size > 0) {
    ...
    if (eml_n != NULL) {
      ...
      if (emlto != NULL &&
          (c = roxml_get_content(emlto, bf, sizeof(bf), &size)) != NULL &&
          size > 0) {
      ...
      if (emlcc != NULL &&
          (c = roxml_get_content(emlcc, bf, sizeof(bf), &size)) != NULL &&
          size > 0) {
    ...
    if (cln_n != NULL) {
      ...
      if (cln_cntr != NULL &&
          (c = roxml_get_content(cln_cntr, bf, sizeof(bf), &size)) != NULL &&
          size > 0) {
      ...
      if (cln_dmgr != NULL &&
          (c = roxml_get_content(cln_dmgr, bf, sizeof(bf), &size)) != NULL &&
          size > 0) {
      ...
      if (cln_ewrk != NULL &&
          (c = roxml_get_content(cln_ewrk, bf, sizeof(bf), &size)) != NULL &&
          size > 0) {
      ...
      if (cln_twrk != NULL &&
          (c = roxml_get_content(cln_twrk, bf, sizeof(bf), &size)) != NULL &&
          size > 0) {
    ...
    if (prt_n != NULL) {
      ...
      if (prt_min != NULL &&
          (c = roxml_get_content(prt_min, bf, sizeof(bf), &size)) != NULL &&
          size > 0) {
      ...
      if (prt_max != NULL &&
          (c = roxml_get_content(prt_max, bf, sizeof(bf), &size)) != NULL &&
          size > 0) {
    ...
    fprintf(stderr,
            "Incorrect root node name in xml config file:%s ... Setting "
            "Defaults \n",
            filename);
    pl2d2->maxNumOfProcess = 4;
...
/**
 * parse dependency file (polling for the moment ), errors logged in dependency
 * manager log
 */
struct _depParameters *ParseXmlDepFile(char *filename, FILE *dmlog) {
  ...
  if ((doc = fopen(filename, "r")) == NULL) {
    fprintf(dmlog,
            "ParseXmlDepFile: Cannot Open XML Polling dependency file:%s \n",
            filename);
    return (NULL);
  }
  ...
  if (strcmp(bf, "pol") != 0) {
    fprintf(dmlog,
            "ParseXmlDepFile: Incorrect root node name in xml polling "
            "dependency file:%s\n",
            filename);
    return (NULL);
  } else if ((listParam = (struct _depParameters *)malloc(
                  sizeof(struct _depParameters))) == NULL) {
    fprintf(dmlog, "ParseXmlDepFile: Cannot malloc on heap inside "
                   "ParseXmlDepFile ... exiting \n");
    exit(1);
  }
...
/**
 * Obtain a lock on a file using a link to it as mechanism, and clears symlink
 * if it is old by x sec remove it return 0 if lock obtained 1 if not
 *
 */
  ...
  if (ret != 0) {
    if ((lstat(dest, &st)) == 0) {
      ...
      if ((diff_t = difftime(now, st.st_mtime)) > LOCK_TIME_TO_LIVE) {
        ...
        if (ret == 0)
          fprintf(mlog,
                  "symlink timeout xpn=%s node=%s Token:%s diff=%f removed\n",
                  xpn, node, md5Token, diff_t);
        else
          fprintf(mlog,
                  "symlink timeout xpn=%s node=%s Token:%s diff=%f could not "
                  "remove\n",
                  xpn, node, md5Token, diff_t);
 ...
 */
int sendmail(const char *to, const char *from, const char *cc,
             const char *subject, const char *message, FILE *mlog) {
  int retval = -1;
 ...
 */
dpnode *getDependencyFiles(char *DDep, char *xp, FILE *fp,
                           const char *deptype) {
  glob_t g_depFiles;
  size_t cnt;
  ssize_t r;
  ...
  if (g_ldp == 0 && g_depFiles.gl_pathc > 0) {
    for (p = g_depFiles.gl_pathv, cnt = g_depFiles.gl_pathc; cnt; p++, cnt--) {
      /* must be a link */
      r = readlink(*p, linkname, 1023);
      linkname[r] = '\0';
      if ((depXp = (struct _depParameters *)ParseXmlDepFile(linkname, fp)) ==
          NULL) {
...
verifying const char * state - what state are we verifying

*/
int l2d2_Util_isNodeXState(const char *node, const char *loopargs,
                           const char *datestamp, const char *exp,
                           const char *state) {
