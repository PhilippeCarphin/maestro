/* SeqLoopsUtil.c - Utility functions for loops in the Maestro sequencer
 * software package.
 */
...
static LISTNODEPTR
loopArg_to_reverse_extension_list(SeqNodeDataPtr _nodeDataPtr,
                                  SeqNameValuesPtr arg);
...
static LISTNODEPTR
SeqNodeData_to_reverse_extension_list(SeqNodeDataPtr _nodeDataPtr);
static LISTNODEPTR
numerical_to_reverse_extension_list(SeqNameValuesPtr loopData);
...
LISTNODEPTR SeqLoops_getLoopContainerExtensions(SeqNodeDataPtr _nodeDataPtr,
                                                const char *depIndex);
...
char *SeqLoops_indexToExt(const char *index) {
   ...
   *        SeqUtil_TRACE(TL_FULL_TRACE, "maestro.validateDependencies()
   * localIndex=%s\n", localIndex ); SeqLoops_parseArgs(&loopArgsPtr,
   * localIndex); tmpExt = (char*) SeqLoops_getExtFromLoopArgs(loopArgsPtr);
...
int SeqLoops_parseArgs(SeqNameValuesPtr *nameValuesPtr, const char *cmd_args) {
  ...
  while (tmpstrtok != NULL) {
    ...
    n = sscanf(tmpstrtok, " %[A-Za-z0-9._:/]=%[A-Za-z0-9._^/-$()*]", loopName,
               loopValue);
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "SeqLoops_parseArgs parsed items (count=%d): %s = %s\n", n,
                  loopName, loopValue);
...
char *SeqLoops_getExtensionBase(SeqNodeDataPtr _nodeDataPtr) {
  ...
  if (_nodeDataPtr->type == Loop || _nodeDataPtr->type == NpassTask ||
      _nodeDataPtr->type == Switch || _nodeDataPtr->type == ForEach) {
...
example:
  ...
  */
SeqNameValuesPtr SeqLoops_convertExtension(SeqLoopsPtr loops_ptr,
                                           char *extension) {
...
/* returns the value of a specific attribute from the attribute list of a loop
   item, return NULL if not found the attr_name is something like NAME, START,
   STEP, END,...
 */
char *SeqLoops_getLoopAttribute(SeqNameValuesPtr loop_attr_ptr,
                                char *attr_name) {
  char *returnValue = NULL;
  SeqNameValuesPtr tmpptr = loop_attr_ptr;

  SeqUtil_TRACE(TL_FULL_TRACE,
                "SeqLoops_getLoopAttribute looking for attribute:%s\n",
                attr_name);
  while (tmpptr != NULL) {
    if (strcmp(tmpptr->name, attr_name) == 0) {
      returnValue = strdup(tmpptr->value);
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "SeqLoops_getLoopAttribute found attribute:%s value:%s \n",
  ...
  if (returnValue == NULL) {
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "SeqLoops_getLoopAttribute(): attribute %s not found\n",
                  attr_name);
  }
  return returnValue;
}

/* changes the value of a specific attribute from the attribute list of a loop
   item, if not found, it will ADD it to the list the attr_name is something
   like NAME, START, STEP, END,...
 */
void SeqLoops_setLoopAttribute(SeqNameValuesPtr *loop_attr_ptr, char *attr_name,
                               char *attr_value) {
  int found = 0;
  ...
  while (loopAttrPtr != NULL) {
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "SeqLoops_setLoopAttribute looking for:%s found:%s \n",
...
*/
char *SeqLoops_ContainerExtension(SeqLoopsPtr loops_ptr,
                                  SeqNameValuesPtr loop_args_ptr) {
  ...
  while (loops_ptr != NULL && isError == 0) {
    ...
    /* inside the SeqLoopsPtr, the loop_name value is stored with the full path
       node of the loop while the loop_name function argument is only the leaf
       part */
    loopLeafName = (char *)SeqUtil_getPathLeaf(loopContainerName);
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "SeqLoops_ContainerExtension loopLeafName=%s\n",
                  loopLeafName);
    thisLoopArgPtr = loop_args_ptr;
    while (thisLoopArgPtr != NULL && isError == 0) {
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "SeqLoops_ContainerExtension loop_name=%s loop_value=%s\n",
      ...
      if (strcmp(thisLoopArgPtr->name, loopLeafName) == 0) {
        ...
        SeqUtil_TRACE(TL_FULL_TRACE,
                      "SeqLoops_ContainerExtension found loop argument "
                      "loop_name=%s loop_value=%s\n",
                      thisLoopArgPtr->name, thisLoopArgPtr->value);
        break;
      }
...
*/
char *SeqLoops_NodeExtension(const char *node_name,
                             SeqNameValuesPtr loop_args_ptr) {
...
LISTNODEPTR SeqLoops_childExtensionsInReverse(SeqNodeDataPtr _nodeDataPtr) {
  ...
  if (tmpExpression != NULL && strcmp(tmpExpression, "") != 0) {
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "SeqLoops_childExtensionsInReverse expression: %s\n",
                  tmpExpression);
    ...
    if ((tmpAttributeValue = SeqLoops_getLoopAttribute(nodeSpecPtr, "START")) !=
        NULL) {
      loopStart = atoi(tmpAttributeValue);
      free(tmpAttributeValue);
    }
    if ((tmpAttributeValue = SeqLoops_getLoopAttribute(nodeSpecPtr, "STEP")) !=
        NULL) {
      loopStep = atoi(tmpAttributeValue);
      free(tmpAttributeValue);
    }
    if ((tmpAttributeValue = SeqLoops_getLoopAttribute(nodeSpecPtr, "END")) !=
        NULL) {
    ...
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "SeqLoops_childExtensionsInReverse extension:%s "
                  "baseExtension:%s START:%d END:%d\n",
...
/* return the loop container extension values that the current node is in. (ex:
 +3+1 -> +3+2 -> +3+3)
 * Current node must be a loop node.
   To be used to check if a loop parent node is done,
   assuming that extension is set in  _nodeDataPtr->extension */
LISTNODEPTR SeqLoops_childExtensions(SeqNodeDataPtr _nodeDataPtr) {
  SeqNameValuesPtr nodeSpecPtr = NULL;
  char tmp[20], *baseExtension;
  int expressionArray[256], i, startIndex = 0, endIndex = 1, stepIndex = 2,
                               detectedEnd = 0;
  char *tmpExpression = NULL, *tmpArrayValue = NULL, *tmpAttributeValue = NULL;
  int loopStart = 0, loopStep = 1, loopEnd = 0, loopCount = 0, breakFlag,
      firstFlag = 1;
  ...
  if ((tmpExpression != NULL && strcmp(tmpExpression, "") != 0) &&
      ((SeqLoops_getLoopAttribute(nodeSpecPtr, "START") != NULL) ||
    ...
    raiseError(
        "SeqLoops_childExtensions Error: loop syntax must be defined with "
        "start, end, step, step attributes OR with an expression attribute\n");
  }
  ...
  if (tmpExpression != NULL && strcmp(tmpExpression, "") != 0) {
    ...
    while (loopCount <= detectedEnd) {
      ...
      while (loopCount <= expressionArray[endIndex] && breakFlag == 0) {
        if (firstFlag == 0) {
          if ((loopCount + expressionArray[stepIndex]) <=
              expressionArray[endIndex]) {
    ...
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "SeqLoops_childExtensions extension:%s baseExtension:%s\n",
 ...
 * { +B+X, +A+X } x { +E, +D, +C} = { +B+X+E, +B+X+D, +B+X+C, +A+X+E, +A+X+D,
 *+A+X+C }.
 ********************************************************************************/
LISTNODEPTR
SeqLoops_getLoopContainerExtensionsInReverse(SeqNodeDataPtr _nodeDataPtr,
                                             const char *depIndex) {
  LISTNODEPTR loopExtensions = NULL, prod = NULL, rhs = NULL;
  SeqNameValuesPtr depArgs = NULL, arg = NULL;

  SeqUtil_TRACE(TL_FULL_TRACE,
                "SeqLoops_getLoopContainerExtensionsInReverse(): Begin call\n");

  if (depIndex == NULL || strlen(depIndex) == 0) {
    SeqListNode_pushFront(&loopExtensions, "");
    SeqUtil_TRACE(
        TL_FULL_TRACE,
        "No extension, returning 1 elem list containing empty string");
    goto out;
  }
...
out:
  SeqUtil_TRACE(
      TL_FULL_TRACE,
      "SeqLoops_getLoopContainerExtensionsInReverse(): returning list:   \n");
  SeqListNode_printList(loopExtensions);
  SeqUtil_TRACE(TL_FULL_TRACE, "\n");
  return loopExtensions;
}
...
loopArg_to_reverse_extension_list(SeqNodeDataPtr _nodeDataPtr,
  ...
  if (!hasWildCard(arg)) {
    ...
    if (loopPtr != NULL)
      ...
      raiseError("loopArg_to_reverse_extension_list() unable to find loop "
                 "matching arg->name=%s\n",
                 arg->name);
  }
...
/********************************************************************************
 * Looks through a SeqLoopsPtr linked list to find a loopPtr whose loop_name
 *field has a leaf matching the name argument.
 ********************************************************************************/
SeqLoopsPtr SeqLoops_findLoopByName(SeqLoopsPtr loopsPtr, char *name) {
  SeqLoopsPtr current = loopsPtr;
  SeqUtil_TRACE(
      TL_FULL_TRACE,
      "SeqLoops_findLoopByName(): Looking for loop matching name=%s\n", name);
  for (current = loopsPtr; current != NULL; current = current->nextPtr) {
    if (strcmp(SeqUtil_getPathLeaf(current->loop_name), name) == 0) {
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "SeqLoops_findLoopByName(): match found %s\n", name);
      return current;
    }
  }
  SeqUtil_TRACE(TL_FULL_TRACE,
                "SeqLoops_findLoopByName(): no match found. returning NULL\n");
  return NULL;
}
...
numerical_to_reverse_extension_list(SeqNameValuesPtr loopData) {
  ...
  SeqUtil_TRACE(TL_FULL_TRACE,
                "numerical_to_reverse_extension_list(): begin call\n");

  if ((attrib = (char *)SeqLoops_getLoopAttribute(loopData, "EXPRESSION")) !=
          NULL &&
      strcmp(attrib, "") != 0) {
    retval = expression_to_reverse_extension_list(attrib);
  } else {
    if ((attrib = (char *)SeqLoops_getLoopAttribute(loopData, "START")) !=
        NULL) {
      start = atoi(attrib);
      free(attrib);
    }
    ...
    if ((attrib = (char *)SeqLoops_getLoopAttribute(loopData, "STEP")) !=
        NULL) {
      step = atoi(attrib);
      free(attrib);
    }
  ...
  SeqUtil_TRACE(TL_FULL_TRACE,
                "numerical_to_reverse_extension_list(): returning list: ");
  SeqListNode_printList(retval);
  SeqUtil_TRACE(TL_FULL_TRACE, "\n");
  return retval;
}
...
static LISTNODEPTR expression_to_reverse_extension_list(char *expression) {
  ...
  for (i = 0, token = strtok(expression, ":,"); token != NULL;
       i++, token = strtok(NULL, ":,"))
  ...
  for (i = number_of_definitions; i > 0; i--, currentDef -= 4) {
    rhs = def_to_reverse_extension_list(
        currentDef[DEF_START], currentDef[DEF_END], currentDef[DEF_STEP]);
 ...
 * a child of a loop container or loop containers, defined in the dep_index list
 * of loops iterations. For instance, this function is used to support
 * dependency wildcard when a node is dependant on all the iterations of a node
 * that is a child of a loop container
 */
LISTNODEPTR SeqLoops_getLoopContainerExtensions(SeqNodeDataPtr _nodeDataPtr,
                                                const char *depIndex) {
  char tmp[100];
  int expressionArray[256], i, j, startIndex = 0, endIndex = 1, stepIndex = 2,
                                  detectedEnd = 0, breakFlag = 0, firstFlag = 1,
                                  detectedStart;
  ...
  memset(tmp, '\0', sizeof tmp);
  SeqUtil_TRACE(TL_FULL_TRACE,
                "SeqLoops_getLoopContainerExtensions depIndex: %s\n", depIndex);

  /* looping over each depedency index argument */
  while (depArgs != NULL) {
    SeqUtil_TRACE(
        TL_FULL_TRACE,
        "SeqLoops_getLoopContainerExtensions dealing with argument %s=%s\n",
        depArgs->name, depArgs->value);
    if (strstr(depArgs->value, "*") != NULL) {
      /* wildcard found */
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "SeqLoops_getLoopContainerExtensions getting loop "
                    "information from node %s\n",
                    _nodeDataPtr->name);
      ...
      while (loopsPtr != NULL && !foundIt) {
        ...
        if (loopsDataPtr != NULL) {
          ...
          if (strcmp(SeqUtil_getPathLeaf(loopsPtr->loop_name), depArgs->name) ==
              0) {
            loopsDataPtr = loopsDataPtr->nextPtr;

            tmpExpression =
                SeqLoops_getLoopAttribute(loopsDataPtr, "EXPRESSION");
            if (tmpExpression != NULL && strcmp(tmpExpression, "") != 0) {
              SeqUtil_TRACE(
                  TL_FULL_TRACE,
                  "SeqLoops_getLoopContainerExtensions expression: %s\n",
                  tmpExpression);
            } else {
              loopStart =
                  atoi(SeqLoops_getLoopAttribute(loopsDataPtr, "START"));
              if (SeqLoops_getLoopAttribute(loopsDataPtr, "STEP") != NULL) {
                loopStep =
                    atoi(SeqLoops_getLoopAttribute(loopsDataPtr, "STEP"));
              }
              loopEnd = atoi(SeqLoops_getLoopAttribute(loopsDataPtr, "END"));
              loopCount = loopStart;
              SeqUtil_TRACE(TL_FULL_TRACE,
                            "SeqLoops_getLoopContainerExtensions loopStart:%d "
                            "loopStep:%d loopEnd=%d \n",
      ...
      /* if targetting a loop node as a dependency, _nodeDataPtr->data will have
       * the loop attributes */
      loopsDataPtr = _nodeDataPtr->data;
      if ((strcmp(_nodeDataPtr->nodeName, depArgs->name) == 0) &&
          (loopsDataPtr != NULL)) {
        SeqUtil_TRACE(
            TL_FULL_TRACE,
            "SeqLoops_getLoopContainerExtensions targetting a loop node\n");
      ...
      loopEnd = loopCount;
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "SeqLoops_getLoopContainerExtensions Found single value "
                    "for loop: loop name = %s ; loop value = %d \n",
    ...
    do {
      if (tmpExpression != NULL && strcmp(tmpExpression, "") != 0 &&
        ...
        while (loopCount <= detectedEnd) {
          ...
          breakFlag = 0;
          if ((startIndex - 4) >= 0 &&
              loopCount != expressionArray[endIndex - 4]) {
            firstFlag = 1;
          }
          while (loopCount <= expressionArray[endIndex] && breakFlag == 0) {
            if (firstFlag == 0) {
            ...
            if (breakFlag == 0) {
              if (loopExtensions != NULL) {
                sprintf(tmp, "%s%s%d", loopExtensions->data, EXT_TOKEN,
                        loopCount);
              } else {
        ...
        while (loopCount <= loopEnd) {
          ...
          SeqUtil_TRACE(
              TL_FULL_TRACE,
              "SeqLoops_getLoopContainerExtensions new extension added:%s\n",
              tmp);
          SeqListNode_insertItem(&tmpLoopExts, tmp);
          loopCount = loopCount + loopStep;
        }
...
/* If the user doesn't provide a loop extension, we're returning the first loop
   iteration If the user provides a loop extension, we're returning the current
   extension We're assuming that the _nodeDataPtr->extension has already been
   set previously To be used when the current node is of type loop */
SeqNameValuesPtr SeqLoops_submitLoopArgs(const SeqNodeDataPtr _nodeDataPtr,
                                         SeqNameValuesPtr _loop_args) {
  SeqNameValuesPtr newLoopsArgsPtr = NULL, loopArgsTmpPtr = NULL;
  SeqNameValuesPtr nodeSpecPtr = NULL;
  char *tmpExpression = NULL, *tmpArrayValue = NULL;
  char loopStart[10];
  int foundExt = 0, _i, _j, expressionArray[256], detectedEnd = 0,
      detectedStart;
  ...
  if (!foundExt) {
    ...
    if (tmpExpression != NULL && strcmp(tmpExpression, "") != 0) {
      ...
      sprintf(loopStart, "%d", detectedStart);
      SeqUtil_TRACE(TL_FULL_TRACE, "SeqLoops_submitLoopArgs() loopstart:%s\n",
                    loopStart);
      SeqLoops_setLoopAttribute(&newLoopsArgsPtr, _nodeDataPtr->nodeName,
                                loopStart);

    } else {

      sprintf(loopStart, "%s", SeqLoops_getLoopAttribute(nodeSpecPtr, "START"));
      SeqUtil_TRACE(TL_FULL_TRACE, "SeqLoops_submitLoopArgs() loopstart:%s\n",
                    loopStart);
      SeqLoops_setLoopAttribute(&newLoopsArgsPtr, _nodeDataPtr->nodeName,
                                loopStart);
 ...
 * returns 0 otherwise
 ********************************************************************************/
int SeqLoops_isLastIteration(const SeqNodeDataPtr _nodeDataPtr,
                             SeqNameValuesPtr _loop_args) {
  SeqNameValuesPtr nodeSpecPtr = NULL;
  char tmp[20];
  ...
  if ((loopCurrentStr = SeqLoops_getLoopAttribute(
           _loop_args, _nodeDataPtr->nodeName)) != NULL)
  ...
  if (tmpExpression != NULL && strcmp(tmpExpression, "") != 0) {
    ...
    if (loopCurrent ==
        lastDefIter(lastDef[DEF_START], lastDef[DEF_END], lastDef[DEF_STEP])) {
      isLast = 1;
    }
    free(tmpExpression);
  } else {
    ...
    SeqUtil_TRACE(
        TL_MEDIUM,
        "SeqLoops_isLastIteration() loopCurrent:%d loopStep:%d loopTotal:%d\n",
 ...
 ********************************************************************************/
SeqNameValuesPtr SeqLoops_nextLoopArgs(const SeqNodeDataPtr _nodeDataPtr,
                                       SeqNameValuesPtr _loop_args,
                                       int *_newDefNumber) {

  SeqNameValuesPtr nextLoopArgsPtr = NULL;
  SeqNameValuesPtr nodeSpecPtr = _nodeDataPtr->data;
  const int loopCurrent =
      atoi(SeqLoops_getLoopAttribute(_loop_args, _nodeDataPtr->nodeName));
  char *expression, *token;
  char nextIterStr[200];
  int expressionArray[256], index = 0, startIndex = 0, nextIter, loopStart,
                            loopSet = 1, loopStep = 1, loopEnd;
  int *currentDef;
  ...
  if (expression != NULL && strcmp(expression, "")) {
    ...
    startIndex = 0;
    while (((startIndex + 4) < index) &&
           !isInDefinition(loopCurrent, expressionArray[startIndex + DEF_START],
    ...
    if (isInDefinition(nextIter, currentDef[DEF_START], currentDef[DEF_END],
                       currentDef[DEF_STEP])) {
      ...
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "SeqLoops_nextLoopArgs(): setting loop attribure of %s\n",
                    _nodeDataPtr->nodeName);
      SeqLoops_setLoopAttribute(&nextLoopArgsPtr, _nodeDataPtr->nodeName,
                                nextIterStr);
    } else if (loopCurrent == lastDefIter(currentDef[DEF_START],
                                          currentDef[DEF_END],
                                          currentDef[DEF_STEP])) {
      if (!currentDefIsLast) {
        /* Inform caller that a new definition is to be started by * specifying
         * it's number*/
        *_newDefNumber = (startIndex / 4) + 1;
        SeqUtil_TRACE(TL_MEDIUM,
                      "SeqLoops_nextLoopArgs():Informing caller of "
                      "newdefinition, and also returning newdefinition number "
                      "= %d within expressionArray.\n",
                      *_newDefNumber);
      }
    }
  } else {
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "SeqLoops_nextLoopArgs(): Running non-expression code \n");
...
int SeqLoops_validateLoopArgs(const SeqNodeDataPtr _nodeDataPtr,
                              SeqNameValuesPtr _loop_args) {
  SeqLoopsPtr loopsPtr = _nodeDataPtr->loops;
  ...
  if ((_nodeDataPtr->type == NpassTask || _nodeDataPtr->type == ForEach) &&
      tmpValue != NULL) {
  ...
  if (loopsPtr != NULL) {
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "SeqLoops_validateLoopArgs() loop pointer found\n");
    /* check if the user has given us argument for looping */
    if (_loop_args == NULL) {
      raiseError("SeqLoops_validateLoopArgs(): No loop arguments found for "
                 "container loop!\n");
    }

    /* build loop extension for containers */
    if ((loopExtension = (char *)SeqLoops_ContainerExtension(
             loopsPtr, _loop_args)) == NULL) {
      raiseError("SeqLoops_validateLoopArgs(): Missing loop arguments for "
                 "container loop!\n");
    }
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "SeqLoops_validateLoopArgs() loop extension:%s\n",
                  loopExtension);
  } else {
    SeqUtil_TRACE(TL_ERROR,
                  "SeqLoops_validateLoopArgs() loop pointer not found\n");
  }
 ...
 ********************************************************************************/
SeqNameValuesPtr SeqLoops_getContainerArgs(const SeqNodeDataPtr _nodeDataPtr,
                                           SeqNameValuesPtr _loop_args) {
  ...
  SeqUtil_TRACE(
      TL_FULL_TRACE,
      "SeqLoops_getContainerArgs(): Called for node %s and _loop_args :\n",
  ...
  while (token != NULL && current != NULL) {
    if (strcmp(token, current->name) == 0) {
      /* Issue when token is duplicated and the final token is an npasstask. Ex:
       * /path/to/node/node and the last node is an npasstask */
      if (!(_nodeDataPtr->type == NpassTask &&
            strcmp(token, _nodeDataPtr->nodeName) == 0)) {
        SeqUtil_TRACE(
            TL_FULL_TRACE,
            "SeqLoops_getContainerArgs adding loop item %s of value %s \n",
  ...
  free(path);
  SeqUtil_TRACE(TL_FULL_TRACE,
                "SeqLoops_getContainerArgs(): returning loopArgs : \n");
 ...
 ********************************************************************************/
SeqNameValuesPtr SeqLoops_getLoopSetArgs(const SeqNodeDataPtr _nodeDataPtr,
                                         SeqNameValuesPtr _loop_args,
                                         int defNumber) {
  ...
  char tmp[20];
  int loopStart = 0, loopEnd = 0, loopSet = 1, loopStep = 1, loopCount = 0,
      loopSetCount = 0;
  char *tmpExpression = NULL, *tmpArrayValue = NULL;
  ...
  int *currentDef = NULL;
  SeqUtil_TRACE(TL_FULL_TRACE,
                "SeqLoops_getLoopSetArgs(): Called with defNumber = %d\n",
                defNumber);
  ...
  if (!foundExt) {
    ...
    if (tmpExpression != NULL && strcmp(tmpExpression, "") != 0) {
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "SeqLoops_getLoopSetArgs() loop expression:%s\n",
                    tmpExpression);
      ...
      SeqUtil_TRACE(TL_FULL_TRACE, "SeqLoops_getLoopSetArgs():Definition for "
                                   "which to submit initial set is \n");
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "\t Start = %d, end = %d, step = %d, set = %d \n",
                    currentDef[DEF_START], currentDef[DEF_END],
                    currentDef[DEF_STEP], currentDef[DEF_SET]);
      loopCount = currentDef[DEF_START];
      /* Deal with the possibility of current definition's start being equal to
      ...
      while (loopCount <= currentDef[DEF_END] &&
             loopSetCount < currentDef[DEF_SET]) {
        sprintf(tmp, "%d", loopCount);
        SeqNameValues_insertItem(&newLoopsArgsPtr, _nodeDataPtr->nodeName, tmp);
        loopCount = loopCount + currentDef[DEF_STEP];
        loopSetCount++;
      }
      SeqUtil_TRACE(TL_FULL_TRACE, "SeqLoops_getLoopSetArgs():Returning "
                                   "newLoopsArgsPtr with contents: \n");
      ...
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "SeqLoops_getLoopSetArgs() loopstart:%d loopSet:%d "
                    "loopStep:%d loopEnd:%d\n",
...
/* checks if values are valid for a numerical loop in an expression format, so
 * step !=0, set !=0, loop start <= loop end when step is positive, and loop
 * start >= loop end when step is negative */
...
void SeqLoops_validateNumLoopExpression(char *_expression) {
  ...
  while ((tmpExpression != NULL) && (done == 0)) {
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "SeqLoops_validateNumLoopExpression working expression %s \n",
                  tmpExpression);
    ...
    SeqUtil_TRACE(
        TL_FULL_TRACE,
        "SeqLoops_validateNumLoopExpression checking sub-expression %s \n",
        expressionBuffer);
    if (sscanf(expressionBuffer, "%d:%d:%d:%d", &tmpStart, &tmpEnd, &tmpStep,
               &tmpSet) != 4) {
      raiseError("SeqLoops_validateNumLoopExpression format error in "
                 "expression.Is %s; Accepted format is comma-separated list of "
                 "start:end:step:set.\n",
                 expressionBuffer);
    }
    if (tmpStep == 0) {
      raiseError("SeqLoops_validateNumLoopExpression value error. loop step "
                 "(%d) cannot be 0. Readjust value.\n",
                 tmpStep);
    }
    if (tmpSet <= 0) {
      raiseError("SeqLoops_validateNumLoopExpression value error. loop set "
                 "(%d) must be greater than 0. Readjust value.\n",
                 tmpSet);
    }

    if ((tmpStart > tmpEnd) && (tmpStep > 0)) {
      raiseError(
          "SeqLoops_validateNumLoopExpression value error. loop start (%d) > "
          "loop end (%d)  with loop step > 0 (%d) . Readjust values.\n",
