/* nodeinfo.c - Creator of the node construct used by the Maestro sequencer
 * software package.
 */
...
void parseBatchResources(xmlXPathObjectPtr _result,
  ...
  if (_result) {
    ...
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "nodeinfo.parseBatchResources() nodeset->nodeNr=%d\n",
                  nodeset->nodeNr);
    for (i = 0; i < nodeset->nodeNr; i++) {
      nodePtr = nodeset->nodeTab[i];
      nodeName = nodePtr->name;
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "nodeinfo.parseBatchResources() nodePtr->name=%s\n",
                    nodePtr->name);
      ...
      if (strcmp(nodeName, "cpu") == 0) {
        ...
        /* if cpu has already been set, and the mpi flag is on, it will need to
         * be recalculated depending on the format it may change for npex / omp
         */
        if (cpuString != NULL && _nodeDataPtr->mpi)
          SeqNode_setCpu(_nodeDataPtr, cpuString);
      } else if (strcmp(nodeName, "soumet_args") == 0) {
        /* add soumet args in the following order: 1) resource file 2) args sent
         * by command line, who will override 1*/
        ...
        raiseError(
            "nodeinfo.parseBatchResources() ERROR: Unprocessed attribute=%s\n",
            nodeName);
      }
    }
  }
 ...
 ********************************************************************************/
int nodeToDepArgs(SeqNodeDataPtr _nodeDataPtr, SeqNameValuesPtr *depArgs,
                  SeqNameValuesPtr *localArgs) {
  ...
  while (loopsPtr != NULL) {
    if (strstr(_nodeDataPtr->pathToModule, loopsPtr->loop_name) != NULL) {
      ...
      char *value =
          SeqNameValues_getValue(_nodeDataPtr->loop_args, tmpLoopName);
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "Nodeinfo_nodeToDepArgs() adding loop argument %s=%s\n",
                    tmpLoopName, value);
      if (value) {
  ...
  if (SeqLoops_parseArgs(localArgs, dep->local_index) != -1) {
    ...
    for (itr = *localArgs; itr != NULL; itr = itr->nextPtr) {
      ...
      if (strcmp(itr->value, "CURRENT_INDEX") == 0) {
        char *tmpValue = NULL;
        if ((tmpValue = SeqNameValues_getValue(_nodeDataPtr->loop_args,
                                               itr->name)) != NULL) {
        ...
        SeqUtil_TRACE(TL_FULL_TRACE, "Nodeinfo_parseDepends() itr->name=%s \n",
                      itr->name);
        if ((tok_value = SeqNameValues_getValue(_nodeDataPtr->loop_args,
                                                itr->name)) != NULL) {
 ...
 ********************************************************************************/
void validateDepIndices(SeqNodeDataPtr _nodeDataPtr, SeqDepDataPtr dep,
                        int isIntraDep) {
  SeqUtil_TRACE(TL_FULL_TRACE, "validateDepIndices() begin\n");
  SeqUtil_TRACE(TL_FULL_TRACE,
                "Nodeinfo_parseDepends() dep->local_index = %s\n",
                dep->local_index);
 ...
 ********************************************************************************/
SeqDepDataPtr xmlDepNode_to_depDataPtr(SeqNodeDataPtr _nodeDataPtr,
                                       xmlNodePtr nodePtr, int isIntraDep) {
...
void parseDepends(xmlXPathObjectPtr _result, SeqNodeDataPtr _nodeDataPtr,
  ...
  for (i = 0; i < nodeset->nodeNr; i++) {
    ...
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "nodeinfo.parseDepends()   *** depends_item=%s ***\n",
                  nodePtr->name);
    ...
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "nodeinfo.parseDepends() Parsing Dependency Type:%s\n",
                  depType);
    if (depType == NULL)
      depType = strdup("node");
    ...
    if (strcmp(depType, "node") == 0) {
      ...
      SeqDepDataPtr dep =
          xmlDepNode_to_depDataPtr(_nodeDataPtr, nodePtr, isIntraDep);
      ...
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "nodeinfo.parseDepends() no dependency found.\n");
    }
    free(depType);
#endif
  }
...
void parseLoopAttributes(xmlXPathObjectPtr _result, const char *_loop_node_path,
                         SeqNodeDataPtr _nodeDataPtr) {
  xmlNodeSetPtr nodeset;
  xmlNodePtr nodePtr;
  const xmlChar *nodeName = NULL;
  xmlChar *loopStart = DEFAULT_LOOP_START_STR,
          *loopStep = DEFAULT_LOOP_STEP_STR, *loopEnd = DEFAULT_LOOP_END_STR,
          *loopSet = DEFAULT_LOOP_SET_STR, *loopExpression = strdup("");
  int i = 0;

  if (_result) {
    nodeset = _result->nodesetval;
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "nodeinfo.parseLoopAttributes()   *** loop info***\n");

    for (i = 0; i < nodeset->nodeNr; i++) {
      nodePtr = nodeset->nodeTab[i];
      nodeName = nodePtr->name;
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "nodeinfo.parseLoopAttributes() nodeName=%s, value:%s\n",
  ...
  if (_result) {
    ...
    for (i = 0; i < nodeset->nodeNr; i++) {
      ...
      while (propertiesPtr != NULL) {
        ...
        } else if (strcmp(propertyName, "type") == 0 &&
                   strcmp(propertiesPtr->children->content, "user") == 0) {
          isSubmit = 0;
          SeqUtil_TRACE(TL_FULL_TRACE,
                        "nodeinfo.parseSubmits() got user submit node\n");
        }
...
void parseAbortActions(xmlXPathObjectPtr _result, SeqNodeDataPtr _nodeDataPtr) {
  ...
  if (_result) {
    nodeset = _result->nodesetval;
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "nodeinfo.parseAbortActions() nodeset->nodeNr=%d\n",
                  nodeset->nodeNr);
    for (i = 0; i < nodeset->nodeNr; i++) {
      nodePtr = nodeset->nodeTab[i];
      nodeName = nodePtr->name;
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "nodeinfo.parseAbortActions() nodePtr->name=%s\n",
                    nodePtr->name);
...
/* to parse attributes that are specific to nodes for example
   loops(start-step-end), case (exec_script) etc there should only be one node
   in the result set */
void parseNodeSpecifics(SeqNodeType _nodeType, xmlXPathObjectPtr _result,
                        SeqNodeDataPtr _nodeDataPtr) {
  xmlNodeSetPtr nodeset = NULL;
  xmlNodePtr nodePtr = NULL;
  const xmlChar *nodeName = NULL;
  int i = 0;
  SeqUtil_TRACE(TL_FULL_TRACE,
                "nodeinfo.parseNodeSpecifics() called node_type=%s\n",
                SeqNode_getTypeString(_nodeType));
  nodeset = _result->nodesetval;
  SeqUtil_TRACE(TL_FULL_TRACE,
                "nodeinfo.parseNodeSpecifics() nodeset cardinal=%d\n",
                nodeset->nodeNr);
  for (i = 0; i < nodeset->nodeNr; i++) {
    nodePtr = nodeset->nodeTab[i];
    nodeName = nodePtr->name;
    ...
    if (nodePtr->children != NULL && strcmp((char *)nodeName, "name") != 0) {
      SeqUtil_TRACE(TL_FULL_TRACE, "nodeinfo.parseNodeSpecifics() %s=%s\n",
                    nodeName, nodePtr->children->content);
      SeqNode_addSpecificData(_nodeDataPtr, nodeName,
                              nodePtr->children->content);
    }
  }
  if (_nodeType == Loop) {
...
/* To parse the siblings to generate a list. This list will be used to monitor
 * container ends.  */
    ...
    for (i = 0; i < nodeset->nodeNr; i++) {
      ...
      while (propertiesPtr != NULL) {
        ...
        if (strcmp((char *)propertyName, "name") == 0) {
          ...
          SeqUtil_TRACE(TL_FULL_TRACE,
                        "nodeinfo.parseNodeSiblings() sibling:%s\n",
                        propertiesPtr->children->content);
        }
        propertiesPtr = propertiesPtr->next;
      }
...
char *switchReturn(SeqNodeDataPtr _nodeDataPtr, const char *switchType) {
  ...
  if (strcmp(switchType, "datestamp_hour") == 0) {
    ...
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "switchReturn datestamp parser on datestamp = %s\n",
  ...
  if (strcmp(switchType, "day_of_week") == 0) {
    ...
    SeqUtil_TRACE(
        TL_FULL_TRACE,
        "switchReturn datestamp parser on day of the week for date: %s%s%s \n",
        year, month, day);
    sprintf(returnValue, "%d",
            SeqDatesUtil_dow(atoi(year), atoi(month), atoi(day)));
  }
...
/*  parseForEachTarget
Parses the information from the ressources file to know the ForEach container
target and adds it to the node input: _result = FOR_EACH xml tag and attributes
...
*/
void parseForEachTarget(xmlXPathObjectPtr _result,
                        SeqNodeDataPtr _nodeDataPtr) {
  ...
  if (_result) {
    nodeset = _result->nodesetval;
    SeqUtil_TRACE(TL_FULL_TRACE,
                  "nodeinfo.parseForEachTarget() nodeset->nodeNr=%d\n",
                  nodeset->nodeNr);
    for (i = 0; i < nodeset->nodeNr; i++) {
      nodePtr = nodeset->nodeTab[i];
      nodeName = nodePtr->name;
      SeqUtil_TRACE(TL_FULL_TRACE,
                    "nodeinfo.parseForEachTarget() nodePtr->name=%s\n",
                    nodePtr->name);
    ...
    if (t_exp == NULL)
      t_exp = strdup(_nodeDataPtr->expHome);
    if (t_hour == NULL)
      t_hour = strdup("0");
    if ((t_node == NULL) || (t_index == NULL)) {
      raiseError("Node's xml resource file does not contain mandatory node "
                 "and/or index attributes.\n");
    }
...
void getFlowInfo(SeqNodeDataPtr _nodeDataPtr, const char *_seq_exp_home,
                 const char *_nodePath, const char *switch_args,
                 unsigned int filters) {
  if (_nodePath == NULL || strcmp(_nodePath, "") == 0)
    raiseError("Calling getFlowInfo() with an empty path'\n");
  FlowVisitorPtr flow_visitor =
      Flow_newVisitor(_nodePath, _seq_exp_home, switch_args);
 ...
 * flow visitor.
 *********************************************************************************/
int doesNodeExist(const char *_nodePath, const char *_seq_exp_home,
                  const char *_datestamp) {
