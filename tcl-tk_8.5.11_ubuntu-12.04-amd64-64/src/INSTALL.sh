...
if [[ ! -d tcl8.5.11 ]] ; then
  ...
  cd tcl8.5.11/unix
  ./configure --enable-threads --enable-shared --prefix=${InstallPrefix} --enable-64bit
  make 
  make install
fi
cd ${InstallPrefix}/bin
rm -f wish tclsh maestro_tclsh8.5 maestro_wish8.5
ln -s tclsh8.5 tclsh
ln -s wish8.5 wish
...
if [[ ! -d thread2.6.7 ]] ; then
  ...
  cd thread2.6.7
  ./configure --enable-threads --enable-shared --prefix=${InstallPrefix} -with-tcl=${InstallPrefix}/lib --with-tclinclude=${InstallPrefix}/include --enable-64bit
  make 
  make install
fi
cd $ICI
...
if [[ ! -d Tktable2.10 ]] ; then
  ...
  ./configure --enable-threads --enable-shared --prefix=${InstallPrefix} -with-tcl=${InstallPrefix}/lib --with-tclinclude=${InstallPrefix}/include \
              --with-tk=${InstallPrefix}/lib --enable-64bit
  make 
  make install
fi
cd $ICI
